...
/// Returns an diem channel for testing
fn create_diem_channel<K: Eq + Hash + Clone, T>(
) -> (diem_channel::Sender<K, T>, diem_channel::Receiver<K, T>) {
...
fn create_network_sender_and_events(
    ...
    HashMap<NetworkId, diem_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>>,
    HashMap<NetworkId, diem_channel::Sender<(PeerId, ProtocolId), PeerManagerNotification>>,
) {
    ...
    for network_id in network_ids {
        ...
        let (inbound_request_sender, inbound_request_receiver) = create_diem_channel();
        let (outbound_request_sender, outbound_request_receiver) = create_diem_channel();
        let (connection_outbound_sender, _connection_outbound_receiver) = create_diem_channel();
        let (_connection_inbound_sender, connection_inbound_receiver) = create_diem_channel();
