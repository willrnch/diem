...
pub struct Peer<TSocket> {
    ...
    connection_notifs_tx: diem_channels::Sender<TransportNotification<TSocket>>,
    ...
    TSocket: AsyncRead + AsyncWrite + Send + 'static,
{
    ...
    fn start_writer_task(
        ...
        max_frame_size: usize,
        max_message_size: usize,
    ) -> (diem_channels::Sender<NetworkMessage>, oneshot::Sender<()>) {
        let remote_peer_id = connection_metadata.remote_peer_id;
        let (write_reqs_tx, mut write_reqs_rx): (diem_channels::Sender<NetworkMessage>, _) =
            diem_channels::new(1024, &counters::PENDING_WIRE_MESSAGES);
        let (close_tx, mut close_rx) = oneshot::channel();

        let (mut msg_tx, msg_rx) = diem_channels::new(1024, &counters::PENDING_MULTIPLEX_MESSAGE);
        let (stream_msg_tx, stream_msg_rx) =
            diem_channels::new(1024, &counters::PENDING_MULTIPLEX_STREAM);
