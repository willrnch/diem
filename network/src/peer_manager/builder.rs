...
use crate::{
    ...
    transport::{self, DiemNetTransport, Connection, DIEM_TCP_TRANSPORT},
    ProtocolId,
};
...
use diem_netcore::transport::memory::MemoryTransport;
use diem_netcore::transport::{
...
type MemoryPeerManager =
    PeerManager<DiemNetTransport<MemoryTransport>, NoiseStream<diem_memsocket::MemorySocket>>;
...
impl PeerManagerBuilder {
    ...
    pub fn build(&mut self, executor: &Handle) -> &mut Self {
        ...
        let mut diem_tcp_transport = DIEM_TCP_TRANSPORT.clone();
        let tcp_cfg = self.get_tcp_buffers_cfg();
        diem_tcp_transport.set_tcp_buffers(&tcp_cfg);
