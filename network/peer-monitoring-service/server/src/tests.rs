...
use diem_peer_monitoring_service_types::{
...
use diem_storage_interface::{DbReader, ExecutedTrees, Order};
        ...
        async fn test_performance_monitoring_request() {
            ...
            for i in 0..10 {
                let request = PeerMonitoringServiceRequest::PerformanceMonitoringRequest(
                    diem_peer_monitoring_service_types::request::PerformanceMonitoringRequest {
                        request_counter: i,
...
fn transform_connection_metadata(
    expected_peers: BTreeMap<PeerNetworkId, ConnectionMetadata>,
) -> BTreeMap<PeerNetworkId, diem_peer_monitoring_service_types::response::ConnectionMetadata> {
    expected_peers
        .into_iter()
        .map(|(peer_id, metadata)| {
            let connection_metadata =
                diem_peer_monitoring_service_types::response::ConnectionMetadata::new(
                    metadata.addr,
...
async fn verify_node_information(
    ...
    let expected_response =
        PeerMonitoringServiceResponse::NodeInformation(NodeInformationResponse {
            build_information: diem_build_info::get_build_information(),
            highest_synced_epoch,
            highest_synced_version,
            ledger_timestamp_usecs,
            lowest_available_version,
...
impl MockClient {
    fn new(
        ...
        for network_id in network_ids {
            let queue_cfg = diem_channel::Config::new(
