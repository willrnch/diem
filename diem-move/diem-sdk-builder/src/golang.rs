        ...
        .filter(|abi| {
            if let EntryABI::EntryFunction(sf) = abi {
                sf.module_name().name().as_str() != "code"
                    ...
                    && sf.module_name().name().as_str() != "diem_account"
    ...
    fn output_encode_method(&mut self, abis: &[EntryABI]) -> Result<()> {
        ...
        if !transaction_script_abis.is_empty() {
            writeln!(
                self.out,
                r#"
// Build an Diem `Script` from a structured object `ScriptCall`.
...
func EncodeEntryFunction(call EntryFunctionCall) diemtypes.TransactionPayload {{"#
            )?;
            self.out.indent();
            writeln!(self.out, "switch call := call.(type) {{")?;
    ...
    fn output_transaction_script_decode_method(&mut self) -> Result<()> {
        writeln!(
            self.out,
            r#"
// Try to recognize an Diem `Script` and convert it into a structured object `ScriptCall`.
...
func DecodeEntryFunctionPayload(script diemtypes.TransactionPayload) (EntryFunctionCall, error) {{
    switch script := script.(type) {{
        case *diemtypes.TransactionPayload__EntryFunction:
    ...
    fn output_entry_function_encoder_function(&mut self, abi: &EntryFunctionABI) -> Result<()> {
        writeln!(
            self.out,
            "\n{}\nfunc Encode{}{}({}) diemtypes.TransactionPayload {{",
            Self::quote_doc(abi.doc()),
            abi.module_name().name().to_string().to_camel_case(),
            abi.name().to_camel_case(),
            [
        ...
        writeln!(
            self.out,
            r#"return &diemtypes.TransactionPayload__EntryFunction {{
            diemtypes.EntryFunction {{
                Module: {},
                Function: {},
                TyArgs: []diemtypes.TypeTag{{{}}},
                Args: [][]byte{{{}}},
    ...
    fn output_transaction_script_decoder_function(
        ...
        writeln!(
            self.out,
            "\nfunc decode_{}(script *diemtypes.Script) (ScriptCall, error) {{",
            abi.name(),
        )?;
        self.out.indent();
        writeln!(
...
"#,
    ...
    fn output_entry_function_decoder_function(&mut self, abi: &EntryFunctionABI) -> Result<()> {
        writeln!(
            self.out,
            "\nfunc decode_{}_{}(script diemtypes.TransactionPayload) (EntryFunctionCall, error) {{",
...
var script_decoder_map = map[string]func(*diemtypes.Script) (ScriptCall, error) {{"#
    ...
    fn output_entry_function_decoder_map(&mut self, abis: &[EntryFunctionABI]) -> Result<()> {
        writeln!(
            self.out,
            r#"
var entry_function_decoder_map = map[string]func(diemtypes.TransactionPayload) (EntryFunctionCall, error) {{"#
        )?;
        self.out.indent();
        for abi in abis {
            writeln!(
...
"#,
    ...
    fn quote_transaction_argument_for_script(type_tag: &TypeTag, name: &str) -> String {
        ...
        match type_tag {
            ...
            U16 => format!("(*diemtypes.TransactionArgument__U16)(&{})", name),
            U32 => format!("(*diemtypes.TransactionArgument__U32)(&{})", name),
            U64 => format!("(*diemtypes.TransactionArgument__U64)(&{})", name),
            U128 => format!("(*diemtypes.TransactionArgument__U128)(&{})", name),
            U256 => format!("(*diemtypes.TransactionArgument__U256)(&{})", name),
