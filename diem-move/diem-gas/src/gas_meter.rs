...
pub struct NativeGasParameters {
    pub move_stdlib: diem_move_stdlib::natives::GasParameters,
    pub diem_framework: diem_framework::natives::GasParameters,
...
impl FromOnChainGasSchedule for NativeGasParameters {
    fn from_on_chain_gas_schedule(
        ...
        Ok(Self {
            ...
            diem_framework: FromOnChainGasSchedule::from_on_chain_gas_schedule(
                gas_schedule,
                feature_version,
            )?,
...
impl NativeGasParameters {
    pub fn zeros() -> Self {
        Self {
            move_stdlib: diem_move_stdlib::natives::GasParameters::zeros(),
            diem_framework: diem_framework::natives::GasParameters::zeros(),
...
impl InitialGasSchedule for NativeGasParameters {
    fn initial() -> Self {
        Self {
            ...
            diem_framework: InitialGasSchedule::initial(),
            table: InitialGasSchedule::initial(),
        }
    }
}

/// Gas parameters for everything that is needed to run the Diem blockchain, including
/// instructions, transactions and native functions from various packages.
#[derive(Debug, Clone)]
pub struct DiemGasParameters {
...
impl FromOnChainGasSchedule for DiemGasParameters {
    fn from_on_chain_gas_schedule(
        gas_schedule: &BTreeMap<String, u64>,
        feature_version: u64,
    ) -> Result<Self, String> {
...
impl ToOnChainGasSchedule for DiemGasParameters {
    fn to_on_chain_gas_schedule(&self, feature_version: u64) -> Vec<(String, u64)> {
...
impl InitialGasSchedule for DiemGasParameters {
    fn initial() -> Self {
        Self {
...
/// Trait representing a gas meter used inside the Diem VM.
///
/// It extends Move VM's `GasMeter` trait with a few Diem-specific callbacks, along with
/// some primitives that would allow gas meters to be composable.
pub trait DiemGasMeter: MoveGasMeter {
...
/// The official gas meter used inside the Diem VM.
...
impl MoveGasMeter for StandardGasMeter {
    ...
    fn charge_native_function_before_execution(
        ...
        // TODO(Gas): https://github.com/aptos-labs/diem-core/issues/5485
...
impl DiemGasMeter for StandardGasMeter {
    fn feature_version(&self) -> u64 {
        self.feature_version
    }
