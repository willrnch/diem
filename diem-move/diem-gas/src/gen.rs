...
use crate::gas_meter::{
    DiemGasParameters, InitialGasSchedule, ToOnChainGasSchedule, LATEST_GAS_FEATURE_VERSION,
};
use anyhow::Result;
use diem_package_builder::PackageBuilder;
use diem_types::on_chain_config::GasScheduleV2;
use clap::Parser;
...
fn generate_script(gas_schedule: &GasScheduleV2) -> Result<String> {
    ...
    emitln!(writer, "use diem_framework::gas_schedule;");
    emitln!(writer);
...
fn diem_framework_path() -> PathBuf {
    Path::join(
        Path::new(env!("CARGO_MANIFEST_DIR")),
        "../framework/diem-framework",
    )
}
...
pub fn generate_update_proposal(args: &GenArgs) -> Result<()> {
    ...
    pack.add_local_dep("DiemFramework", &diem_framework_path().to_string_lossy());
