...
use crate::{DiemGasParameters, LATEST_GAS_FEATURE_VERSION};
use diem_types::{
    on_chain_config::{ConfigStorage, OnChainConfig, StorageGasSchedule},
    state_store::state_key::StateKey,
    write_set::WriteOp,
};
use diem_vm_types::{change_set::VMChangeSet, check_change_set::CheckChangeSet};
use move_core_types::{
...
impl StoragePricingV1 {
    fn new(gas_params: &DiemGasParameters) -> Self {
        Self {
...
impl StoragePricingV2 {
    pub fn zeros() -> Self {
        Self::new_without_storage_curves(LATEST_GAS_FEATURE_VERSION, &DiemGasParameters::zeros())
    }
...
impl ChangeSetConfigs {
    ...
    pub fn new(feature_version: u64, gas_params: &DiemGasParameters) -> Self {
    ...
    pub fn legacy_resource_creation_as_modification(&self) -> bool {
        ...
        // modification.
        // Modules and table items were not affected (https://github.com/aptos-labs/diem-core/pull/4722/commits/7c5e52297e8d1a6eac67a68a804ab1ca2a0b0f37).
    ...
    fn from_gas_params(gas_feature_version: u64, gas_params: &DiemGasParameters) -> Self {
        Self::new_impl(
            gas_feature_version,
