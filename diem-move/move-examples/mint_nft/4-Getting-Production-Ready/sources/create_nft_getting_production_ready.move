...
/// We use signature validation for many reasons. For example, in diem_framework::account::rotate_authentication_key(),
...
/// For more information about events, see: https://diem.dev/concepts/events/.
///
/// Move unit tests
/// We added a few unit tests to make sure that our code is working as expected. For more information on how to write
/// Move unit tests, see: https://diem.dev/guides/move-guides/book/unit-testing
///
...
/// Run the following command in directory `diem-core/diem-move/move-examples/mint_nft/4-Getting-Production-Ready`.
/// - 2.b Run the following CLI command to publish the module under a resource account.
/// diem move create-resource-account-and-publish-package --seed [seed] --address-name mint_nft --profile default --named-addresses source_addr=[default account's address]
///
...
/// diem move run --function-id [resource account's address]::create_nft_getting_production_ready::set_public_key --args hex:[public key we just generated] --profile admin
/// (To find the public key we just generated, run `nano output.key.pub`.)
/// example output:
/*
4-Getting-Production-Ready % diem move run --function-id f634035fea40e23c5ed8817f7e996d96372dd5dbd64e853fb3c108817d92dcb4::create_nft_getting_production_ready::set_public_key --args hex:5C0637A3865FCA80550502BC30C8E7B4CCA7C8AB3B4FFECEFC8C43F7D0D44DEE --profile admin
...
///  Open up file `diem-core/diem-move/e2e-move-tests/src/tests/mint_nft.rs`.
...
///  You can find the `admin_private_key` by running `nano output.key`, and the `receiver_account_sequence_number` by looking up the receiver's address on the Diem Explorer under tab `Info`.
///  Make sure you're in the right directory.
///  Run the following command in directory `diem-core/diem-move/e2e-move-tests`
...
/// diem move run --function-id [resource account's address]::create_nft_getting_production_ready::mint_event_ticket --args hex:[signature generated in last step] --profile nft-receiver
/// example output:
/*
4-Getting-Production-Ready % diem move run --function-id f634035fea40e23c5ed8817f7e996d96372dd5dbd64e853fb3c108817d92dcb4::create_nft_getting_production_ready::mint_event_ticket --args hex:fc833512ad1c575850569d14f5e434b929a19eb491c08df9f6b91584a13551bdb95830081a429f148fddcb9ba201cf72a357957849046da0d60675ed034f580f --profile nft-receiver
...
module mint_nft::create_nft_getting_production_ready {
    ...
    #[test (origin_account = @0xcafe, resource_account = @0xc3bb8488ab1a5815a9d543d7e41b0e0df46a7396f89b22821f07a4362f75ddc5, nft_receiver = @0x123, nft_receiver2 = @0x234, diem_framework = @diem_framework)]
    public entry fun test_happy_path(origin_account: signer, resource_account: signer, nft_receiver: signer, nft_receiver2: signer, diem_framework: signer) acquires ModuleData {
        let (admin_sk, admin_pk) = ed25519::generate_keys();
        set_up_test(origin_account, &resource_account, &admin_pk, diem_framework, &nft_receiver, 10);
    ...
    #[test (origin_account = @0xcafe, resource_account = @0xc3bb8488ab1a5815a9d543d7e41b0e0df46a7396f89b22821f07a4362f75ddc5, nft_receiver = @0x123, diem_framework = @diem_framework)]
    #[expected_failure(abort_code = 0x50002, location = mint_nft::create_nft_getting_production_ready)]
    public entry fun test_minting_expired(origin_account: signer, resource_account: signer, nft_receiver: signer, diem_framework: signer) acquires ModuleData {
        let (admin_sk, admin_pk) = ed25519::generate_keys();
        set_up_test(origin_account, &resource_account, &admin_pk, diem_framework, &nft_receiver, 100000000001);
        let receiver_addr = signer::address_of(&nft_receiver);
        let proof_challenge = MintProofChallenge {
            receiver_account_sequence_number: account::get_sequence_number(receiver_addr),
            receiver_account_address: receiver_addr,
    ...
    #[test (origin_account = @0xcafe, resource_account = @0xc3bb8488ab1a5815a9d543d7e41b0e0df46a7396f89b22821f07a4362f75ddc5, admin = @admin_addr, nft_receiver = @0x123, diem_framework = @diem_framework)]
    #[expected_failure(abort_code = 0x50002, location = mint_nft::create_nft_getting_production_ready)]
    public entry fun test_update_expiration_time(origin_account: signer, resource_account: signer, admin: signer, nft_receiver: signer, diem_framework: signer) acquires ModuleData {
        let (admin_sk, admin_pk) = ed25519::generate_keys();
        set_up_test(origin_account, &resource_account, &admin_pk, diem_framework, &nft_receiver, 10);
        let receiver_addr = signer::address_of(&nft_receiver);
        let proof_challenge = MintProofChallenge {
    ...
    public entry fun test_update_minting_enabled(origin_account: signer, resource_account: signer, admin: signer, nft_receiver: signer, diem_framework: signer) acquires ModuleData {
        let (admin_sk, admin_pk) = ed25519::generate_keys();
        set_up_test(origin_account, &resource_account, &admin_pk, diem_framework, &nft_receiver, 10);
        let receiver_addr = signer::address_of(&nft_receiver);
        let proof_challenge = MintProofChallenge {
    ...
    public entry fun test_invalid_signature(origin_account: signer, resource_account: signer, nft_receiver: signer, diem_framework: signer) acquires ModuleData {
        let (admin_sk, admin_pk) = ed25519::generate_keys();
        set_up_test(origin_account, &resource_account, &admin_pk, diem_framework, &nft_receiver, 10);
        let receiver_addr = signer::address_of(&nft_receiver);
        let proof_challenge = MintProofChallenge {
