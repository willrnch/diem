...
module dao_platform::nft_dao {
    use diem_framework::account::{SignerCapability, create_signer_with_capability};
    ...
    use diem_std::table;
    use diem_token::property_map::PropertyMap;
    use diem_token::property_map;
    use diem_token::token::{Self, TokenId, create_token_id_raw};
    ...
    public fun create_dao_and_get_dao_address(
        ...
        // register diem coin
        coin::register<DiemCoin>(&res_signer);
    ...
    fun transfer_fund(res_acct: &signer, dst: address, amount: u64) {
        coin::transfer<DiemCoin>(res_acct, dst, amount);
    }
    ...
    #[test_only]
    use diem_token::token::create_token_script;
    #[test_only]
    use diem_framework::diem_account::transfer_coins;
    #[test_only]
    use diem_framework::diem_coin;
    use diem_token::token_transfers;
    ...
    #[test(diem_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_e2e_scenario(diem_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        ...
        coin::register<DiemCoin>(creator);
        coin::register<DiemCoin>(voter);
        coin::deposit(creator_addr, coins);
        coin::destroy_burn_cap(burn_cap);
        coin::destroy_mint_cap(mint_cap);

        // now resource account has a fund pool of 90 coins
        transfer_coins<DiemCoin>(creator, res_acc, 90);
        ...
        assert!(coin::balance<DiemCoin>(signer::address_of(voter)) == 45, 1);
    }

    #[test(diem_framework = @0x1, admin = @0xdeaf, new_admin = @0xaf)]
    public fun test_dao_offer_and_claim(diem_framework: &signer, admin: &signer, new_admin: &signer) acquires DAO {
        // admin creates a dao
        timestamp::set_time_has_started_for_testing(diem_framework);
    ...
    #[test(diem_framework = @0x1, admin = @0xdeaf)]
    public fun test_transferring_ownership_to_zero_address(diem_framework: &signer, admin: &signer) acquires DAO {
        // admin creates a dao
        timestamp::set_time_has_started_for_testing(diem_framework);
        let old_addr = signer::address_of(admin);
        account::create_account_for_test(@0x1);
    ...
    public fun test_double_vote(diem_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(diem_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(diem_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_resolve_with_no_sufficient_votes(diem_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(diem_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    public fun test_resolve_earlier_than_ending_time(diem_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(diem_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(diem_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_admin_execute_proposal(diem_framework: &signer, creator: &signer, voter: &signer)acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(diem_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(diem_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_admin_veto_a_proposal(diem_framework: &signer, creator: &signer, voter: &signer)acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(diem_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(diem_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_set_dao_config(diem_framework: &signer, creator: &signer, voter: &signer) acquires DAO {
        timestamp::set_time_has_started_for_testing(diem_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(diem_framework = @0x1, admin = @0xdeaf, new_admin = @0xaf)]
    public fun test_admin_create_proposal_without_token(diem_framework: &signer, admin: &signer, new_admin: &signer) acquires DAO, Proposals {
        // admin creates a dao
        timestamp::set_time_has_started_for_testing(diem_framework);
        let new_addr = signer::address_of(new_admin);
        let old_addr = signer::address_of(admin);
    ...
    #[test(diem_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_transfer_multiple_nfts(diem_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(diem_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
