---
title: "Diem Move Tutorial"
slug: "move-tutorial"
---

# Move Tutorial

Welcome to the Diem Move Tutorial! This is the original Move language and tools tutorial, independent of a particular network, adapted to use Diem tooling. Here you will learn basic usage of the Move language and tools for compiling, testing, and verifying Move.

This tutorial does not teach you how to use the existing [Diem Move frameworks](https://diem.dev/reference/move) or how to run code on the Diem network. See the [Diem developer documentation](../../../developer-docs-site/docs/tutorials/index.md) for those instructions.
...
You also need a recent version of the [Diem CLI](../../../developer-docs-site/docs/tools/install-cli/index.md). This tutorial is written using the following version:
...
If you want to use an IDE for working through the examples, we recommend CLion/IntelliJ, which has good support for Diem Move.
...
Let us make sure it builds! This can be done with the `diem move compile` command from within the package directory ([`step_1/basic_coin`](./step_1/basic_coin/)):
...
You can run the tests with the `diem move test` command:
...
* Change the assertion to `11` so that the test fails. Find a flag that you can
  pass to the `diem move test` command that will dump the global state when
...
* Find a flag that allows you to gather test coverage information, and
  then experiment with using the `diem move coverage` command to examine
...
> NOTE: The coin and balance interfaces are for illustration of Move concepts only. Diem uses a different, richer [coin type](https://github.com/aptos-labs/diem-core/blob/main/diem-move/framework/diem-framework/doc/coin.md#0x1_coin) contained in the Diem framework.
...
[`error` module]: https://github.com/aptos-labs/diem-core/blob/main/diem-move/framework/move-stdlib/sources/error.move
...
To get started, run the `diem move test` command in the [`step_5/basic_coin`](./step_5/basic_coin) directory:
...
Read more about [phantom type](../../../developer-docs-site/docs/move/book/generics.md#phantom-type-parameters) parameters in the Diem Move Book.
</details>
