...
module collection_offer {
    ...
    use diem_framework::object::{Self, DeleteRef, Object};
    use diem_framework::timestamp;

    use diem_token::token as tokenv1;

    use diem_token_objects::collection::Collection;
    use diem_token_objects::royalty;
    use diem_token_objects::token::{Self as tokenv2, Token as TokenV2};
...
module collection_offer_tests {
    ...
    use diem_framework::object;
    use diem_framework::option;
    use diem_framework::timestamp;

    use diem_token::token as tokenv1;

    use diem_token_objects::collection as collectionv2;
    use diem_token_objects::token as tokenv2;
    ...
    fun test_token_v2(
        ...
        let collection_offer = collection_offer::init_for_tokenv2<DiemCoin>(
            purchaser,
        ...
        assert!(coin::balance<DiemCoin>(purchaser_addr) == 8999, 0);
        assert!(coin::balance<DiemCoin>(seller_addr) == 10000, 0);

        collection_offer::sell_tokenv2<DiemCoin>(seller, collection_offer, token);
        assert!(coin::balance<DiemCoin>(marketplace_addr) == 6, 0);
        assert!(coin::balance<DiemCoin>(purchaser_addr) == 8999, 0);
        assert!(coin::balance<DiemCoin>(seller_addr) == 10495, 0);
        assert!(object::is_owner(token, purchaser_addr), 0);
        assert!(collection_offer::remaining(collection_offer) == 1, 0);

        collection_offer::sell_tokenv2<DiemCoin>(purchaser, collection_offer, token);
        assert!(coin::balance<DiemCoin>(marketplace_addr) == 11, 0);
        assert!(coin::balance<DiemCoin>(purchaser_addr) == 9489, 0);
        assert!(coin::balance<DiemCoin>(seller_addr) == 10500, 0);
        assert!(object::is_owner(token, purchaser_addr), 0);
    ...
    fun test_token_v1_direct_deposit(
        ...
        let collection_offer = collection_offer::init_for_tokenv1<DiemCoin>(
            purchaser,
            creator_addr,
            collection_name,
            test_utils::fee_schedule(marketplace),
        ...
        collection_offer::sell_tokenv1<DiemCoin>(seller, collection_offer, token_name, property_version);
        assert!(coin::balance<DiemCoin>(marketplace_addr) == 6, 0);
        assert!(coin::balance<DiemCoin>(purchaser_addr) == 8999, 0);
        assert!(coin::balance<DiemCoin>(seller_addr) == 10495, 0);
        ...
        collection_offer::sell_tokenv1<DiemCoin>(purchaser, collection_offer, token_name, property_version);
        assert!(coin::balance<DiemCoin>(marketplace_addr) == 11, 0);
        assert!(coin::balance<DiemCoin>(purchaser_addr) == 9489, 0);
        assert!(coin::balance<DiemCoin>(seller_addr) == 10500, 0);
        assert!(tokenv1::balance_of(purchaser_addr, token_id) == 1, 0);
    ...
    fun test_token_v1_indirect(
        ...
        let token_container = collection_offer::sell_tokenv1<DiemCoin>(
            seller,
            collection_offer,
            token_name,
            property_version,
    ...
    #[test(diem_framework = @0x1, marketplace = @0x111, seller = @0x222, purchaser = @0x333)]
    #[expected_failure(abort_code = 0x50004, location = diem_framework::object)]
    fun test_token_v2_has_none(
        ...
        collection_offer::sell_tokenv2<DiemCoin>(marketplace, collection_offer, token);
    }

    #[test(diem_framework = @0x1, marketplace = @0x111, seller = @0x222, purchaser = @0x333)]
    #[expected_failure(abort_code = 0x10005, location = diem_token::token)]
    fun test_token_v1_has_none(
        ...
        collection_offer::sell_tokenv1<DiemCoin>(
            marketplace,
            collection_offer,
            token_name,
            property_version,
