...
module coin_listing {
    ...
    use diem_framework::object::{Self, ConstructorRef, Object, ObjectCore};
...
module listing_tests {
    ...
    fun test_fixed_price(
        ...
        let (marketplace_addr, seller_addr, purchaser_addr) =
            test_utils::setup(diem_framework, marketplace, seller, purchaser);

        let (token, fee_schedule, listing) = fixed_price_listing(marketplace, seller);

        assert!(coin::balance<DiemCoin>(marketplace_addr) == 1, 0);
        assert!(coin::balance<DiemCoin>(seller_addr) == 9999, 0);
        assert!(listing::listed_object(listing) == object::convert(token), 0);
        assert!(listing::fee_schedule(listing) == fee_schedule, 0);
        assert!(coin_listing::price<DiemCoin>(listing) == option::some(500), 0);
        assert!(!coin_listing::is_auction<DiemCoin>(listing), 0);

        coin_listing::purchase<DiemCoin>(purchaser, listing);

        assert!(object::owner(token) == purchaser_addr, 0);
        assert!(coin::balance<DiemCoin>(marketplace_addr) == 6, 0);
        assert!(coin::balance<DiemCoin>(seller_addr) == 10494, 0);
        assert!(coin::balance<DiemCoin>(purchaser_addr) == 9500, 0);
    }

    #[test(diem_framework = @0x1, marketplace = @0x111, seller = @0x222, purchaser = @0x333)]
    fun test_fixed_price_end(
        ...
        assert!(coin::balance<DiemCoin>(marketplace_addr) == 1, 0);
        coin_listing::end_fixed_price<DiemCoin>(seller, listing);
        assert!(coin::balance<DiemCoin>(marketplace_addr) == 1, 0);
        assert!(coin::balance<DiemCoin>(seller_addr) == 9999, 0);
    ...
    fun test_auction_purchase(
        ...
        assert!(coin_listing::price<DiemCoin>(listing) == option::some(500), 0);
        assert!(coin_listing::is_auction<DiemCoin>(listing), 0);
        assert!(coin_listing::starting_bid<DiemCoin>(listing) == 100, 0);
        assert!(coin_listing::bid_increment<DiemCoin>(listing) == 50, 0);
        assert!(coin_listing::auction_end_time<DiemCoin>(listing) == timestamp::now_seconds() + 200, 0);
        assert!(coin_listing::minimum_bid_time_before_end<DiemCoin>(listing) == 150, 0);
        assert!(coin_listing::current_amount<DiemCoin>(listing) == option::none(), 0);
        assert!(coin_listing::current_bidder<DiemCoin>(listing) == option::none(), 0);
    ...
    fun test_auction_bid_then_purchase(
        ...
        coin_listing::bid<DiemCoin>(seller, listing, 100);
        assert!(coin_listing::current_amount<DiemCoin>(listing) == option::some(100), 0);
        assert!(coin_listing::current_bidder<DiemCoin>(listing) == option::some(seller_addr), 0);
        assert!(coin::balance<DiemCoin>(marketplace_addr) == 3, 0);
        assert!(coin::balance<DiemCoin>(seller_addr) == 9897, 0);

        // Return the bid and insert a new bid
        coin_listing::bid<DiemCoin>(purchaser, listing, 150);
        assert!(coin_listing::current_amount<DiemCoin>(listing) == option::some(150), 0);
        assert!(coin_listing::current_bidder<DiemCoin>(listing) == option::some(purchaser_addr), 0);
        assert!(coin::balance<DiemCoin>(marketplace_addr) == 5, 0);
        assert!(coin::balance<DiemCoin>(seller_addr) == 9997, 0);
        assert!(coin::balance<DiemCoin>(purchaser_addr) == 9848, 0);

        // Return the bid and replace with a purchase
        coin_listing::purchase<DiemCoin>(purchaser, listing);
        assert!(object::owner(token) == purchaser_addr, 0);
        assert!(coin::balance<DiemCoin>(marketplace_addr) == 10, 0);
        assert!(coin::balance<DiemCoin>(purchaser_addr) == 9498, 0);
    }
    ...
    fun test_auction_bidding(
        ...
        assert!(coin_listing::auction_end_time<DiemCoin>(listing) == end_time, 0);
        ...
        test_utils::increment_timestamp(150);
        coin_listing::complete_auction<DiemCoin>(listing);
        assert!(object::owner(token) == purchaser_addr, 0);
        assert!(coin::balance<DiemCoin>(marketplace_addr) == 6, 0);
        assert!(coin::balance<DiemCoin>(seller_addr) == 10146, 0);
    }
    ...
    fun test_not_started_fixed_price(
        ...
        let listing = coin_listing::init_fixed_price_internal<DiemCoin>(
            seller,
    ...
    fun test_not_started_auction(
        ...
        let token = test_utils::mint_tokenv2(seller);
        let fee_schedule = test_utils::fee_schedule(marketplace);
        let listing = coin_listing::init_auction_internal<DiemCoin>(
            seller,
            object::convert(token),
            fee_schedule,
            timestamp::now_seconds() + 1,
        ...
        coin_listing::bid<DiemCoin>(purchaser, listing, 1000);
    }
    ...
    #[test(diem_framework = @0x1, marketplace = @0x111, seller = @0x222, purchaser = @0x333)]
    #[expected_failure(abort_code = 0x10006, location = diem_framework::coin)]
    ...
    fun test_not_enough_coin_auction_bid(
        ...
        let (_token, _fee_schedule, listing) = auction_listing(marketplace, seller);
        coin_listing::bid<DiemCoin>(purchaser, listing, 100000);
    }
    ...
    fun test_bid_too_low(
        ...
        let (_token, _fee_schedule, listing) = auction_listing(marketplace, seller);
        coin_listing::bid<DiemCoin>(purchaser, listing, 100);
        coin_listing::bid<DiemCoin>(purchaser, listing, 125);
    }
    ...
    fun test_auction_view_on_fixed_price(
        ...
        coin_listing::auction_end_time<DiemCoin>(listing);
    }
    ...
    fun test_bad_fixed_price_end(
        ...
        let (_token, _fee_schedule, listing) = fixed_price_listing(marketplace, seller);
        coin_listing::end_fixed_price<DiemCoin>(purchaser, listing);
    }
    ...
    inline fun fixed_price_listing_for_tokenv1(
        ...
        let fee_schedule = test_utils::fee_schedule(marketplace);
        let listing = coin_listing::init_fixed_price_for_tokenv1_internal<DiemCoin>(
            seller,
            creator_addr,
            collection_name,
            token_name,
    ...
    inline fun auction_listing_for_tokenv1(
        ...
        let (creator_addr, collection_name, token_name, property_version) =
            tokenv1::get_token_id_fields(&token_id);
        let fee_schedule = test_utils::fee_schedule(marketplace);
        let listing = coin_listing::init_auction_for_tokenv1_internal<DiemCoin>(
            seller,
            creator_addr,
            collection_name,
            token_name,
