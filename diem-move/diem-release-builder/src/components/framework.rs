...
use crate::{diem_core_path, components::get_execution_hash};
use anyhow::Result;
...
pub struct FrameworkReleaseConfig {
    ...
    /// If set to None, we will use the diem framework under current repo.
...
pub fn generate_upgrade_proposals(
    ...
    let mut package_path_list = vec![
        ("0x1", "diem-move/framework/move-stdlib"),
        ("0x1", "diem-move/framework/diem-stdlib"),
        ("0x1", "diem-move/framework/diem-framework"),
    ...
    let commit_info = if let Some(revision) = &config.git_hash {
        ...
        let repository = Repository::clone(DIEM_GIT_PATH, temp_root_path.path())?;
    ...
    for (publish_addr, relative_package_path) in package_path_list.iter() {
        ...
        // 0-move-stdlib.move	2-diem-framework.move	4-gas-schedule.move	6-features.move
        // 1-diem-stdlib.move	3-diem-token.move	5-version.move		7-consensus-config.move
        // The first framework file being generated is 3-diem-token.move. It's using the next_execution_hash being passed in (so in this case, the hash of 4-gas-schedule.move being passed in mod.rs).
        // The second framework file being generated would be 2-diem-framework.move, and it's using the hash of 3-diem-token.move (which would be result.last()).
