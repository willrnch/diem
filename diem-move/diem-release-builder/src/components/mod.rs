...
use crate::{diem_core_path, diem_framework_path, components::feature_flags::Features};
use anyhow::{anyhow, bail, Context, Result};
use diem::governance::GenerateExecutionHash;
...
fn default_url() -> String {
    "https://github.com/aptos-labs/diem-core".to_string()
}
...
impl ReleaseEntry {
    pub fn generate_release_script(
        ...
        match self {
            ...
            ReleaseEntry::DefaultGas => {
                let gas_schedule = diem_gas::gen::current_gas_schedule();
                if !fetch_and_equals::<GasScheduleV2>(client, &gas_schedule)? {
                    result.append(&mut gas::generate_gas_upgrade_proposal(
                        &gas_schedule,
                        is_testnet,
            ...
            ReleaseEntry::FeatureFlag(feature_flags) => {
                ...
                if let Some(client) = client {
                    let features = block_on(async {
                        client
                            .get_account_resource_bcs::<diem_types::on_chain_config::Features>(
                                CORE_CODE_ADDRESS,
            ...
            ReleaseEntry::RawScript(script_path) => {
                let base_path = diem_core_path().join(script_path.as_path());
    ...
    pub fn validate_upgrade(&self, client: &Client) -> Result<()> {
        ...
        match self {
            ...
            ReleaseEntry::DefaultGas => {
                if !fetch_and_equals(client_opt, &diem_gas::gen::current_gas_schedule())? {
...
pub fn get_execution_hash(result: &Vec<(String, String)>) -> Vec<u8> {
    if result.is_empty() {
        ...
        let (_, hash) = GenerateExecutionHash {
            ...
            framework_local_dir: Some(diem_framework_path()),
        }
        .generate_hash()
        .unwrap();
        hash.to_vec()
...
impl Default for ProposalMetadata {
    fn default() -> Self {
        ProposalMetadata {
            ...
            // Diem CLI need a valid url for the two fields.
