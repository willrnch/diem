...
use diem_api_types::AsConverter;
use diem_crypto::{
    ed25519::{Ed25519PrivateKey, Ed25519PublicKey},
    hash::HashValue,
    ValidCryptoMaterialStringExt,
};
use diem_gas::{InitialGasSchedule, TransactionGasParameters};
use diem_language_e2e_tests::data_store::{FakeDataStore, GENESIS_CHANGE_SET_HEAD};
use diem_state_view::TStateView;
use diem_types::{
    access_path::AccessPath,
    account_config::{diem_test_root_address, AccountResource, CoinStoreResource},
...
use diem_vm::{data_cache::AsMoveResolver, DiemVM, VMExecutor};
...
impl diem_storage_interface::DbReader for FakeDbReader {
...
static PRECOMPILED_DIEM_FRAMEWORK: Lazy<FullyCompiledProgram> = Lazy::new(|| {
    let deps = vec![PackagePaths {
        name: None,
        paths: diem_cached_packages::head_release_bundle()
            .files()
            .unwrap(),
        named_address_map: diem_framework::named_addresses().clone(),
    }];
...
impl<'a> DiemTestAdapter<'a> {
    ...
    /// Obtain the DiemCoin amount under address `signer_addr`
    fn fetch_account_balance(&self, signer_addr: &AccountAddress) -> Result<u64> {
        let diem_coin_tag = CoinStoreResource::struct_tag();

        let coin_access_path =
            AccessPath::resource_access_path(*signer_addr, diem_coin_tag.clone())
                .expect("access path in test");
        ...
        let annotated = MoveValueAnnotator::new(&self.storage.as_move_resolver())
            .view_resource(&diem_coin_tag, &balance_blob)?;
    ...
    fn run_transaction(&mut self, txn: Transaction) -> Result<TransactionOutput> {
        let mut outputs = DiemVM::execute_block(vec![txn], &self.storage.clone(), None)?;
    ...
    fn create_and_fund_account(&mut self, account_addr: AccountAddress, amount: u64) {
        let parameters = self
            .fetch_transaction_parameters(&diem_test_root_address(), None, None, None, None)
            .unwrap();

        let txn = RawTransaction::new(
            diem_test_root_address(),
            parameters.sequence_number,
            diem_cached_packages::diem_stdlib::diem_account_create_account(account_addr),
        ...
        let txn = RawTransaction::new(
            ...
            diem_cached_packages::diem_stdlib::diem_coin_mint(account_addr, amount),
            parameters.max_gas_amount,
            parameters.gas_unit_price,
            parameters.expiration_timestamp_secs,
            ChainId::test(),
        ...
        self.run_transaction(Transaction::UserTransaction(txn))
            .expect("Failed to mint diem coin. This should not happen.");
    }
}
...
impl<'a> MoveTestAdapter<'a> for DiemTestAdapter<'a> {
    ...
    fn init(
        ...
        let mut named_address_mapping = diem_framework::named_addresses().clone();
    ...
    fn handle_subcommand(&mut self, input: TaskInput<Self::Subcommand>) -> Result<Option<String>> {
        match input.command {
            ...
            DiemSubCommand::ViewTableCommand(view_table_cmd) => {
...
pub fn run_diem_test(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
