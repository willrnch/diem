...
use crate::{assert_success, DiemPackageHooks};
use anyhow::Error;
use diem::move_tool::MemberId;
use diem_cached_packages::diem_stdlib;
use diem_crypto::{ed25519::Ed25519PrivateKey, PrivateKey, Uniform};
use diem_framework::{natives::code::PackageMetadata, BuildOptions, BuiltPackage};
use diem_gas::{
    DiemGasParameters, FromOnChainGasSchedule, InitialGasSchedule, ToOnChainGasSchedule,
};
...
impl MoveHarness {
    ...
    pub fn new() -> Self {
        register_package_hooks(Box::new(DiemPackageHooks {}));
        Self {
    ...
    /// Gets the account where the Diem framework is installed (0x1).
    pub fn diem_framework_account(&mut self) -> Account {
    ...
    pub fn create_publish_package(
        ...
        self.create_transaction_payload(
            account,
            diem_stdlib::code_publish_package_txn(
    ...
    pub fn enable_features(&mut self, enabled: Vec<FeatureFlag>, disabled: Vec<FeatureFlag>) {
        let acc = self.diem_framework_account();
    ...
    pub fn increase_transaction_size(&mut self) {
        // TODO: The DiemGasParameters::zeros() schedule doesn't do what we want, so
        ...
        let entries = DiemGasParameters::initial()
            .to_on_chain_gas_schedule(diem_gas::LATEST_GAS_FEATURE_VERSION);
        let entries = entries
            .into_iter()
            .map(|(name, val)| {
    ...
    pub fn modify_gas_schedule(&mut self, modify: impl FnOnce(&mut DiemGasParameters)) {
        let gas_schedule: GasScheduleV2 = self
            .read_resource(&CORE_CODE_ADDRESS, GasScheduleV2::struct_tag())
            .unwrap();
        let feature_version = gas_schedule.feature_version;
        let mut gas_params = DiemGasParameters::from_on_chain_gas_schedule(
...
macro_rules! enable_golden {
    ($h:expr) => {
        $h.internal_set_golden(
            std::file!(),
            diem_language_e2e_tests::current_function_name!(),
        )
    };
}
...
macro_rules! assert_abort {
    ($s:expr, $c:pat) => {{
        assert!(matches!(
            $s,
            diem_types::transaction::TransactionStatus::Keep(
                diem_types::transaction::ExecutionStatus::MoveAbort { code: $c, .. }
            ),
        ));
    }};
}
