...
use diem_cached_packages::{diem_stdlib, diem_token_sdk_builder};
use diem_crypto::{bls12381, PrivateKey, Uniform};
use diem_gas_profiling::TransactionGasLog;
use diem_types::account_address::{default_stake_pool_address, AccountAddress};
use diem_vm::DiemVM;
...
fn test_gas() {
    ...
    DiemVM::set_paranoid_type_checks(true);

    run(
        &mut harness,
        "Transfer",
        account_1,
        diem_stdlib::diem_coin_transfer(account_2_address, 1000),
    );
    ...
    run(
        ...
        "CreateAccount",
        account_1,
        diem_stdlib::diem_account_create_account(
    ...
    run(
        ...
        "CreateTransfer",
        account_1,
        diem_stdlib::diem_account_transfer(
    ...
    run(
        ...
        "RotateConsensusKey",
        account_2,
        diem_stdlib::stake_rotate_consensus_key(
            pool_address,
            consensus_pubkey,
            proof_of_possession,
        ),
    ...
    run(
        ...
        "JoinValidator100",
        account_2,
        diem_stdlib::stake_join_validator_set(pool_address),
    );
    run(
        &mut harness,
        "AddStake",
        account_1,
        diem_stdlib::staking_contract_add_stake(account_2_address, 1000),
    );
    run(
        &mut harness,
        "UnlockStake",
        account_1,
        diem_stdlib::staking_contract_unlock_stake(account_2_address, 1000),
    );
    ...
    run(
        ...
        "WithdrawStake",
        account_1,
        diem_stdlib::staking_contract_distribute(account_1_address, account_2_address),
    );
    run(
        &mut harness,
        "LeaveValidatorSet100",
        account_2,
        diem_stdlib::stake_leave_validator_set(pool_address),
    );
    ...
    run(
        ...
        "CreateCollection",
        account_1,
        diem_token_sdk_builder::token_create_collection_script(
    ...
    run(
        ...
        account_1,
        diem_token_sdk_builder::token_create_token_script(
            collection_name.clone(),
            token_name.clone(),
    ...
    run(
        ...
        "MintToken",
        account_1,
        diem_token_sdk_builder::token_mint_script(
            account_1_address,
            collection_name.clone(),
            token_name.clone(),
            1,
    ...
    run(
        ...
        "MutateToken",
        account_1,
        diem_token_sdk_builder::token_mutate_token_properties(
            account_1_address,
            account_1_address,
            collection_name.clone(),
            token_name.clone(),
    ...
    let publisher = &harness.diem_framework_account();
    publish(
        &mut harness,
        "PublishLarge",
        publisher,
