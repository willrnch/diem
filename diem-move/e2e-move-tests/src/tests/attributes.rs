...
use diem_types::{account_address::AccountAddress, on_chain_config::FeatureFlag};
...
fn test_bad_fun_attribute_in_compiled_module() {
    ...
    let mut value = diem_framework::RuntimeModuleMetadataV1 {
    ...
    .unwrap();
    let known_attribute =
        bcs::from_bytes::<diem_framework::KnownAttribute>(&fake_attribute).unwrap();
    value
        .fun_attributes
        .insert("view".to_string(), vec![known_attribute]);

    let metadata = Metadata {
        key: diem_framework::DIEM_METADATA_KEY_V1.to_vec(),
...
fn test_bad_view_attribute_in_compiled_module() {
    ...
    let result = h.run_transaction_payload(
        &account,
        diem_stdlib::code_publish_package_txn(metadata, code),
    );
