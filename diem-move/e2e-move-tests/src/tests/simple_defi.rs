...
const DIEM_COIN_STRUCT_STRING: &str = "0x1::coin::CoinStore<0x1::diem_coin::DiemCoin>";
...
fn exchange_e2e_test() {
    ...
    let result = h.run_transaction_payload(
        &origin_account,
        diem_stdlib::resource_account_create_resource_account_and_publish_package(
            vec![],
    ...
    assert_coin_store_balance(
        ...
        DIEM_COIN_STRUCT_STRING,
        20,
    );

    // swap from 5 diem coins to 5 chloe's coins
    ...
    assert_coin_store_balance(&h, &resource_address, DIEM_COIN_STRUCT_STRING, 5);
    assert_coin_store_balance(&h, &resource_address, CHLOE_COIN_STRUCT_STRING, 0);

    // swap to 3 diem coins from 3 chloe's diem coins
    ...
    assert_coin_store_balance(&h, &resource_address, DIEM_COIN_STRUCT_STRING, 2);
    assert_coin_store_balance(&h, &resource_address, CHLOE_COIN_STRUCT_STRING, 0);
}
