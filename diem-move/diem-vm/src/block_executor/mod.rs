...
use crate::{
    ...
    block_executor::vm_wrapper::DiemExecutorTask,
    counters::{
...
use diem_vm_logging::{flush_speculative_logs, init_speculative_logs};
...
#[derive(Debug)]
pub struct DiemTransactionOutput {
...
impl BlockExecutorTransactionOutput for DiemTransactionOutput {
...
impl BlockDiemVM {
        ...
        match transactions {
            BlockExecutorTransactions::Unsharded(transactions) => {
                let signature_verified_txns = transactions
                    .into_par_iter()
                    .with_min_len(25)
                    .map(preprocess_transaction::<DiemVM>)
                    .collect();
                    ...
                    .map(|sub_block| {
                        ...
                        let verified_txns = sub_block
                            ...
                            .map(|txn_with_deps| {
                                ...
                                let preprocessed_txn = preprocess_transaction::<DiemVM>(txn);
    ...
    pub fn execute_block<
        ...
        L: TransactionCommitHook<
            ExecutionStatus = BlockExecutorExecutionStatus<DiemTransactionOutput, Error<VMStatus>>,
        >,
    >(
