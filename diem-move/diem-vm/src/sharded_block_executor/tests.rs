...
use crate::{
    sharded_block_executor::sharded_executor_client::ShardedExecutorClient, DiemVM,
    ShardedBlockExecutor, VMExecutor,
};
use diem_block_partitioner::sharded_block_partitioner::ShardedBlockPartitioner;
...
fn test_sharded_block_executor_no_conflict() {
    ...
    let sharded_txn_output = sharded_block_executor
        ...
        .unwrap();
    let unsharded_txn_output = DiemVM::execute_block(
...
fn test_sharded_block_executor_with_conflict() {
    ...
    let unsharded_txn_output =
        DiemVM::execute_block(execution_ordered_txns, &executor.data_store(), None).unwrap();
    compare_txn_outputs(unsharded_txn_output, sharded_txn_output);
}
