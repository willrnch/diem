...
use diem_framework::prover::ProverOptions;
...
pub fn run_prover_for_pkg(path_to_pkg: impl Into<String>) {
    ...
    if no_tools {
        ...
        See https://github.com/aptos-labs/diem-core/blob/main/diem-move/framework/FRAMEWORK-PROVER-GUIDE.md \
        for instructions, or \
...
fn move_framework_prover_tests() {
    run_prover_for_pkg("diem-framework");
}

#[test]
fn move_token_prover_tests() {
    run_prover_for_pkg("diem-token");
}

#[test]
fn move_diem_stdlib_prover_tests() {
    run_prover_for_pkg("diem-stdlib");
}
