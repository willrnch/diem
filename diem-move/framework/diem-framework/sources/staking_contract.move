...
module diem_framework::staking_contract {
    use std::bcs;
    use std::error;
    use std::signer;
    use std::vector;
    ...
    use diem_framework::account::{Self, SignerCapability};
    use diem_framework::diem_coin::DiemCoin;
    use diem_framework::coin::{Self, Coin};
    use diem_framework::event::{EventHandle, emit_event};
    use diem_framework::stake::{Self, OwnerCapability};
    use diem_framework::staking_config;

    const SALT: vector<u8> = b"diem_framework::staking_contract";
    ...
    #[resource_group_member(group = diem_framework::staking_contract::StakingGroupContainer)]
    ...
    public entry fun create_staking_contract(
        ...
        let staked_coins = coin::withdraw<DiemCoin>(staker, amount);
    ...
    #[test_only]
    public fun setup(diem_framework: &signer, staker: &signer, operator: &signer, initial_balance: u64) {
        // Reward rate of 0.1% per epoch.
        stake::initialize_for_test_custom(diem_framework, INITIAL_BALANCE, MAXIMUM_STAKE, 3600, true, 10, 10000, 1000000);
    ...
    public fun setup_staking_contract(
        ...
        setup(diem_framework, staker, operator, amount);
        let operator_address = signer::address_of(operator);
    ...
    #[test(diem_framework = @0x1, staker = @0x123, operator = @0x234)]
    public entry fun test_end_to_end(diem_framework: &signer, staker: &signer, operator: &signer) acquires Store {
        setup_staking_contract(diem_framework, staker, operator, INITIAL_BALANCE, 10);
        let staker_address = signer::address_of(staker);
        let operator_address = signer::address_of(operator);
        ...
        let operator_balance = coin::balance<DiemCoin>(operator_address);
        ...
        distribute(staker_address, operator_address);
        operator_balance = coin::balance<DiemCoin>(operator_address);
        ...
        let staker_balance = coin::balance<DiemCoin>(staker_address);
    ...
    public entry fun test_operator_cannot_request_same_commission_multiple_times(
        diem_framework: &signer, staker: &signer, operator: &signer) acquires Store {
        setup_staking_contract(diem_framework, staker, operator, INITIAL_BALANCE, 10);
        let staker_address = signer::address_of(staker);
        let operator_address = signer::address_of(operator);
        let pool_address = stake_pool_address(staker_address, operator_address);
    ...
    public entry fun test_staker_cannot_create_staking_contract_with_invalid_commission(
        ...
        setup_staking_contract(diem_framework, staker, operator, INITIAL_BALANCE, 101);
    }
    ...
    public entry fun test_staker_cannot_create_staking_contract_with_less_than_min_stake_required(
        ...
        setup_staking_contract(diem_framework, staker, operator, 50, 100);
    }
    ...
    #[test(diem_framework = @0x1, staker = @0x123, operator_1 = @0x234, operator_2 = @0x345)]
    public entry fun test_staker_can_switch_operator(
        ...
        setup_staking_contract(diem_framework, staker, operator_1, INITIAL_BALANCE, 10);
        ...
        let operator_1_balance = coin::balance<DiemCoin>(operator_1_address);
        ...
        let operator_2_balance = coin::balance<DiemCoin>(operator_2_address);
        new_balance = with_rewards(new_balance);
    ...
    #[test(diem_framework = @0x1, staker = @0x123, operator = @0x234)]
    public entry fun test_update_commission(diem_framework: &signer, staker: &signer, operator: &signer) acquires Store, StakingGroupUpdateCommissionEvent {
        let initial_balance = INITIAL_BALANCE * 2;
        setup_staking_contract(diem_framework, staker, operator, initial_balance, 10);
        let staker_address = signer::address_of(staker);
        let operator_address = signer::address_of(operator);
