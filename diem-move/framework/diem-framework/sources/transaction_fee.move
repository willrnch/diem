...
module diem_framework::transaction_fee {
    ...
    ) acquires DiemCoinCapabilities, CollectedFeesPerBlock {
        system_addresses::assert_diem_framework(diem_framework);
    ...
    fun burn_coin_fraction(coin: &mut Coin<DiemCoin>, burn_percentage: u8) acquires DiemCoinCapabilities {
        assert!(burn_percentage <= 100, error::out_of_range(EINVALID_BURN_PERCENTAGE));
        ...
        if (amount_to_burn > 0) {
            ...
            coin::burn(
                coin_to_burn,
                &borrow_global<DiemCoinCapabilities>(@diem_framework).burn_cap,
            );
        }
    }
    ...
    public(friend) fun process_collected_fees() acquires DiemCoinCapabilities, CollectedFeesPerBlock {
        if (!is_fees_collection_enabled()) {
            return
        };
        let collected_fees = borrow_global_mut<CollectedFeesPerBlock>(@diem_framework);
    ...
    public(friend) fun burn_fee(account: address, fee: u64) acquires DiemCoinCapabilities {
        coin::burn_from<DiemCoin>(
            account,
            fee,
            &borrow_global<DiemCoinCapabilities>(@diem_framework).burn_cap,
        );
    ...
    public(friend) fun collect_fee(account: address, fee: u64) acquires CollectedFeesPerBlock {
        ...
        coin::collect_into_aggregatable_coin<DiemCoin>(account, fee, collected_amount);
    }

    /// Only called during genesis.
    public(friend) fun store_diem_coin_burn_cap(diem_framework: &signer, burn_cap: BurnCapability<DiemCoin>) {
        system_addresses::assert_diem_framework(diem_framework);
        move_to(diem_framework, DiemCoinCapabilities { burn_cap })
    }
    ...
    #[test(diem_framework = @diem_framework)]
    fun test_burn_fraction_calculation(diem_framework: signer) acquires DiemCoinCapabilities {
        ...
        let c1 = coin::mint<DiemCoin>(100, &mint_cap);
        assert!(*option::borrow(&coin::supply<DiemCoin>()) == 100, 0);

        // Burning 25%.
        burn_coin_fraction(&mut c1, 25);
        assert!(coin::value(&c1) == 75, 0);
        assert!(*option::borrow(&coin::supply<DiemCoin>()) == 75, 0);
        ...
        assert!(*option::borrow(&coin::supply<DiemCoin>()) == 0, 0);

        coin::destroy_zero(c1);
        let c2 = coin::mint<DiemCoin>(10, &mint_cap);
        assert!(*option::borrow(&coin::supply<DiemCoin>()) == 10, 0);
    ...
    #[test(diem_framework = @diem_framework, alice = @0xa11ce, bob = @0xb0b, carol = @0xca101)]
    fun test_fees_distribution(
        ...
        let (burn_cap, mint_cap) = diem_coin::initialize_for_test(&diem_framework);
        store_diem_coin_burn_cap(&diem_framework, burn_cap);
        initialize_fee_collection_and_distribution(&diem_framework, 10);

        // Create dummy accounts.
        let alice_addr = signer::address_of(&alice);
        let bob_addr = signer::address_of(&bob);
        let carol_addr = signer::address_of(&carol);
        diem_account::create_account(alice_addr);
        diem_account::create_account(bob_addr);
        diem_account::create_account(carol_addr);
        ...
        assert!(*option::borrow(&coin::supply<DiemCoin>()) == 30000, 0);
        ...
        assert!(coin::balance<DiemCoin>(alice_addr) == 10000, 0);
        assert!(coin::balance<DiemCoin>(bob_addr) == 9000, 0);
        assert!(coin::balance<DiemCoin>(carol_addr) == 10000, 0);
        ...
        assert!(*option::borrow(&coin::supply<DiemCoin>()) == 29900, 0);
        ...
        assert!(coin::balance<DiemCoin>(alice_addr) == 10000, 0);
        assert!(coin::balance<DiemCoin>(bob_addr) == 0, 0);
        assert!(coin::balance<DiemCoin>(carol_addr) == 10000, 0);
        ...
        assert!(*option::borrow(&coin::supply<DiemCoin>()) == 29000, 0);
        ...
        assert!(coin::balance<DiemCoin>(alice_addr) == 8000, 0);
        assert!(coin::balance<DiemCoin>(bob_addr) == 0, 0);
        ...
        assert!(coin::is_aggregatable_coin_zero(&collected_fees.amount), 0);
        assert!(*option::borrow(&collected_fees.proposer) == alice_addr, 0);
        assert!(*option::borrow(&coin::supply<DiemCoin>()) == 28800, 0);
