spec diem_framework::diem_governance {
    ...
    spec initialize_partial_voting(
        ...
        aborts_if exists<VotingRecordsV2>(@diem_framework);
    }
    ...
    spec schema CreateProposalAbortsIf {
        ...
        let addr = diem_std::type_info::type_of<DiemCoin>().account_address;
        aborts_if !exists<coin::CoinInfo<DiemCoin>>(addr);
        let maybe_supply = global<coin::CoinInfo<DiemCoin>>(addr).supply;
        let supply = option::spec_borrow(maybe_supply);
        let total_supply = diem_framework::optional_aggregator::optional_aggregator_value(supply);
        ...
        let post post_voting_forum = global<voting::VotingForum<GovernanceProposal>>(@diem_framework);
    ...
    spec schema VoteAbortIf {
        ...
        aborts_if spec_proposal_expiration <= locked_until && !exists<timestamp::CurrentTimeMicroseconds>(@diem_framework);
        let spec_proposal_expiration = voting::spec_get_proposal_expiration_secs<GovernanceProposal>(@diem_framework, proposal_id);
        ...
        aborts_if !remain_zero_1_cond && !exists<VotingRecords>(@diem_framework);
        include !remain_zero_1_cond && !entirely_voted ==> GetVotingPowerAbortsIf {
            pool_address: stake_pool
        };

        // TODO: Enable the following when smart_table is supported.
        // let staking_config = global<staking_config::StakingConfig>(@diem_framework);
        // let spec_voting_power = spec_get_voting_power(stake_pool, staking_config);
        // let voting_records_v2 = borrow_global<VotingRecordsV2>(@diem_framework);
        ...
        //
        // aborts_if !exists<VotingRecords>(@diem_framework);
        // let voting_records = global<VotingRecords>(@diem_framework);
        //
        //
        // // verify get_voting_power(stake_pool)
        ...
        // let post post_approved_hashes = global<ApprovedExecutionHashes>(@diem_framework);
        //
        // // Due to the complexity of the success state, the validation of 'borrow_global_mut<ApprovedExecutionHashes>(@diem_framework);' is discussed in four cases.
        // aborts_if
        //     if (should_pass) {
        //         proposal_state_successed_0 && !exists<ApprovedExecutionHashes>(@diem_framework)
        //     } else {
        //         proposal_state_successed_1 && !exists<ApprovedExecutionHashes>(@diem_framework)
        //     };
        // aborts_if
        //     if (should_pass) {
        //         proposal_state_successed_2 && !exists<ApprovedExecutionHashes>(@diem_framework)
        //     } else {
        //         proposal_state_successed_3 && !exists<ApprovedExecutionHashes>(@diem_framework)
        //     };
        ...
        //
        // aborts_if features::spec_partial_governance_voting_enabled() && !exists<VotingRecordsV2>(@diem_framework);
    }
    ...
    spec resolve(proposal_id: u64, signer_address: address): signer {
        ...
        let is_multi_step_proposal = diem_std::from_bcs::deserialize<bool>(simple_map::spec_get(proposal.metadata, multi_step_key));
        aborts_if has_multi_step_key && !diem_std::from_bcs::deserializable<bool>(simple_map::spec_get(proposal.metadata, multi_step_key));
        aborts_if !string::spec_internal_check_utf8(voting::IS_MULTI_STEP_PROPOSAL_KEY);
        ...
        aborts_if !exists<ApprovedExecutionHashes>(@diem_framework);
        let post post_approved_hashes = global<ApprovedExecutionHashes>(@diem_framework).hashes;
        ensures !simple_map::spec_contains_key(post_approved_hashes, proposal_id);

        // verify get_signer
        include GetSignerAbortsIf;
        let governance_responsibility = global<GovernanceResponsbility>(@diem_framework);
    ...
    spec reconfigure(diem_framework: &signer) {
        ...
        requires exists<staking_config::StakingRewardsConfig>(@diem_framework);
    ...
    spec resolve_multi_step_proposal(proposal_id: u64, signer_address: address, next_execution_hash: vector<u8>): signer {
        ...
        aborts_if simple_map::spec_contains_key(proposal.metadata, multi_step_key) &&
                            diem_std::from_bcs::deserializable<bool>(simple_map::spec_get(proposal.metadata, multi_step_key));
        let is_multi_step = simple_map::spec_contains_key(proposal.metadata, multi_step_key) &&
                            diem_std::from_bcs::deserialize<bool>(simple_map::spec_get(proposal.metadata, multi_step_key));
        ...
        // verify remove_approved_hash
        aborts_if next_execution_hash_is_empty && !exists<ApprovedExecutionHashes>(@diem_framework);
        let post post_approved_hashes = global<ApprovedExecutionHashes>(@diem_framework).hashes;
    ...
    spec schema VotingIsProposalResolvableAbortsif {
        ...
        let resolvable_time = diem_std::from_bcs::deserialize<u64>(simple_map::spec_get(proposal.metadata, utf8(voting::RESOLVABLE_TIME_METADATA_KEY)));
        aborts_if !diem_std::from_bcs::deserializable<u64>(simple_map::spec_get(proposal.metadata, utf8(voting::RESOLVABLE_TIME_METADATA_KEY)));
