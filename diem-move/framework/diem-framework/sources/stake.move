...
module diem_framework::stake {
    ...
    use diem_framework::staking_config::{Self, StakingConfig, StakingRewardsConfig};
    ...
    /// power.
    public(friend) fun on_new_epoch() acquires StakePool, DiemCoinCapabilities, ValidatorConfig, ValidatorPerformance, ValidatorSet, ValidatorFees {
        let validator_set = borrow_global_mut<ValidatorSet>(@diem_framework);
        let config = staking_config::get();
        let validator_perf = borrow_global_mut<ValidatorPerformance>(@diem_framework);
    ...
    ) acquires StakePool, DiemCoinCapabilities, ValidatorConfig, ValidatorFees {
        let stake_pool = borrow_global_mut<StakePool>(pool_address);
        let validator_config = borrow_global<ValidatorConfig>(pool_address);
    ...
    use diem_std::bls12381::proof_of_possession_from_bytes;
    #[test_only]
    ...
    public fun initialize_for_test(diem_framework: &signer) {
        initialize_for_test_custom(diem_framework, 100, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 100, 1000000);
    }
    ...
    ) acquires DiemCoinCapabilities, StakePool, ValidatorConfig, ValidatorPerformance, ValidatorSet, ValidatorFees {
    ...
    public fun initialize_for_test_custom(
        ...
        if (!exists<ValidatorSet>(@diem_framework)) {
        ...
        if (!exists<DiemCoinCapabilities>(@diem_framework)) {
            let (burn_cap, mint_cap) = diem_coin::initialize_for_test(diem_framework);
            store_diem_coin_mint_cap(diem_framework, mint_cap);
            coin::destroy_burn_cap<DiemCoin>(burn_cap);
        };
    }

    // This function assumes the stake module already the capability to mint diem coins.
    #[test_only]
    public fun mint_coins(amount: u64): Coin<DiemCoin> acquires DiemCoinCapabilities {
        let mint_cap = &borrow_global<DiemCoinCapabilities>(@diem_framework).mint_cap;
        coin::mint(amount, mint_cap)
    }

    #[test_only]
    public fun mint(account: &signer, amount: u64) acquires DiemCoinCapabilities {
    ...
    public fun mint_and_add_stake(
        account: &signer, amount: u64) acquires DiemCoinCapabilities, OwnerCapability, StakePool, ValidatorSet {
        mint(account, amount);
    ...
    public fun initialize_test_validator(
        ...
        should_end_epoch: bool,
    ) acquires AllowedValidators, DiemCoinCapabilities, OwnerCapability, StakePool, ValidatorConfig, ValidatorPerformance, ValidatorSet, ValidatorFees {
    ...
    #[test(diem_framework = @0x1, validator_1 = @0x123, validator_2 = @0x234)]
    #[expected_failure(abort_code = 0x10007, location = Self)]
    public entry fun test_pending_active_validator_cannot_add_stake_if_exceeding_max_allowed(
        ...
        initialize_for_test_custom(diem_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 100000);
    ...
    #[test(diem_framework = @diem_framework, validator_1 = @0x123, validator_2 = @0x234)]
    #[expected_failure(abort_code = 0x10007, location = Self)]
    ...
    public entry fun test_end_to_end(
        diem_framework: &signer,
        validator: &signer,
    ) acquires AllowedValidators, OwnerCapability, StakePool, DiemCoinCapabilities, ValidatorConfig, ValidatorPerformance, ValidatorSet, ValidatorFees {
        initialize_for_test(diem_framework);
        let (_sk, pk, pop) = generate_identity();
        initialize_test_validator(&pk, &pop, validator, 100, true, true);
        ...
        assert!(coin::balance<DiemCoin>(validator_address) == 800, 2);
        ...
        assert!(coin::balance<DiemCoin>(validator_address) == 850, 6);
        assert_validator_state(validator_address, 102, 51, 0, 0, 0);
        withdraw(validator, 51);
        assert!(coin::balance<DiemCoin>(validator_address) == 901, 7);
    ...
    public entry fun test_cannot_reduce_lockup(
        ...
        staking_config::update_recurring_lockup_duration_secs(diem_framework, 1);
    ...
    public entry fun test_inactive_validator_cannot_join_if_exceed_increase_limit(
        ...
        initialize_for_test_custom(diem_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 50);
        let (_sk_1, pk_1, pop_1) = generate_identity();
        let (_sk_2, pk_2, pop_2) = generate_identity();
    ...
    public entry fun test_pending_active_validator_can_add_more_stake(
        ...
        initialize_for_test_custom(diem_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 10000);
    ...
    public entry fun test_pending_active_validator_cannot_add_more_stake_than_limit(
        ...
        initialize_for_test_custom(diem_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 100);
        // Need 1 validator to be in the active validator set so joining limit works.
        let (_sk_1, pk_1, pop_1) = generate_identity();
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100, true, true);
    ...
    public entry fun test_pending_active_validator_leaves_validator_set(
        ...
        assert!(borrow_global<ValidatorSet>(@diem_framework).total_joining_power == 100, 0);
        ...
        assert!(borrow_global<ValidatorSet>(@diem_framework).total_joining_power == 0, 1);
    }
    ...
    public entry fun test_active_validator_withdraw_should_cap_by_inactive_stake(
        ...
        assert!(coin::balance<DiemCoin>(validator_address) == 1001, 2);
    ...
    #[test(diem_framework = @diem_framework, validator = @0x123, validator_2 = @0x234)]
    ...
    #[test(diem_framework = @0x1, validator_1 = @0x123, validator_2 = @0x234, validator_3 = @0x345)]
    public entry fun test_multiple_validators_join_and_leave(
        ...
        initialize_for_test_custom(diem_framework, 100, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 100, 100);
        let (_sk_1, pk_1, pop_1) = generate_identity();
        ...
        assert!(vector::borrow(&borrow_global<ValidatorSet>(@diem_framework).pending_inactive, 0).addr == validator_2_address, 0);
        ...
        assert!(vector::borrow(&borrow_global<ValidatorSet>(@diem_framework).pending_active, 0).addr == validator_3_address, 0);
        assert!(vector::borrow(&borrow_global<ValidatorSet>(@diem_framework).active_validators, 0).config.consensus_pubkey == pk_1_bytes, 0);
        ...
        assert!(vector::borrow(&borrow_global<ValidatorSet>(@diem_framework).active_validators, 0).config.consensus_pubkey == pk_1b_bytes, 0);
    ...
    public entry fun test_validator_cannot_join_post_genesis(
        ...
        initialize_for_test_custom(diem_framework, 100, 10000, LOCKUP_CYCLE_SECONDS, false, 1, 100, 100);
    ...
    #[test(
        diem_framework = @diem_framework,
        validator_1 = @diem_framework,
        validator_2 = @0x2,
        validator_3 = @0x3,
        validator_4 = @0x4,
        validator_5 = @0x5
    ...
    public entry fun test_invalid_config(
        diem_framework: &signer,
        validator: &signer,
    ) acquires AllowedValidators, DiemCoinCapabilities, OwnerCapability, StakePool, ValidatorConfig, ValidatorSet {
        initialize_for_test_custom(diem_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 100, 100);
    ...
    public entry fun test_removing_validator_from_active_set(
        ...
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100, true, false);
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100, true, true);
        assert!(vector::length(&borrow_global<ValidatorSet>(@diem_framework).active_validators) == 2, 0);
        ...
        remove_validators(diem_framework, &vector[validator_to_remove]);
        assert!(vector::length(&borrow_global<ValidatorSet>(@diem_framework).active_validators) == 1, 0);
        assert!(get_validator_state(validator_to_remove) == VALIDATOR_STATUS_PENDING_INACTIVE, 1);
    }

    #[test_only]
    public fun end_epoch() acquires StakePool, DiemCoinCapabilities, ValidatorConfig, ValidatorPerformance, ValidatorSet, ValidatorFees {
    ...
    #[test(diem_framework = @0x1, validator = @0x123)]
    public entry fun test_allowed_validators(
        diem_framework: &signer,
        validator: &signer,
    ) acquires AllowedValidators, OwnerCapability, StakePool, ValidatorSet {
        let addr = signer::address_of(validator);
        configure_allowed_validators(diem_framework, vector[addr]);
    ...
    public entry fun test_not_allowed_validators(
        ...
        configure_allowed_validators(diem_framework, vector[]);
    ...
    public fun get_validator_fee(validator_addr: address): u64 acquires ValidatorFees {
        let fees_table = &borrow_global<ValidatorFees>(@diem_framework).fees_table;
    ...
    fun test_distribute_validator_fees(
        ...
        features::change_feature_flags(diem_framework, vector[COLLECT_AND_DISTRIBUTE_GAS_FEES], vector[]);
        assert!(features::collect_and_distribute_gas_fees(), 0);

        // Initialize staking and validator fees table.
        initialize_for_test(diem_framework);
        initialize_validator_fees(diem_framework);
