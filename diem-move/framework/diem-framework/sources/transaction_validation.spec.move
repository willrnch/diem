spec diem_framework::transaction_validation {
    ...
    spec schema PrologueCommonAbortsIf {
        ...
        aborts_if !(txn_sequence_number == global<Account>(transaction_sender).sequence_number);
        aborts_if !exists<CoinStore<DiemCoin>>(gas_payer);
        aborts_if !(global<CoinStore<DiemCoin>>(gas_payer).coin.value >= max_transaction_fee);
    }
    ...
    spec epilogue(
        ...
        use diem_framework::transaction_fee::{DiemCoinCapabilities, CollectedFeesPerBlock};
        ...
        let apt_addr = type_info::type_of<DiemCoin>().account_address;
        let maybe_apt_supply = global<CoinInfo<DiemCoin>>(apt_addr).supply;
        ...
        aborts_if if (features::spec_is_enabled(features::COLLECT_AND_DISTRIBUTE_GAS_FEES)) {
            !exists<CollectedFeesPerBlock>(@diem_framework)
                || transaction_fee_amount > 0 &&
                    ( // `exists<CoinStore<DiemCoin>>(addr)` checked above.
            ...
            // Sufficiency of funds is checked above.
            !exists<DiemCoinCapabilities>(@diem_framework) ||
            // Existence of APT's CoinInfo
            transaction_fee_amount > 0 && !exists<CoinInfo<DiemCoin>>(diem_addr) ||
    ...
    spec epilogue_gas_payer(
        ...
        aborts_if !exists<CoinStore<DiemCoin>>(gas_payer);
        // Sufficiency of funds
        aborts_if !(global<CoinStore<DiemCoin>>(gas_payer).coin.value >= transaction_fee_amount);

        aborts_if !exists<Account>(addr);
        aborts_if !(global<Account>(addr).sequence_number < MAX_U64);

        let pre_balance = global<coin::CoinStore<DiemCoin>>(gas_payer).coin.value;
        let post balance = global<coin::CoinStore<DiemCoin>>(gas_payer).coin.value;
        let pre_account = global<account::Account>(addr);
        let post account = global<account::Account>(addr);
        ensures balance == pre_balance - transaction_fee_amount;
        ensures account.sequence_number == pre_account.sequence_number + 1;
