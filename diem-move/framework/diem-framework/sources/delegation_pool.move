 ...
 */
module diem_framework::delegation_pool {
    use std::error;
    use std::features;
    use std::signer;
    use std::vector;

    use diem_std::pool_u64_unbound::{Self as pool_u64, total_coins};
    use diem_std::table::{Self, Table};
    use diem_std::smart_table::{Self, SmartTable};
    ...
    const MODULE_SALT: vector<u8> = b"diem_framework::delegation_pool";
    ...
    public fun calculate_and_update_remaining_voting_power(pool_address: address, voter_address: address, proposal_id: u64): u64 acquires DelegationPool, GovernanceRecords {
        ...
        if (diem_governance::get_remaining_voting_power(pool_address, proposal_id) == 0) {
            return 0
        };
    ...
    public entry fun initialize_delegation_pool(
        ...
        coin::register<DiemCoin>(&stake_pool_signer);
    ...
    public entry fun vote(voter: &signer, pool_address: address, proposal_id: u64, voting_power: u64, should_pass: bool) acquires DelegationPool, GovernanceRecords {
        ...
        *used_voting_power = *used_voting_power + voting_power;
        ...
        diem_governance::partial_vote(&pool_signer, pool_address, proposal_id, voting_power, should_pass);
    ...
    public entry fun create_proposal(
        ...
        assert!(
            total_voting_power >= diem_governance::get_required_proposer_stake(),
            error::invalid_argument(EINSUFFICIENT_PROPOSER_STAKE));
        let pool_signer = retrieve_stake_pool_owner(borrow_global<DelegationPool>(pool_address));
        let proposal_id = diem_governance::create_proposal_v2_impl(
            &pool_signer,
            pool_address,
            execution_hash,
            metadata_location,
    ...
    public entry fun add_stake(delegator: &signer, pool_address: address, amount: u64) acquires DelegationPool, GovernanceRecords {
        ...
        coin::transfer<DiemCoin>(delegator, pool_address, amount);
    ...
    fun withdraw_internal(pool: &mut DelegationPool, delegator_address: address, amount: u64) acquires GovernanceRecords {
        ...
        coin::transfer<DiemCoin>(stake_pool_owner, delegator_address, amount);
    ...
    inline fun assert_and_update_proposal_used_voting_power(
        ...
        let stake_pool_remaining_voting_power = diem_governance::get_remaining_voting_power(pool_address, proposal_id);
        let stake_pool_used_voting_power = diem_governance::get_voting_power(pool_address) - stake_pool_remaining_voting_power;
    ...
    #[test_only]
    use diem_framework::timestamp::fast_forward_seconds;
    ...
    #[test_only]
    public fun end_diem_epoch() {
        stake::end_epoch(); // additionally forwards EPOCH_DURATION seconds
        reconfiguration::reconfigure_for_test_custom();
    }

    #[test_only]
    public fun initialize_for_test(diem_framework: &signer) {
    ...
    #[test_only]
    public fun initialize_for_test_no_reward(diem_framework: &signer) {
        initialize_for_test_custom(
            diem_framework,
            100 * ONE_APT,
            10000 * ONE_APT,
    ...
    public fun initialize_for_test_custom(
        ...
        reconfiguration::initialize_for_test(diem_framework);
        features::change_feature_flags(diem_framework, vector[DELEGATION_POOLS], vector[]);
    }
    ...
    #[test(diem_framework = @diem_framework, validator = @0x123)]
    #[expected_failure(abort_code = 0x3000A, location = Self)]
    public entry fun test_delegation_pools_disabled(
        diem_framework: &signer,
        validator: &signer,
    ) acquires DelegationPool, GovernanceRecords {
        initialize_for_test(diem_framework);
        features::change_feature_flags(diem_framework, vector[], vector[DELEGATION_POOLS]);
    ...
    #[test(diem_framework = @diem_framework, validator = @0x123, delegator1 = @0x010, delegator2 = @0x020)]
    public entry fun test_add_stake_fee(
        ...
        features::change_feature_flags(diem_framework, vector[features::get_periodical_reward_rate_decrease_feature()], vector[]);
    ...
    #[test(diem_framework = @diem_framework, validator = @0x123, delegator = @0x010)]
    ...
    public entry fun test_add_stake_single(
        ...
        let balance = coin::balance<DiemCoin>(validator_address);
        add_stake(validator, pool_address, 250 * ONE_APT);

        // check added stake have been transferred out of delegator account
        assert!(coin::balance<DiemCoin>(validator_address) == balance - 250 * ONE_APT, 0);
    ...
    public entry fun test_unlock_single(
        ...
        end_diem_epoch(); // additionally forwards EPOCH_DURATION seconds
        ...
        end_diem_epoch(); // additionally forwards EPOCH_DURATION seconds and expires lockup cycle
        ...
        assert!(coin::balance<DiemCoin>(delegator_address) == 0, 0);

        // withdraw own unlocked stake
        withdraw(validator, pool_address, 15301499997);
        assert!(coin::balance<DiemCoin>(validator_address) == 15301499997, 0);
        ...
        assert!(coin::balance<DiemCoin>(validator_address) == balance + 5457545100, 0);
        ...
        balance = coin::balance<DiemCoin>(validator_address);
        withdraw(validator, pool_address, 10303010000);

        assert!(coin::balance<DiemCoin>(validator_address) == balance + 10303010000, 0);
        ...
        assert_pending_withdrawal(validator_address, pool_address, false, 0, false, 0);
        assert!(coin::balance<DiemCoin>(validator_address) == balance + 1000000000, 0);
    }
    ...
    public entry fun test_reactivate_stake_single(
        ...
        unlock(validator, pool_address, 100 * ONE_APT);
        assert!(coin::balance<DiemCoin>(validator_address) == 15149999998, 0);
    ...
    public entry fun test_withdraw_many(
        ...
        let balance = coin::balance<DiemCoin>(validator_address);
        withdraw(validator, pool_address, MAX_U64);
        assert!(coin::balance<DiemCoin>(validator_address) == balance + 10100000000, 0);
        assert_pending_withdrawal(validator_address, pool_address, false, 0, false, 0);
        assert_inactive_shares_pool(pool_address, 0, false, 0);
    ...
    public entry fun test_pending_inactive_stake_rewards(
        ...
        end_diem_epoch(); // 81000000001 active stake * 1.01 + 19999999999 pending_inactive stake * 1.01
        end_diem_epoch(); // 81810000001 active stake * 1.01 + 20199999998 pending_inactive stake * 1.01

        timestamp::fast_forward_seconds(LOCKUP_CYCLE_SECONDS);
        end_diem_epoch(); // 82628100001 active stake * 1.01 + 20401999997 pending_inactive stake * 1.01
        end_diem_epoch(); // 83454381001 active stake * 1.01 + 20606019996 pending_inactive stake(now inactive)
        ...
        unlock(validator, pool_address, 200 * ONE_APT);
        assert!(coin::balance<DiemCoin>(validator_address) == 20606019996, 0);
        ...
        end_diem_epoch(); // 64288924812 active stake * 1.01 + 19999999999 pending_inactive stake * 1.01
        end_diem_epoch(); // 64931814060 active stake * 1.01 + 20199999998 pending_inactive stake * 1.01
        end_diem_epoch(); // 65581132200 active stake * 1.01 + 20401999997 pending_inactive stake * 1.01
        end_diem_epoch(); // 66236943522 active stake * 1.01 + 20606019996 pending_inactive stake * 1.01
        assert_delegation(validator_address, pool_address, 66899312957, 0, 20812080195);

        timestamp::fast_forward_seconds(LOCKUP_CYCLE_SECONDS);
        end_diem_epoch(); // 66899312957 active stake * 1.01 + 20812080195 pending_inactive stake * 1.01
        end_diem_epoch(); // 67568306086 active stake * 1.01 + 21020200996 pending_inactive stake(now inactive)
        end_diem_epoch(); // 68243989147 active stake * 1.01 + 21020200996 inactive stake
    ...
    public entry fun test_out_of_order_redeem(
        ...
        assert!(coin::balance<DiemCoin>(delegator1_address) == 15149999998, 0);
        assert!(coin::balance<DiemCoin>(delegator2_address) == 5149999997, 0);
        ...
        assert!(coin::balance<DiemCoin>(delegator2_address) == 15149999998, 0);
        ...
        assert!(coin::balance<DiemCoin>(delegator1_address) == 15149999998 + 10099999998, 0);
        assert_pending_withdrawal(delegator1_address, pool_address, false, 0, false, 0);
    }
    ...
    public entry fun test_operator_fee(
        ...
        coin::register<DiemCoin>(validator);
        synchronize_delegation_pool(pool_address);
        assert!(coin::balance<DiemCoin>(validator_address) == 25536996, 0);
    ...
    #[test(diem_framework = @diem_framework, old_operator = @0x123, delegator = @0x010, new_operator = @0x020)]
    ...
    #[test(diem_framework = @diem_framework, validator = @0x123, delegator1 = @0x010)]
    ...
    public entry fun test_create_proposal_abort_if_inefficient_stake(
        ...
        initialize_for_test(diem_framework);
        diem_governance::initialize_for_test(
            diem_framework,
            (10 * ONE_APT as u128),
            100 * ONE_APT,
            1000,
        );
        diem_governance::initialize_partial_voting(diem_framework);
    ...
    #[test(diem_framework = @diem_framework, validator = @0x123, delegator1 = @0x010, delegator2 = @0x020, voter1 = @0x030, voter2 = @0x040)]
    public entry fun test_voting_power_change(
        ...
        initialize_for_test_no_reward(diem_framework);
        diem_governance::initialize_for_test(
            diem_framework,
            (10 * ONE_APT as u128),
            100 * ONE_APT,
    ...
    #[test(diem_framework = @diem_framework, validator = @0x123, delegator1 = @0x010, voter1 = @0x030)]
    ...
    public entry fun test_voting_power_change_already_voted_before_partial(
        ...
        let proposal1_id = setup_vote(diem_framework, validator, false);
        ...
        let execution_hash = vector::empty<u8>();
        vector::push_back(&mut execution_hash, 1);
        let proposal2_id = diem_governance::create_proposal_v2_impl(
            validator,
            pool_address,
            execution_hash,
            b"",
        ...
        diem_governance::vote(validator, pool_address, proposal1_id, true);
    ...
    public entry fun test_vote_should_failed_if_no_stake(
        ...
        // partial voing hasn't been enabled yet. A proposal has been created by the validator.
        let proposal1_id = setup_vote(diem_framework, validator, true);
    ...
    public entry fun test_delegate_voting_power_should_pass_even_if_no_stake(
        ...
        // partial voing hasn't been enabled yet. A proposal has been created by the validator.
        setup_vote(diem_framework, validator, true);
    ...
    ): u64 acquires DelegationPoolOwnership, DelegationPool, GovernanceRecords {
        initialize_for_test_no_reward(diem_framework);
        diem_governance::initialize_for_test(
            diem_framework,
            (10 * ONE_APT as u128),
