...
pub enum EntryFunctionCall {
    ...
    /// Initialize new coin `CoinType` in Diem Blockchain.
    ...
    pub fn encode(self) -> TransactionPayload {
        ...
        match self {
            ...
            DiemGovernancePartialVote {
                ...
                voting_power,
                should_pass,
            } => diem_governance_partial_vote(stake_pool, proposal_id, voting_power, should_pass),
            DiemGovernanceVote {
                stake_pool,
                proposal_id,
                should_pass,
            } => diem_governance_vote(stake_pool, proposal_id, should_pass),
...
/// Vote on proposal with `proposal_id` and specified voting power from `stake_pool`.
pub fn diem_governance_partial_vote(
    stake_pool: AccountAddress,
    proposal_id: u64,
    voting_power: u64,
    should_pass: bool,
...
mod decoder {
    ...
    pub fn diem_governance_partial_vote(payload: &TransactionPayload) -> Option<EntryFunctionCall> {
        if let TransactionPayload::EntryFunction(script) = payload {
            Some(EntryFunctionCall::DiemGovernancePartialVote {
                stake_pool: bcs::from_bytes(script.args().get(0)?).ok()?,
                proposal_id: bcs::from_bytes(script.args().get(1)?).ok()?,
                voting_power: bcs::from_bytes(script.args().get(2)?).ok()?,
                should_pass: bcs::from_bytes(script.args().get(3)?).ok()?,
...
static SCRIPT_FUNCTION_DECODER_MAP: once_cell::sync::Lazy<EntryFunctionDecoderMap> =
    once_cell::sync::Lazy::new(|| {
        ...
        map.insert(
            "diem_governance_partial_vote".to_string(),
            Box::new(decoder::diem_governance_partial_vote),
        );
        map.insert(
