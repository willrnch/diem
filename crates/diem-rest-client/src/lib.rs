...
pub use crate::client_builder::{DiemBaseUrl, ClientBuilder};
use crate::{
    diem::{DiemVersion, Balance},
    error::RestError,
};
...
use diem_api_types::{
    ...
    DiemError, BcsBlock, Block, GasEstimation, HexEncodedBytes, IndexResponse, MoveModuleId,
    TransactionData, TransactionOnChainData, TransactionsBatchSubmissionResult, UserTransaction,
    VersionedEvent, ViewRequest,
};
use diem_crypto::HashValue;
use diem_logger::{debug, info, sample, sample::SampleRate};
use diem_types::{
...
const X_DIEM_SDK_HEADER_VALUE: &str = concat!("diem-rust-sdk/", env!("CARGO_PKG_VERSION"));

type DiemResult<T> = Result<T, RestError>;
...
impl Client {
    pub fn builder(diem_base_url: DiemBaseUrl) -> ClientBuilder {
        ClientBuilder::new(diem_base_url)
    }

    pub fn new(base_url: Url) -> Self {
        Self::builder(DiemBaseUrl::Custom(base_url)).build()
    }
    ...
    pub fn version_path_base(mut self, version_path_base: String) -> DiemResult<Self> {
    ...
    pub fn build_path(&self, path: &str) -> DiemResult<Url> {
        Ok(self.base_url.join(&self.version_path_base)?.join(path)?)
    }

    pub async fn get_diem_version(&self) -> DiemResult<Response<DiemVersion>> {
        self.get_resource::<DiemVersion>(CORE_CODE_ADDRESS, "0x1::version::Version")
            .await
    }
    ...
    ) -> DiemResult<Response<Balance>> {
        let resp = self
            .get_account_resource(address, "0x1::coin::CoinStore<0x1::diem_coin::DiemCoin>")
            .await?;
    ...
    pub async fn get_index(&self) -> DiemResult<Response<IndexResponse>> {
        self.get(self.build_path("")?).await
    }

    pub async fn get_index_bcs(&self) -> DiemResult<Response<IndexResponseBcs>> {
        let url = self.build_path("")?;
        let response = self.get_bcs(url).await?;
        Ok(response.and_then(|inner| bcs::from_bytes(&inner))?)
    }

    // TODO: Remove this, just use `get_index`: https://github.com/aptos-labs/diem-core/issues/5597.
    pub async fn get_ledger_information(&self) -> DiemResult<Response<State>> {
    ...
    pub async fn view(
        ...
        request: &ViewRequest,
        version: Option<u64>,
    ) -> DiemResult<Response<Vec<serde_json::Value>>> {
    ...
    ) -> DiemResult<Response<Vec<UserTransaction>>> {
    ...
    pub async fn simulate_bcs(
        &self,
        txn: &SignedTransaction,
    ) -> DiemResult<Response<TransactionOnChainData>> {
        let txn_payload = bcs::to_bytes(txn)?;
        let url = self.build_path("transactions/simulate")?;
    ...
    pub async fn submit(
        &self,
        txn: &SignedTransaction,
    ) -> DiemResult<Response<PendingTransaction>> {
        let txn_payload = bcs::to_bytes(txn)?;
    ...
    pub async fn submit_bcs(&self, txn: &SignedTransaction) -> DiemResult<Response<()>> {
        let txn_payload = bcs::to_bytes(txn)?;
        let url = self.build_path("transactions")?;
    ...
    ) -> DiemResult<Response<TransactionsBatchSubmissionResult>> {
    ...
    pub async fn submit_and_wait(
        &self,
        txn: &SignedTransaction,
    ) -> DiemResult<Response<Transaction>> {
    ...
    async fn wait_for_transaction_by_hash_inner<F, Fut, T>(
        ...
        fetch: F,
    ) -> DiemResult<Response<T>>
    where
        F: Fn(HashValue) -> Fut,
        Fut: Future<Output = DiemResult<WaitForTransactionResult<T>>>,
    {
        ...
        loop {
            ...
            match fetch(hash).await {
                ...
                Ok(WaitForTransactionResult::NotFound(error)) => {
                    if let RestError::Api(diem_error_response) = error {
                        if let Some(state) = diem_error_response.state {
                            if expiration_timestamp_secs <= state.timestamp_usecs / 1_000_000 {
                                if reached_mempool {
                                    return Err(anyhow!("Transaction expired. It is guaranteed it will not be committed on chain.").into());
                                } else {
            ...
            if let Some(max_server_lag_wait_duration) = max_server_lag_wait {
                if diem_infallible::duration_since_epoch().as_secs()
    ...
    pub async fn get_transactions(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> DiemResult<Response<Vec<Transaction>>> {
        let url = self.build_path("transactions")?;
    ...
    pub async fn get_transactions_bcs(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> DiemResult<Response<Vec<TransactionOnChainData>>> {
        let url = self.build_path("transactions")?;
    ...
    pub async fn get_transaction_by_hash_bcs(
        &self,
        hash: HashValue,
    ) -> DiemResult<Response<TransactionData>> {
    ...
    pub async fn get_account_resources(
        &self,
        address: AccountAddress,
    ) -> DiemResult<Response<Vec<Resource>>> {
    ...
    pub async fn get_account_resources_bcs(
        &self,
        address: AccountAddress,
    ) -> DiemResult<Response<BTreeMap<StructTag, Vec<u8>>>> {
    ...
    pub async fn get_account_resource(
        ...
        address: AccountAddress,
        resource_type: &str,
    ) -> DiemResult<Response<Option<Resource>>> {
    ...
    pub async fn get_account_modules(
        &self,
        address: AccountAddress,
    ) -> DiemResult<Response<Vec<MoveModuleBytecode>>> {
        self.paginate_with_cursor(
    ...
    pub async fn get_account_modules_bcs(
        &self,
        address: AccountAddress,
    ) -> DiemResult<Response<BTreeMap<MoveModuleId, Vec<u8>>>> {
        self.paginate_with_cursor_bcs(
            &format!("accounts/{}/modules", address),
            MODULES_PER_CALL_PAGINATION,
            None,
    ...
    pub async fn get_account_module(
        ...
        address: AccountAddress,
        module_name: &str,
    ) -> DiemResult<Response<MoveModuleBytecode>> {
    ...
    pub async fn get_account_module_bcs(
        ...
        address: AccountAddress,
        module_name: &str,
    ) -> DiemResult<Response<bytes::Bytes>> {
        let url = self.build_path(&format!("accounts/{}/module/{}", address, module_name))?;
    ...
    pub async fn get_account_events(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> DiemResult<Response<Vec<VersionedEvent>>> {
        let url = self.build_path(&format!(
            "accounts/{}/events/{}/{}",
    ...
    pub async fn get_account_events_bcs(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> DiemResult<Response<Vec<EventWithVersion>>> {
        let url = self.build_path(&format!(
            "accounts/{}/events/{}/{}",
            address.to_hex_literal(),
            struct_tag,
    ...
    pub async fn get_account(&self, address: AccountAddress) -> DiemResult<Response<Account>> {
    ...
    pub async fn get_account_bcs(
        &self,
        address: AccountAddress,
    ) -> DiemResult<Response<AccountResource>> {
        let url = self.build_path(&format!("accounts/{}", address))?;
        let response = self.get_bcs(url).await?;
        Ok(response.and_then(|inner| bcs::from_bytes(&inner))?)
    }

    pub async fn estimate_gas_price(&self) -> DiemResult<Response<GasEstimation>> {
        let url = self.build_path("estimate_gas_price")?;
        let response = self.inner.get(url).send().await?;
        self.json(response).await
    }

    pub async fn set_failpoint(&self, name: String, actions: String) -> DiemResult<String> {
    ...
    ) -> DiemResult<(reqwest::Response, State)> {
        if !response.status().is_success() {
            Err(parse_error(response).await)
        } else {
    ...
    pub async fn health_check(&self, seconds: u64) -> DiemResult<()> {
    ...
    async fn get<T: DeserializeOwned>(&self, url: Url) -> DiemResult<Response<T>> {
        self.json(self.inner.get(url).send().await?).await
    }

    async fn get_bcs(&self, url: Url) -> DiemResult<Response<bytes::Bytes>> {
        let response = self.inner.get(url).header(ACCEPT, BCS).send().await?;
    ...
    where
        F: Fn() -> Fut,
        RetryFun: Fn(StatusCode, Option<DiemError>) -> bool,
        Fut: Future<Output = DiemResult<T>>,
    {
    ...
    pub async fn paginate_with_cursor_bcs<T: for<'a> Deserialize<'a> + Ord>(
        ...
        limit_per_request: u64,
        ledger_version: Option<u64>,
    ) -> DiemResult<Response<BTreeMap<T, Vec<u8>>>> {
...
pub fn retriable_with_404(status_code: StatusCode, diem_error: Option<DiemError>) -> bool {
    retriable(status_code, diem_error) | matches!(status_code, StatusCode::NOT_FOUND)
}

pub fn retriable(status_code: StatusCode, _diem_error: Option<DiemError>) -> bool {
    matches!(
        status_code,
...
fn parse_state(response: &reqwest::Response) -> DiemResult<State> {
...
async fn parse_error(response: reqwest::Response) -> RestError {
    ...
    match response.json::<DiemError>().await {
