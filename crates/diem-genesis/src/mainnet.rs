...
use diem_vm::DiemVM;
use diem_vm_genesis::{AccountBalance, EmployeePool, ValidatorWithCommissionRate};
...
impl MainnetGenesisInfo {
    ...
    fn generate_genesis_txn(&self) -> Transaction {
        diem_vm_genesis::encode_diem_mainnet_genesis_transaction(
            &self.accounts,
            &self.employee_vesting_accounts,
            &self.validators,
            &self.framework,
            self.chain_id,
            &diem_vm_genesis::GenesisConfiguration {
    ...
    pub fn generate_waypoint(&mut self) -> anyhow::Result<Waypoint> {
        ...
        diem_executor::db_bootstrapper::generate_waypoint::<DiemVM>(&db_rw, genesis)
    }
}
