...
use diem_config::config::{ApiConfig, DEFAULT_MAX_PAGE_SIZE};
use diem_logger::prelude::*;
use diem_node::DiemNodeArgs;
use diem_rosetta::bootstrap;
...
async fn main() {
    ...
    match args {
        CommandArgs::OnlineRemote(_) => {
            println!("diem-rosetta: Starting Rosetta in Online remote (no local full node) mode")
        },
        CommandArgs::Online(_) => {
            println!("diem-rosetta: Starting Rosetta in Online (with local full node) mode")
        },
        CommandArgs::Offline(_) => println!("diem-rosetta: Starting Rosetta in Offline mode"),
    }
    ...
    }) = args
    {
            ...
            match client.get_index_bcs().await {
                ...
                Err(err) => {
                    sample!(
                        ...
                        println!(
                            "diem-rosetta: Full node REST API isn't responding yet.  You should check the node logs.  It's been waiting {} seconds.  Error: {:?}",
        ...
        println!("diem-rosetta: Local full node started successfully");
        Some(runtime)
    } else {
    ...
    println!("diem-rosetta: Starting rosetta");
    ...
    .expect("diem-rosetta: Should bootstrap rosetta server");

    println!("diem-rosetta: Rosetta started");
...
trait ServerArgs {
    ...
    fn rest_client(&self) -> Option<diem_rest_client::Client>;
...
/// Diem Rosetta API Server
///
/// Provides an implementation of [Rosetta](https://www.rosetta-api.org/docs/Reference.html) on Diem.
#[derive(Debug, Parser)]
#[clap(name = "diem-rosetta", author, version, propagate_version = true)]
...
pub struct OnlineRemoteArgs {
    ...
    /// URL for the Diem REST API. e.g. https://fullnode.devnet.diemlabs.com
...
impl ServerArgs for OnlineRemoteArgs {
    ...
    fn rest_client(&self) -> Option<diem_rest_client::Client> {
        Some(diem_rest_client::Client::new(self.rest_api_url.clone()))
    }
