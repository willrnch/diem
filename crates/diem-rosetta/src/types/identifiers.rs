...
/// Identifier for a "block".  In diem, we use a transaction model, so the index
...
pub struct NetworkIdentifier {
    /// Blockchain name, should always be `diem` and be hardcoded
...
impl From<diem_crypto::HashValue> for TransactionIdentifier {
    fn from(hash: diem_crypto::HashValue) -> Self {
        TransactionIdentifier {
