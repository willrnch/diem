...
use diem_rest_client::diem_api_types::{TransactionOnChainData, U64};
use diem_types::{
...
/// Representation of a Block for a blockchain.  For diem it is the version
///
...
pub struct Block {
    ...
    pub timestamp: u64,
    /// Transactions associated with the version.  In diem there should only be one transaction
    pub transactions: Vec<Transaction>,
}

/// A combination of a transaction and the block associated.  In Diem, this is just the same
...
///
/// This is known as a write set change within Diem
...
/// Cryptographic signature type used for signing transactions.  Diem only uses
    ...
    if let TransactionPayload::EntryFunction(inner) = payload {
        match (
            *inner.module().address(),
            ...
            (AccountAddress::ONE, DIEM_ACCOUNT_MODULE, TRANSFER_FUNCTION) => {
    ...
    /*    if let Some(owner_address) = server_context.pool_address_to_owner.get(&pool_address) {
            if let Ok(stakepool) = bcs::from_bytes::<StakePool>(data) {
                ...
                let add_stake_events = filter_events(
                    ...
                    |event_key, event| {
                        if let Ok(event) = bcs::from_bytes::<diem_types::stake_pool::AddStakeEvent>(
                            event.event_data(),
                        ) {
                            Some(event)
                        } else {
                ...
                let set_operator_events = filter_events(
                    ...
                    |event_key, event| {
                        if let Ok(event) = bcs::from_bytes::<diem_types::stake_pool::SetOperatorEvent>(
                            event.event_data(),
                        ) {
                            Some(event)
                        } else {
...
impl InternalOperation {
    ...
    pub fn payload(
        &self,
    ) -> ApiResult<(diem_types::transaction::TransactionPayload, AccountAddress)> {
        Ok(match self {
            InternalOperation::CreateAccount(create_account) => (
                diem_stdlib::diem_account_create_account(create_account.new_account),
                create_account.sender,
            ),
            InternalOperation::Transfer(transfer) => {
                is_native_coin(&transfer.currency)?;
                (
                    diem_stdlib::diem_account_transfer(transfer.receiver, transfer.amount.0),
                    transfer.sender,
                )
            },
            InternalOperation::SetOperator(set_operator) => {
                ...
                (
                    diem_stdlib::staking_contract_switch_operator_with_same_commission(
            ...
            InternalOperation::SetVoter(set_voter) => {
                ...
                (
                    diem_stdlib::staking_contract_update_voter(
            ...
            InternalOperation::InitializeStakePool(init_stake_pool) => (
                diem_stdlib::staking_contract_create_staking_contract(
            ...
            InternalOperation::ResetLockup(reset_lockup) => (
                diem_stdlib::staking_contract_reset_lockup(reset_lockup.operator),
                reset_lockup.owner,
            ),
            InternalOperation::UnlockStake(unlock_stake) => (
                diem_stdlib::staking_contract_unlock_stake(
            ...
            InternalOperation::DistributeStakingRewards(distribute_staking_rewards) => (
                diem_stdlib::staking_contract_distribute(
            ...
            InternalOperation::AddDelegatedStake(add_delegated_stake) => (
                diem_stdlib::delegation_pool_add_stake(
            ...
            InternalOperation::UnlockDelegatedStake(unlock_delegated_stake) => (
                diem_stdlib::delegation_pool_unlock(
            ...
            InternalOperation::WithdrawUndelegated(withdraw_undelegated) => (
                diem_stdlib::delegation_pool_withdraw(
