...
[Rosetta](https://en.wikipedia.org/wiki/Rosetta_(software)) works as a sidecar to an Diem fullnode.  Rosetta then proxies the Rosetta standard
API calls to underlying Diem REST API calls and builds the appropriate data.  


## Running Rosetta

The `diem-rosetta` binary can run in three modes:
1. `online` -> This runs a local fullnode and blocks the Diem REST API from outside access, using it only as a local proxy for Rosetta APIs.
...
All transactions are parsed from the events provided by the DiemFramework.  There are a few exceptions to this that use the transaction payload, but only for errors.
...
file to run the automated checks.  Additionally, the [diem.ros](./diem.ros)
file uses the Rosetta CLI DSL to describe the possible operations that
can be run.

Additionally, we have our `diem-rosetta-cli` crate for local testing.
