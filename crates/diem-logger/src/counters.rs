...
/// Counter for failed log ingest writes (see also: diem-telemetry for sender metrics)
pub static DIEM_LOG_INGEST_WRITER_FULL: Lazy<IntCounter> = Lazy::new(|| {
    register_int_counter!(
        "diem_log_ingest_writer_full",
...
/// Counter for failed log ingest writes (see also: diem-telemetry for sender metrics)
pub static DIEM_LOG_INGEST_WRITER_DISCONNECTED: Lazy<IntCounter> = Lazy::new(|| {
    register_int_counter!(
        "diem_log_ingest_writer_disconnected",
