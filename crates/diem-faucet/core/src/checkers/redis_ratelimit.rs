...
/// we'll worry about it as a followup: https://github.com/aptos-labs/diem-tap/issues/15.
...
impl RedisRatelimitChecker {
    ...
    pub async fn get_redis_connection(&self) -> Result<Connection, DiemTapError> {
        self.db_pool.get().await.map_err(|e| {
            DiemTapError::new_with_error_code(
                format!("Failed to connect to redis storage: {}", e),
                DiemTapErrorCode::StorageError,
            )
        })
    }
...
impl CheckerTrait for RedisRatelimitChecker {
    async fn check(
        ...
        let mut conn = self
            ...
            .await
            .map_err(|e| DiemTapError::new_with_error_code(e, DiemTapErrorCode::StorageError))?;
    ...
    async fn complete(&self, data: CompleteData) -> Result<(), DiemTapError> {
