...
use diem_faucet_metrics_server::{run_metrics_server, MetricsServerConfig};
use diem_logger::info;
use diem_sdk::{
    crypto::ed25519::Ed25519PrivateKey,
    types::{account_config::diem_test_root_address, chain_id::ChainId},
};
use clap::Parser;
    ...
    pub fn build_for_cli(
        ...
        Self {
            ...
            funder_config: FunderConfig::MintFunder(MintFunderConfig {
                ...
                mint_account_address: Some(diem_test_root_address()),
                do_not_delegate,
            }),
            handler_config: HandlerConfig {
...
mod test {
    ...
    use crate::{
        endpoints::{
            DiemTapError, DiemTapErrorCode, FundRequest, FundResponse, RejectionReasonCode,
        },
    ...
    async fn test_checkers() -> Result<()> {
        ...
        let response = reqwest::Client::new()
            ...
            .send()
            .await?;
        let diem_error = DiemTapError::parse_from_json_string(&response.text().await?)
            .expect("Failed to read response as DiemError");
        assert!(!diem_error.rejection_reasons.is_empty());
        ...
        let diem_error = DiemTapError::parse_from_json_string(&response.text().await?)
            .expect("Failed to read response as DiemError");
        let rejection_reason_codes: HashSet<RejectionReasonCode> = diem_error
            .rejection_reasons
            .into_iter()
            .map(|r| r.get_code())
            .collect();
    ...
    async fn test_redis_ratelimiter() -> Result<()> {
        ...
        let diem_node_api_client = diem_sdk::rest_client::Client::new(
            reqwest::Url::from_str("http://127.0.0.1:8080").unwrap(),
        );
        diem_node_api_client
            .get_index_bcs()
            .await
    ...
    async fn test_transfer_health() -> Result<()> {
        ...
        let serialized_keys = diem_sdk::bcs::to_bytes(&private_key)?;
        ...
        unwrap_reqwest_result(
            reqwest::Client::new()
                .post("https://faucet.devnet.diemlabs.com/fund")
                .body(
                    FundRequest {
        ...
        let diem_error = DiemTapError::parse_from_json_string(&response.text().await?)
            .expect("Failed to read response as DiemError");
        assert_eq!(
            diem_error.error_code,
            DiemTapErrorCode::FunderAccountProblem
        );
    ...
    async fn test_mint_funder() -> Result<()> {
        ...
        let response = diem_node_api_client
