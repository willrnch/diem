...
use super::errors::DiemTapErrorResponse;
...
/// be a JSON representation of DiemTapError. For our own errors, this is exactly
...
pub async fn convert_error(error: poem::Error) -> impl poem::IntoResponse {
    ...
    if is_framework_error {
        // Build an DiemTapErrorResponse and then reset its status code
...
fn build_error_response(error_string: String) -> Response {
    DiemTapErrorResponse::from(DiemTapError::new_with_error_code(
        error_string,
        DiemTapErrorCode::WebFrameworkError,
    ))
    .into_response()
}
