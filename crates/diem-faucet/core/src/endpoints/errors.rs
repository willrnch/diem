...
pub struct DiemTapError {
    /// A message describing the error
    pub message: String,
    /// A code describing the error for programmatic use.
    pub error_code: DiemTapErrorCode,
...
impl DiemTapError {
    pub fn new(message: String, error_code: DiemTapErrorCode) -> Self {
        if *USE_HELPFUL_ERRORS.get().unwrap_or(&true) {
            ...
            Self {
                ...
                error_code: DiemTapErrorCode::YeahNahYeahYeahYeahNahYeahNah,
                rejection_reasons: vec![],
                txn_hashes: vec![],
            }
        }
...
impl std::fmt::Display for DiemTapError {
...
impl From<DiemTapError> for DiemTapErrorResponse {
    fn from(error: DiemTapError) -> Self {
...
pub enum DiemTapErrorCode {
    ...
    /// Failed when making requests to the Diem API.
    DiemApiError = 50,
...
impl DiemTapErrorCode {
    pub fn status(&self) -> StatusCode {
        match self {
            DiemTapErrorCode::InvalidRequest
            | DiemTapErrorCode::AccountDoesNotExist
            | DiemTapErrorCode::EndpointNotEnabled => StatusCode::BAD_REQUEST,
            DiemTapErrorCode::Rejected
            | DiemTapErrorCode::SourceIpMissing
            | DiemTapErrorCode::TransactionFailed => StatusCode::FORBIDDEN,
            DiemTapErrorCode::DiemApiError
            | DiemTapErrorCode::TransactionTimedOut
            | DiemTapErrorCode::SerializationError
            | DiemTapErrorCode::BypasserError
            | DiemTapErrorCode::CheckerError
            | DiemTapErrorCode::StorageError => StatusCode::INTERNAL_SERVER_ERROR,
            DiemTapErrorCode::ServerOverloaded | DiemTapErrorCode::FunderAccountProblem => {
                StatusCode::SERVICE_UNAVAILABLE
            },
            DiemTapErrorCode::YeahNahYeahYeahYeahNahYeahNah => StatusCode::IM_A_TEAPOT,
            // We shouldn't get here, this code is only used in error_converter.rs.
            DiemTapErrorCode::WebFrameworkError => StatusCode::INTERNAL_SERVER_ERROR,
        }
    }
}
