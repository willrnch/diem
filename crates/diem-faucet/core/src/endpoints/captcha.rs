...
use super::{errors::DiemTapErrorResponse, ApiTags, DiemTapError, DiemTapErrorCode};
...
impl CaptchaApi {
    ...
    async fn request_captcha(&self) -> Result<Response<Binary<Vec<u8>>>, DiemTapErrorResponse> {
        if !self.enabled {
            return Err(DiemTapError::new(
                "The CaptchaChecker is not enabled".to_string(),
                DiemTapErrorCode::EndpointNotEnabled,
            )
            .into());
        }
        ...
        let (key, image) = match captcha_manager.create_challenge() {
            ...
            Err(e) => {
                return Err(
                    DiemTapError::new_with_error_code(e, DiemTapErrorCode::CheckerError).into(),
                );
            },
        };
