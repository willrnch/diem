...
use crate::endpoints::{DiemTapError, DiemTapErrorCode};
...
static MINTER_SCRIPT: &[u8] = include_bytes!(
    "../../../../../diem-move/move-examples/scripts/minter/build/Minter/bytecode_scripts/main.mv"
);
...
impl MintFunder {
    ...
    async fn get_gas_unit_price(&self) -> Result<u64, DiemTapError> {
        match self.gas_unit_price_override {
            ...
            None => self
                ...
                .await
                .map_err(|e| {
                    DiemTapError::new_with_error_code(e, DiemTapErrorCode::DiemApiError)
                }),
        }
    }

    async fn get_transaction_factory(&self) -> Result<TransactionFactory, DiemTapError> {
        Ok(self
            .transaction_factory
            .clone()
    ...
    pub async fn use_delegated_account(&mut self) -> Result<()> {
        ...
        // Delegate minting to the account
        {
            ...
            client
                .submit_and_wait(&faucet_account.sign_with_transaction_builder(
                    transaction_factory.payload(diem_stdlib::diem_coin_delegate_mint_capability(
        ...
        client
            .submit_and_wait(&delegated_account.sign_with_transaction_builder(
                transaction_factory.payload(diem_stdlib::diem_coin_claim_mint_capability()),
            ))
            .await
    ...
    pub async fn process(
        ...
        check_only: bool,
        wait_for_transactions: bool,
    ) -> Result<Vec<SignedTransaction>, DiemTapError> {
