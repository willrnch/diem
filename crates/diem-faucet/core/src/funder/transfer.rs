...
use crate::{
    endpoints::{DiemTapError, DiemTapErrorCode, RejectionReason, RejectionReasonCode},
...
impl TransferFunder {
    ...
    async fn is_healthy_as_result(&self) -> Result<(), DiemTapError> {
...
impl FunderTrait for TransferFunder {
    ...
    async fn fund(
        ...
        let transactions = if check_only {
            ...
            let txn = self
                .execute_transaction(
                    &client,
                    diem_stdlib::diem_account_transfer(receiver_address, amount),
                    &receiver_address,
                )
                .await?;
            info!(
    ...
    async fn is_healthy(&self) -> FunderHealthMessage {
        ...
        let funder_balance = match self
            .get_api_client()
            .get_account_balance_bcs(account_address, "0x1::diem_coin::DiemCoin")
            .await
        {
