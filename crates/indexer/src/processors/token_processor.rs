...
use crate::{
    ...
    models::{
        ...
        token_models::{
            ...
            v2_token_utils::{
                DiemCollection, BurnEvent, FixedSupply, ObjectWithMetadata, PropertyMap, TokenV2,
...
use diem_api_types::{Transaction, TransactionPayload, WriteSetChange};
            ...
            for (_, wsc) in user_txn.info.changes.iter().enumerate() {
                if let WriteSetChange::WriteResource(wr) = wsc {
                    ...
                    if let Some(aggregated_data) = token_v2_metadata_helper.get_mut(&address) {
                        ...
                        if let Some(diem_collection) =
                            DiemCollection::from_write_resource(wr, txn_version).unwrap()
                        {
                            aggregated_data.diem_collection = Some(diem_collection);
                        }
