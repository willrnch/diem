...
### Installing fullnode
Please follow standard fullnode installation guide on diem.dev (https://diem.dev/nodes/full-node/fullnode-source-code-or-docker)

### Running indexer
```bash
cargo run -p diem-node --features "indexer" --release -- -f <some_path>/fullnode.yaml
```
...
> - Diem tests use the `INDEXER_DATABASE_URL` env var. It needs to be set for the relevant tests to run.
...
```bash
alias testnet="cd ~/Desktop/diem-core; CARGO_NET_GIT_FETCH_WITH_CLI=true cargo run -p diem-node -- --test"
```
