...
use diem_logger::{debug, sample, sample::SampleRate, warn};
use diem_rest_client::{diem_api_types::TransactionInfo, error::RestError, Client as RestClient};
use diem_sdk::{
    move_types::account_address::AccountAddress, types::transaction::SignedTransaction,
};
use diem_transaction_generator_lib::{CounterState, ReliableTransactionSubmitter};
...
impl RestApiReliableTransactionSubmitter {
    ...
    async fn submit_check_and_retry(
        ...
        for i in 0..self.max_retries {
            ...
            .concat();
            let mut seeded_rng = StdRng::from_seed(*diem_crypto::HashValue::sha3_256_of(&seed));
