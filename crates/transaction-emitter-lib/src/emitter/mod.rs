...
use diem_config::config::DEFAULT_MAX_SUBMIT_TRANSACTION_BATCH_SIZE;
use diem_logger::{debug, error, info, sample, sample::SampleRate, warn};
use diem_rest_client::{diem_api_types::DiemErrorCode, error::RestError, Client as RestClient};
use diem_sdk::{
    move_types::account_address::AccountAddress,
    transaction_builder::{diem_stdlib, TransactionFactory},
    types::{transaction::SignedTransaction, LocalAccount},
};
use diem_transaction_generator_lib::{create_txn_generator_creator, TransactionType};
...
impl Default for EmitJobRequest {
    fn default() -> Self {
        Self {
            ...
            max_gas_per_txn: diem_global_constants::MAX_GAS_AMOUNT,
            gas_price: diem_global_constants::GAS_UNIT_PRICE,
            ...
            expected_gas_per_txn: diem_global_constants::MAX_GAS_AMOUNT,
...
async fn wait_for_accounts_sequence(
    ...
    loop {
        ...
        if diem_infallible::duration_since_epoch().as_secs() >= txn_expiration_ts_secs + 240 {
            sample!(
    ...
    match &result {
        ...
        Err(e) => {
            ...
            if let RestError::Api(api_error) = e {
                if let DiemErrorCode::AccountNotFound = api_error.error.error_code {
                    return Ok((
                        (address, 0),
...
pub fn gen_transfer_txn_request(
    ...
    sender.sign_with_transaction_builder(
        txn_factory.payload(diem_stdlib::diem_coin_transfer(*receiver, num_coins)),
    )
}
