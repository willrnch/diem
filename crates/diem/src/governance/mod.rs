...
use diem_rest_client::{
    diem_api_types::{Address, HexEncodedBytes, U128, U64},
    Client, Transaction,
};
use diem_sdk::move_types::language_storage::CORE_CODE_ADDRESS;
use diem_types::{
    ...
    async fn execute(mut self) -> CliTypedResult<Vec<ProposalSummary>> {
        ...
        let events = client
            .get_account_events_bcs(
                AccountAddress::ONE,
                "0x1::diem_governance::GovernanceEvents",
...
async fn get_proposal(
    client: &diem_rest_client::Client,
...
impl CliCommand<ProposalSubmissionSummary> for SubmitProposal {
    ...
    async fn execute(mut self) -> CliTypedResult<ProposalSubmissionSummary> {
        ...
        let txn: Transaction = if self.is_multi_step {
            self.txn_options
                .submit_transaction(diem_stdlib::diem_governance_create_proposal_v2(
            ...
            self.txn_options
                .submit_transaction(diem_stdlib::diem_governance_create_proposal(
                    self.pool_address_args.pool_address,
                    script_hash.to_vec(),
                    self.metadata_url.to_string().as_bytes().to_vec(),
                    metadata_hash.to_hex().as_bytes().to_vec(),
        ...
        if let Transaction::UserTransaction(inner) = txn {
            ...
            let proposal_id = if let Some(event) = inner.events.into_iter().find(|event| {
                event.typ.to_string().as_str() == "0x1::diem_governance::CreateProposalEvent"
            }) {
    ...
    async fn execute(mut self) -> CliTypedResult<Vec<TransactionSummary>> {
        ...
        let voting_records = client
            .get_account_resource_bcs::<VotingRecords>(
                CORE_CODE_ADDRESS,
                "0x1::diem_governance::VotingRecords",
            )
            .await
            .unwrap()
            .into_inner()
        ...
        for pool_address in self.pool_addresses {
            ...
            summaries.push(
                self.txn_options
                    .submit_transaction(diem_stdlib::diem_governance_vote(
                        pool_address,
                        proposal_id,
                        vote,
                    ))
...
/// This is needed for upgrading large packages such as diem-framework.
#[derive(Parser)]
...
impl CliCommand<TransactionSummary> for ApproveExecutionHash {
    ...
    async fn execute(mut self) -> CliTypedResult<TransactionSummary> {
        Ok(self
            .txn_options
            .submit_transaction(
                diem_stdlib::diem_governance_add_approved_script_hash_script(self.proposal_id),
            )
            .await
            .map(TransactionSummary::from)?)
    }
        ...
        let framework_local_dir = if self.framework_local_dir.is_some() {
            ...
            Option::from({
                ...
                path.join("diem-move")
                    ...
                    .map_err(|err| {
                        CliError::IO(
                            format!("Failed to canonicalize diem framework path: {:?}", path),
                            err,
                        )
                    })?
            })
...
struct JsonProposal {
    ...
    execution_hash: diem_rest_client::diem_api_types::HashValue,
    expiration_secs: U64,
    is_resolved: bool,
    min_vote_threshold: U128,
    no_votes: U128,
