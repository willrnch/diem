...
use diem_rest_client::{
    diem_api_types::{MoveStructTag, MoveType},
    Transaction,
};
use diem_sdk::move_types::{account_address::AccountAddress, language_storage::ModuleId};
...
impl CliTestFramework {
    ...
    async fn check_account_exists(&self, index: usize) -> bool {
        ...
        let client = diem_rest_client::Client::new(self.endpoint.clone());
            ...
            entry_function_args: EntryFunctionArguments {
                ...
                type_arg_vec: TypeArgVec {
                    type_args: vec![MoveType::Struct(MoveStructTag::new(
                        ...
                        ident_str!("diem_coin").into(),
                        ident_str!("DiemCoin").into(),
                        vec![],
                    ))],
                },
                json_file: None,
    ...
    async fn last_n_transactions_details(&self, count: u16) -> String {
        let result = diem_rest_client::Client::new(self.endpoint.clone())
    ...
    pub fn add_move_files(&self) {
        ...
        let hello_blockchain_test_contents = include_str!("../../../../diem-move/move-examples/hello_blockchain/sources/hello_blockchain_test.move");
    ...
    /// Runs the given script contents using the local diem_framework directory.
    pub async fn run_script(
        ...
        self.run_script_with_framework_package(index, script_contents, FrameworkPackageArgs {
            framework_git_rev: None,
            framework_local_dir: Some(Self::diem_framework_dir()),
            skip_fetch_latest_git_deps: false,
        })
        .await
    }

    /// Runs the given script contents using the diem_framework from diem-core git repository.
