...
use diem_config::config::Token;
use diem_framework::ReleaseBundle;
use diem_genesis::config::Layout;
...
impl FromStr for GithubRepo {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if parts.len() != 2 {
            Err(CliError::CommandArgumentError("Invalid repository must be of the form 'owner/repository` e.g. 'aptos-labs/diem-core'".to_string()))
        } else {
            Ok(GithubRepo {
...
pub struct GitOptions {
    /// Github repository e.g. 'aptos-labs/diem-core'
    ///
