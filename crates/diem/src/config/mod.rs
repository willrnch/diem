...
use diem_cli_common::generate_cli_completions;
...
/// Tool for interacting with configuration of the Diem CLI tool
///
...
impl CliCommand<()> for GenerateShellCompletions {
    ...
    async fn execute(self) -> CliTypedResult<()> {
        generate_cli_completions::<Tool>("diem", self.shell, self.output_file.as_path())
...
pub struct SetGlobalConfig {
    ...
    ///
    /// `Workspace` will put the `.diem/` folder in the current directory, where
    /// `Global` will put the `.diem/` folder in your home directory
    #[clap(long)]
...
pub enum ConfigType {
    /// Per system user configuration put in `<HOME>/.diem`
    Global,
    /// Per directory configuration put in `<CURRENT_DIR>/.diem`
    Workspace,
}
