...
pub struct UpdateTool {
    ...
    #[clap(long, default_value = "diem-core")]
    repo_name: String,
}
    ...
    fn update(&self) -> CliTypedResult<String> {
        ...
        match installation_method {
            ...
            InstallationMethod::Homebrew => {
                return Err(anyhow!(
                    "Detected this CLI comes from homebrew, use `brew upgrade diem` instead"
                )
                .into());
            },
        ...
        let result = config
            .update()
            .map_err(|e| anyhow!("Failed to update Diem CLI: {:#}", e))?;
...
impl CliCommand<String> for UpdateTool {
    ...
    async fn execute(self) -> CliTypedResult<String> {
        tokio::task::spawn_blocking(move || self.update())
            .await
            .context("Failed to self-update Diem CLI")?
    }
}
