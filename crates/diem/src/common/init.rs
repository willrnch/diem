...
use diem_crypto::{ed25519::Ed25519PrivateKey, PrivateKey, ValidCryptoMaterialStringExt};
...
/// Tool to initialize current directory for the diem tool
///
/// Configuration will be pushed into .diem/config.yaml
#[derive(Debug, Parser)]
pub struct InitTool {
...
impl CliCommand<()> for InitTool {
    ...
    async fn execute(self) -> CliTypedResult<()> {
        ...
        let mut profile_config = if let Some(profile_config) = config.remove_profile(profile_name) {
            prompt_yes_with_override(&format!("Diem already initialized for profile {}, do you want to overwrite the existing config?", profile_name), self.prompt_options)?;
            profile_config
        } else {
        ...
        match network {
            Network::Mainnet => {
                profile_config.rest_url =
                    Some("https://fullnode.mainnet.diemlabs.com".to_string());
                profile_config.faucet_url = None;
            },
            Network::Testnet => {
                profile_config.rest_url =
                    Some("https://fullnode.testnet.diemlabs.com".to_string());
                profile_config.faucet_url =
                    Some("https://faucet.testnet.diemlabs.com".to_string());
            },
            Network::Devnet => {
                profile_config.rest_url = Some("https://fullnode.devnet.diemlabs.com".to_string());
                profile_config.faucet_url = Some("https://faucet.devnet.diemlabs.com".to_string());
            },
            Network::Local => {
        ...
        let client = diem_rest_client::Client::new(
            Url::parse(
                profile_config
                    .rest_url
                    .as_ref()
        ...
        config.save()?;
        eprintln!("\n---\nDiem CLI is now set up for account {} as profile {}!  Run `diem --help` for more information about commands", address, self.profile_options.profile_name().unwrap_or(DEFAULT_PROFILE));
        Ok(())
    }
}
