...
use diem_gas_profiling::FrameName;
use diem_global_constants::adjust_gas_headroom;
use diem_keygen::KeyGen;
use diem_logger::Level;
use diem_rest_client::{
    diem_api_types::{EntryFunctionId, HashValue, MoveType, ViewRequest},
    error::RestError,
    DiemBaseUrl, Client, Transaction,
};
...
pub const USER_AGENT: &str = concat!("diem-cli/", env!("CARGO_PKG_VERSION"));
...
const X_DIEM_CLIENT_VALUE: &str = concat!("diem-cli/", env!("CARGO_PKG_VERSION"));
...
impl From<diem_config::config::Error> for CliError {
    fn from(e: diem_config::config::Error) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}

impl From<diem_github_client::Error> for CliError {
    fn from(e: diem_github_client::Error) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}
...
impl From<diem_crypto::CryptoMaterialError> for CliError {
    fn from(e: diem_crypto::CryptoMaterialError) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}
...
/// Config saved to `.diem/config.yaml`
...
pub const CONFIG_FOLDER: &str = ".diem";
...
pub struct ProfileConfig {
    ...
    /// URL for the Diem rest endpoint
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl CliConfig {
    ...
    pub fn config_exists(mode: ConfigSearchMode) -> bool {
        if let Ok(folder) = Self::diem_folder(mode) {
    ...
    pub fn load(mode: ConfigSearchMode) -> CliTypedResult<Self> {
        let folder = Self::diem_folder(mode)?;
    ...
    /// Saves the config to ./.diem/config.yaml
    pub fn save(&self) -> CliTypedResult<()> {
        let diem_folder = Self::diem_folder(ConfigSearchMode::CurrentDir)?;

        // Create if it doesn't exist
        create_dir_if_not_exist(diem_folder.as_path())?;

        // Save over previous config file
        let config_file = diem_folder.join(CONFIG_FILE);
        ...
        let legacy_config_file = diem_folder.join(LEGACY_CONFIG_FILE);
    ...
    /// Finds the current directory's .diem folder
    fn diem_folder(mode: ConfigSearchMode) -> CliTypedResult<PathBuf> {
...
impl RestOptions {
    ...
    pub fn url(&self, profile: &ProfileOptions) -> CliTypedResult<reqwest::Url> {
        ...
        .map(|p| p.rest_url)
        {
            ...
            Err(CliError::CommandArgumentError("No rest url given.  Please add --url or add a rest_url to the .diem/config.yaml for the current profile".to_string()))
        }
    }

    pub fn client(&self, profile: &ProfileOptions) -> CliTypedResult<Client> {
        Ok(Client::builder(DiemBaseUrl::Custom(self.url(profile)?))
            .timeout(Duration::from_secs(self.connection_timeout_secs))
            .header(diem_api_types::X_DIEM_CLIENT, X_DIEM_CLIENT_VALUE)?
            .build())
    }
}
...
pub fn load_account_arg(str: &str) -> Result<AccountAddress, CliError> {
            ...
            .map(|p| p.private_key)
    {
        ...
        Err(CliError::CommandArgumentError(
            "'--account' or '--profile' after using diem init must be provided".to_string(),
        ))
    }
}
...
pub struct FaucetOptions {
    /// URL for the faucet endpoint e.g. `https://faucet.devnet.diemlabs.com`
    #[clap(long)]
...
impl FaucetOptions {
    ...
    pub fn faucet_url(&self, profile: &ProfileOptions) -> CliTypedResult<reqwest::Url> {
        ...
        .map(|profile| profile.faucet_url)
        {
            ...
            Err(CliError::CommandArgumentError("No faucet given.  Please add --faucet-url or add a faucet URL to the .diem/config.yaml for the current profile".to_string()))
        }
    }
}
...
impl TransactionOptions {
    ...
    pub async fn profile_gas(
        ...
        let debugger = DiemDebugger::rest_client(client).unwrap();
