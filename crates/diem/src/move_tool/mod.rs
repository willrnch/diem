...
use diem_gas::{AbstractValueSizeGasParameters, NativeGasParameters};
use diem_rest_client::diem_api_types::{
...
pub struct FrameworkPackageArgs {
    /// Git revision or branch for the Diem framework
    ///
    ...
    /// Local framework directory for the Diem framework
    ///
...
impl FrameworkPackageArgs {
    pub fn init_move_dir(
        ...
        const DIEM_FRAMEWORK: &str = "DiemFramework";
        const DIEM_GIT_PATH: &str = "https://github.com/aptos-labs/diem-core.git";
        const SUBDIR_PATH: &str = "diem-move/framework/diem-framework";
        ...
        if let Some(ref path) = self.framework_local_dir {
            dependencies.insert(DIEM_FRAMEWORK.to_string(), Dependency {
            ...
            dependencies.insert(DIEM_FRAMEWORK.to_string(), Dependency {
                local: None,
                git: Some(DIEM_GIT_PATH.to_string()),
...
/// This can then be run with `diem move run-script`
#[derive(Parser)]
...
pub struct TestPackage {
    ...
    /// Collect coverage information for later use with the various `diem move coverage` subcommands
    #[clap(long = "coverage")]
...
impl CliCommand<&'static str> for TestPackage {
    ...
    async fn execute(self) -> CliTypedResult<&'static str> {
        ...
        let result = move_cli::base::test::run_move_unit_tests(
            ...
            diem_debug_natives::diem_debug_natives(
        ...
        if self.compute_coverage {
            ...
            println!("Please use `diem move coverage -h` for more detailed source or bytecode test coverage of this package");
        }
    ...
    async fn execute(self) -> CliTypedResult<TransactionSummary> {
        ...
        .map(|p| p.account)
        {
            ...
            return Err(CliError::CommandArgumentError(
                "Please provide an account using --profile or run diem init".to_string(),
            ));
        };
        ...
        let payload = diem_cached_packages::diem_stdlib::resource_account_create_resource_account_and_publish_package(
            seed,
