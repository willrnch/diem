...
/// # Safety
///
/// Run the diem CLI async; Use this function if you are expecting the diem CLI command
/// to run in the background, or different thread
/// Note: This function should only be called from other SDK (i.g Typescript)
///
/// Return: the pointer to c string: 'true'
#[no_mangle]
pub unsafe extern "C" fn run_diem_async(s: *mut c_char) -> *mut c_char {
    println!("Running diem...");
    let c_str = unsafe {
        assert!(!s.is_null());
        CStr::from_ptr(s)
    };
...
/// # Safety
///
/// After running the diem CLI using FFI. Make sure to invoke this method to free up or
