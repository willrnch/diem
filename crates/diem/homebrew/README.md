...
The [Diem command line interface (CLI)](https://diem.dev/cli-tools/diem-cli-tool/install-diem-cli) may be installed via [Homebrew](https://brew.sh/) for simplicity. This is an in-depth overview of Homebrew and the Diem formula. In this guide, we go over each section of the Homebrew formula and steps to implement changes in the future.

## Quick guide

- [Formula in Homebrew GitHub](https://github.com/Homebrew/homebrew-core/blob/master/Formula/diem.rb)
- [Diem 1.0.3 New Formula PR for GitHub](https://github.com/Homebrew/homebrew-core/pull/119832)
- [Diem Formula Fix PR to use build_cli_release.sh](https://github.com/Homebrew/homebrew-core/pull/120051)
...
# diem 1.0.5
# Diem Labs <opensource@diemlabs.com>
...
Note: This guide is for developers who are trying to update the Diem homebrew formula.

Copy the `diem.rb` file to your `homebrew` `formula` directory. For example, on macOS with an M1, this will likely be:
...
After you've copied `diem.rb` to your local `homebrew` `formula` directory, you can modify it and use the commands below for testing.
...
brew audit --new-formula diem      # For new formula
brew audit diem --strict --online
brew install diem
brew test diem

# For debugging issues during the installation process you can do
brew install diem --interactive    # Interactive, gives you access to the shell
brew install diem -d               # Debug mode

# Livecheck
brew livecheck --debug diem
```
...
1. Commit your changes to `diem-core` in `crates/diem/homebrew`.
...
class Diem < Formula
  ...
  url "https://github.com/aptos-labs/diem-core/archive/refs/tags/diem-cli-v1.0.3.tar.gz"
...
[Brew livecheck](https://docs.brew.sh/Brew-Livecheck) uses strategies to find the newest version of a formula or cask’s software by checking upstream. The strategy used below checks for all `diem-cli-v<SEMVER>` tags for `diem-core`. The regex ensures that releases for other, non-CLI builds are not factored into livecheck. 
...
...
  ...
  # with matching regex of href="<URL>/tag/diem-cli-v<SEMVER>". This
  # is done to automatically check for new release versions of the CLI.
  livecheck do
    url :stable
    regex(/^diem-cli[._-]v?(\d+(?:\.\d+)+)$/i)
  end
...
```
...
```ruby
  # Installs listed homebrew dependencies before Diem installation
  # Dependencies needed: https://diem.dev/cli-tools/build-diem-cli
  # See scripts/dev_setup.sh in diem-core for more info
  ...
  # root Cargo.toml file of diem-core (currently it is pegged to Rust 
...
The current test generates a new key via the Diem CLI and ensures the shell output matches the filename(s) for that key.

```ruby
  ...
  test do
    assert_match(/output.pub/i, shell_output("#{bin}/diem key generate --output-file output"))
  end
  ...
```
