...
def run_node(network: Network, image_repo_with_project: str):
    ...
    container_name = f"diem-tools-{network}"
    LOG.info(f"Trying to run diem CLI local testnet from image: {image_name}")
    ...
    LOG.info(f"Running diem CLI local testnet from image: {image_name}")
