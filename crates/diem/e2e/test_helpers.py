...
class RunHelper:
    ...
    def prepare_move(self):
        shutil.copytree(
            "../../../diem-move/move-examples/cli-e2e-tests",
        ...
        if self.image_tag:
            ...
            if response["Result"]["config_type"].lower() != "workspace":
                raise RuntimeError(
                    ...
                    "try running `diem config set-global-config --config-type workspace`"
                )

    # Get the account info of the account created by test_init.
    def get_account_info(self):
        path = os.path.join(self.host_working_directory, ".diem", "config.yaml")
        with open(path) as f:
