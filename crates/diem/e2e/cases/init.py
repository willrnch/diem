...
def test_init(run_helper: RunHelper, test_name=None):
    ...
    run_helper.run_command(
        test_name,
        ["diem", "init", "--assume-yes", "--network", "local"],
        input="\n",
    )

    # Assert that the CLI config is there.
    config_path = os.path.join(
        run_helper.host_working_directory, ".diem", "config.yaml"
    )
    if not os.path.exists(config_path):
        raise TestError(
            f"{config_path} not found (in host working dir) after running diem init"
        )
...
@test_case
def test_diem_header_included(run_helper: RunHelper, test_name=None):
    # Make sure the diem-cli header is included on the original request
    response = requests.get(run_helper.get_metrics_url())

    if 'request_source_client="diem-cli' not in response.text:
        raise TestError("Request should contain the correct diem header: diem-cli")
