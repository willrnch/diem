...
use crate::{
    ...
    test_utils::{random_subset, KeyPair, TestDiemCrypto},
...
fn random_message_for_signing(rng: &mut OsRng) -> TestDiemCrypto {
    TestDiemCrypto(
...
fn random_messages_for_signing(rng: &mut OsRng, n: usize) -> Vec<TestDiemCrypto> {
    (0..n)
        .map(|_| random_message_for_signing(rng))
        .collect::<Vec<TestDiemCrypto>>()
}
...
fn bls12381_aggsig_should_verify() {
    ...
    let msgs_refs = messages.iter().collect::<Vec<&TestDiemCrypto>>();
    ...
    let msgs_wrong_refs = messages_wrong.iter().collect::<Vec<&TestDiemCrypto>>();
...
/// diem-move/framework/move-stdlib/sources/signer.move
...
fn bls12381_sample_signature_verifies() {
    ...
    assert!(sig.verify_arbitrary_msg(b"Hello Diem!", &pk).is_ok());
}
...
fn bls12381_sample_doc_test_for_normal_sigs() {
    ...
    let message = TestDiemCrypto("test".to_owned());
    ...
    println!("// signature on TestDiemCrypto(\"test\".to_owned())");
    println!(
...
fn bls12381_sample_aggregate_pk_and_aggsig() {
    ...
    println!(
        "// The signed messages are \"Hello, Diem <i>!\", where <i> \\in {{1, ..., {}}}",
        num
    );
