...
use diem_crypto::{x25519, ValidCryptoMaterialStringExt};
...
#[clap(name = "Diem Telemetry Service", author, version)]
pub struct DiemTelemetryServiceArgs {
    #[clap(short = 'f', long, value_parser)]
    config_path: PathBuf,
}

impl DiemTelemetryServiceArgs {
    pub async fn run(self) {
        ...
        let validators = Arc::new(diem_infallible::RwLock::new(HashMap::new()));
        let validator_fullnodes = Arc::new(diem_infallible::RwLock::new(HashMap::new()));
...
fn verify_tool() {
    ...
    DiemTelemetryServiceArgs::command().debug_assert()
}
