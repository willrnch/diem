...
use diem_api_types::{
    verify_function_identifier, verify_module_identifier, Address, DiemError, DiemErrorCode,
...
use diem_crypto::{hash::CryptoHash, signing_message};
use diem_types::{
...
use diem_vm::{data_cache::AsMoveResolver, DiemVM};
use poem_openapi::{
    param::{Path, Query},
    payload::Json,
    ApiRequest, OpenApi,
...
pub enum SubmitTransactionPost {
    ...
    // TODO: https://github.com/aptos-labs/diem-core/issues/2275
    #[oai(content_type = "application/x.diem.signed_transaction+bcs")]
    Bcs(Bcs),
}
...
impl TransactionsApi {
    ...
    ///   1. Hash message bytes: "RawTransaction" bytes + BCS bytes of [Transaction](https://aptos-labs.github.io/diem-core/diem_types/transaction/enum.Transaction.html).
    ...
    /// Make sure to use the `application/x.diem.signed_transaction+bcs` Content-Type.
    ...
    async fn encode_submission(
        ...
        data.0
            ...
            .map_err(|err| {
                BasicError::bad_request_with_code_no_info(err, DiemErrorCode::InvalidInput)
            })?;
...
impl TransactionsApi {
    ...
    async fn create_internal(&self, txn: SignedTransaction) -> Result<(), DiemError> {
        ...
        match mempool_status.code {
            ...
            MempoolStatusCode::InvalidSeqNumber => Err(DiemError::new_with_error_code(
                mempool_status.message,
                DiemErrorCode::SequenceNumberTooOld,
            )),
            MempoolStatusCode::InvalidUpdate => Err(DiemError::new_with_error_code(
                mempool_status.message,
                DiemErrorCode::InvalidTransactionUpdate,
            )),
            MempoolStatusCode::UnknownStatus => Err(DiemError::new_with_error_code(
    ...
    async fn create(
        ...
        match self.create_internal(txn.clone()).await {
            ...
            Err(error) => match error.error_code {
                DiemErrorCode::InternalError => Err(
                    SubmitTransactionError::internal_from_diem_error(error, ledger_info),
                ),
                DiemErrorCode::VmError
                | DiemErrorCode::SequenceNumberTooOld
                | DiemErrorCode::InvalidTransactionUpdate => Err(
                    SubmitTransactionError::bad_request_from_diem_error(error, ledger_info),
                ),
                DiemErrorCode::MempoolIsFull => Err(
                    SubmitTransactionError::insufficient_storage_from_diem_error(
                        error,
                        ledger_info,
                    ),
                ),
                _ => Err(SubmitTransactionError::internal_from_diem_error(
                    error,
                    ledger_info,
                )),
            },
    ...
    // TODO: This function leverages a lot of types from diem_types, use the
    ...
    pub async fn simulate(
        ...
        let (_, output) = DiemVM::simulate_signed_transaction(&txn, &move_resolver);
        ...
        let txn = diem_types::transaction::Transaction::UserTransaction(txn);
        let zero_hash = diem_crypto::HashValue::zero();
        let info = diem_types::transaction::TransactionInfo::new(
            txn.hash(),
            zero_hash,
            zero_hash,
            None,
    ...
    pub fn get_signing_message(
        ...
        let raw_txn: RawTransaction = resolver
            ...
            .map_err(|err| {
                BasicError::bad_request_with_code(err, DiemErrorCode::InvalidInput, &ledger_info)
            })?;
