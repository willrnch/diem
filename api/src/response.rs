...
//! The Diem API response / error handling philosophy.
//!
...
// TODO: https://github.com/aptos-labs/diem-core/issues/2279

use super::{accept_type::AcceptType, bcs_payload::Bcs};
use diem_api_types::{Address, DiemError, DiemErrorCode, HashValue, LedgerInfo};
use move_core_types::{
...
pub enum DiemResponseContent<T: ToJSON + Send + Sync> {
...
/// an error response to manipulate the DiemError inside it.
pub trait DiemErrorResponse {
    fn inner_mut(&mut self) -> &mut DiemError;
}
...
/// implementor also implements DiemErrorResponse, which saves functions from
...
macro_rules! generate_error_traits {
    ($($trait_name:ident),*) => {
        paste::paste! {
        $(
        pub trait [<$trait_name Error>]: DiemErrorResponse {
            // With ledger info and an error code
            fn [<$trait_name:snake _with_code>]<Err: std::fmt::Display>(
                err: Err,
                error_code: diem_api_types::DiemErrorCode,
                ledger_info: &diem_api_types::LedgerInfo,
            ...
            fn [<$trait_name:snake _with_vm_status>]<Err: std::fmt::Display>(
                err: Err,
                error_code: diem_api_types::DiemErrorCode,
                vm_status: diem_types::vm_status::StatusCode,
                ledger_info: &diem_api_types::LedgerInfo
            ) -> Self where Self: Sized;

            fn [<$trait_name:snake _from_diem_error>](
                diem_error: diem_api_types::DiemError,
                ledger_info: &diem_api_types::LedgerInfo
            ) -> Self where Self: Sized;
        }
        )*
...
macro_rules! generate_error_response {
    ($enum_name:ident, $(($status:literal, $name:ident)),*) => {
        ...
        pub enum $enum_name {
            $(
            #[oai(status = $status)]
            $name(poem_openapi::payload::Json<Box<diem_api_types::DiemError>>,
                // We use just regular u64 here instead of U64 since all header
                // values are implicitly strings anyway.
                /// Chain ID of the current chain
                #[oai(header = "X-Diem-Chain-Id")] Option<u8>,
                /// Current ledger version of the chain
                #[oai(header = "X-Diem-Ledger-Version")] Option<u64>,
                /// Oldest non-pruned ledger version of the chain
                #[oai(header = "X-Diem-Ledger-Oldest-Version")] Option<u64>,
                /// Current timestamp of the chain
                #[oai(header = "X-Diem-Ledger-TimestampUsec")] Option<u64>,
                /// Current epoch of the chain
                #[oai(header = "X-Diem-Epoch")] Option<u64>,
                /// Current block height of the chain
                #[oai(header = "X-Diem-Block-Height")] Option<u64>,
                /// Oldest non-pruned block height of the chain
                #[oai(header = "X-Diem-Oldest-Block-Height")] Option<u64>,
            ),
            )*
        }
        ...
        impl $crate::response::[<$name Error>] for $enum_name {
            fn [<$name:snake _with_code>]<Err: std::fmt::Display>(
                ...
                let error = diem_api_types::DiemError::new_with_error_code(err, error_code);
            ...
            fn [<$name:snake _with_vm_status>]<Err: std::fmt::Display>(
                ...
                let error = diem_api_types::DiemError::new_with_vm_status(err, error_code, vm_status);
                let payload = poem_openapi::payload::Json(Box::new(error));
                Self::from($enum_name::$name(
                    payload,
                    Some(ledger_info.chain_id),
            ...
            fn [<$name:snake _from_diem_error>](
                diem_error: diem_api_types::DiemError,
                ledger_info: &diem_api_types::LedgerInfo
            ) -> Self where Self: Sized {
                let payload = poem_openapi::payload::Json(Box::new(diem_error));
                Self::from($enum_name::$name(
                    payload,
                    Some(ledger_info.chain_id),
                    Some(ledger_info.ledger_version.into()),
        ...
        // Generate a function that helps get the DiemError within.
        impl $crate::response::DiemErrorResponse for $enum_name {
            fn inner_mut(&mut self) -> &mut diem_api_types::DiemError {
                match self {
                    $(
    ...
    ($enum_name:ident, $(($status:literal, $name:ident)),*) => {
        ...
        pub enum $enum_name<T: poem_openapi::types::ToJSON + Send + Sync> {
            ...
            $name(
                // We use just regular u64 here instead of U64 since all header
                // values are implicitly strings anyway.
                $crate::response::DiemResponseContent<T>,
                /// Chain ID of the current chain
                #[oai(header = "X-Diem-Chain-Id")] u8,
                /// Current ledger version of the chain
                #[oai(header = "X-Diem-Ledger-Version")] u64,
                /// Oldest non-pruned ledger version of the chain
                #[oai(header = "X-Diem-Ledger-Oldest-Version")] u64,
                /// Current timestamp of the chain
                #[oai(header = "X-Diem-Ledger-TimestampUsec")] u64,
                /// Current epoch of the chain
                #[oai(header = "X-Diem-Epoch")] u64,
                /// Current block height of the chain
                #[oai(header = "X-Diem-Block-Height")] u64,
                /// Oldest non-pruned block height of the chain
                #[oai(header = "X-Diem-Oldest-Block-Height")] u64,
                ...
                #[oai(header = "X-Diem-Cursor")] Option<String>,
            ),
            )*
        }
        ...
        // Generate a From impl that builds a response from DiemResponseContent.
        ...
        impl <T: poem_openapi::types::ToJSON + Send + Sync> From<($crate::response::DiemResponseContent<T>, &diem_api_types::LedgerInfo, [<$enum_name Status>])>
            for $enum_name<T>
        {
            fn from(
                (value, ledger_info, status): (
                    $crate::response::DiemResponseContent<T>,
                    &diem_api_types::LedgerInfo,
                    [<$enum_name Status>]
                ),
            ) -> Self {
                match status {
        ...
        impl<T: poem_openapi::types::ToJSON + Send + Sync> From<(poem_openapi::payload::Json<T>, &diem_api_types::LedgerInfo, [<$enum_name Status>])>
            for $enum_name<T>
        {
            fn from(
                (value, ledger_info, status): (poem_openapi::payload::Json<T>, &diem_api_types::LedgerInfo, [<$enum_name Status>]),
            ) -> Self {
                let content = $crate::response::DiemResponseContent::Json(value);
                Self::from((content, ledger_info, status))
            }
        }

        // Generate a From impl that builds a response from a Bcs<Vec<u8>> and friends.
        impl<T: poem_openapi::types::ToJSON + Send + Sync> From<($crate::bcs_payload::Bcs, &diem_api_types::LedgerInfo, [<$enum_name Status>])>
            for $enum_name<T>
        {
            fn from(
                (value, ledger_info, status): (
                ...
                let content = $crate::response::DiemResponseContent::Bcs(value);
                Self::from((content, ledger_info, status))
            }
        }
        ...
        impl<T: poem_openapi::types::ToJSON + Send + Sync + serde::Serialize> $enum_name<T> {
                        ...
                        $crate::bcs_payload::Bcs(
                            bcs::to_bytes(&value)
                                .map_err(|e| E::internal_with_code(
                                    e,
                                    diem_api_types::DiemErrorCode::InternalError,
                                    ledger_info
                                ))?
                        ),
                        ledger_info,
           ...
           pub fn try_from_json<E: $crate::response::InternalError>(
            ...
            pub fn with_cursor(mut self, new_cursor: Option<diem_types::state_store::state_key::StateKey>) -> Self {
                match self {
                    $(
                    [<$enum_name>]::$name(_, _, _, _, _, _, _, _, ref mut cursor) => {
                        *cursor = new_cursor.map(|c| diem_api_types::StateKeyWrapper::from(c).to_string());
                    }
                    )*
                }
                self
...
pub fn transaction_not_found_by_version<E: NotFoundError>(
    ...
    build_not_found(
        "Transaction",
        format!("Ledger version({})", ledger_version),
        DiemErrorCode::TransactionNotFound,
        ledger_info,
    )
}
...
pub fn resource_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        DiemErrorCode::ResourceNotFound,
        ledger_info,
    )
}
...
pub fn struct_field_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        DiemErrorCode::StructFieldNotFound,
        ledger_info,
    )
}
...
pub fn table_item_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        DiemErrorCode::TableItemNotFound,
        ledger_info,
    )
}
