...
use diem_api_types::{
    DiemErrorCode, AsConverter, BcsBlock, GasEstimation, LedgerInfo, ResourceGroup,
...
impl Context {
    ...
    pub fn latest_state_view_poem<E: InternalError>(
        ...
        self.db
            ...
            .map_err(|e| E::internal_with_code(e, DiemErrorCode::InternalError, ledger_info))
    }
    ...
    pub fn state_view<E: StdApiError>(
        ...
        let state_view = self
            ...
            .map_err(|err| {
                E::internal_with_code(err, DiemErrorCode::InternalError, &latest_ledger_info)
            })?;
    ...
    pub fn get_latest_ledger_info<E: ServiceUnavailableError>(&self) -> Result<LedgerInfo, E> {
        let maybe_oldest_version = self
            ...
            .map_err(|e| {
                E::service_unavailable_with_code_no_info(e, DiemErrorCode::InternalError)
            })?;
    ...
    pub fn get_block_timestamp<E: InternalError>(
        ...
        self.db
            ...
            .map_err(|err| E::internal_with_code(err, DiemErrorCode::InternalError, ledger_info))
    }
    ...
    pub fn render_transactions_sequential<E: InternalError>(
        ...
        mut timestamp: u64,
    ) -> Result<Vec<diem_api_types::Transaction>, E> {
        if data.is_empty() {
            return Ok(vec![]);
        }
        ...
        let txns: Vec<diem_api_types::Transaction> = data
            .into_iter()
            .map(|t| {
            ...
            .map_err(|err| {
                E::internal_with_code(err, DiemErrorCode::InternalError, ledger_info)
            })?;
    ...
    pub fn get_gas_schedule<E: InternalError>(
        &self,
        ledger_info: &LedgerInfo,
    ) -> Result<(u64, DiemGasParameters), E> {
        ...
        // Otherwise refresh the cache
        {
            ...
            let state_view = self
                ...
                .map_err(|e| {
                    E::internal_with_code(e, DiemErrorCode::InternalError, ledger_info)
                })?;
            ...
            let gas_schedule_params =
                match GasScheduleV2::fetch_config(&storage_adapter).and_then(|gas_schedule| {
                    ...
                    DiemGasParameters::from_on_chain_gas_schedule(&gas_schedule, feature_version)
                        .ok()
                }) {
                    ...
                    None => GasSchedule::fetch_config(&storage_adapter)
                        .and_then(|gas_schedule| {
                            let gas_schedule = gas_schedule.to_btree_map();
                            DiemGasParameters::from_on_chain_gas_schedule(&gas_schedule, 0).ok()
                        })
                        .ok_or_else(|| {
                            E::internal_with_code(
...
pub struct GasScheduleCache {
    ...
    gas_schedule_params: Option<DiemGasParameters>,
}
