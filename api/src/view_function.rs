...
use diem_api_types::{DiemErrorCode, AsConverter, MoveValue, ViewRequest, U64};
...
impl ViewFunctionApi {
    ...
    ///
    /// The Diem nodes prune account state history, via a configurable time window.
    ...
    async fn view_function(
        ...
        let return_vals = DiemVM::execute_view_function(
            &state_view,
        ...
        .map_err(|err| {
            BasicErrorWith404::bad_request_with_code_no_info(err, DiemErrorCode::InvalidInput)
        })?;
        match accept_type {
            AcceptType::Bcs => {
