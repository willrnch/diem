...
pub struct DiemError {
    /// A message describing the error
    pub message: String,
    pub error_code: DiemErrorCode,
    /// A code providing VM error details when submitting transactions to the VM
    pub vm_error_code: Option<u64>,
}

impl std::fmt::Display for DiemError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "Error({:?}): {:#}", self.error_code, self.message)
    }
}

impl std::error::Error for DiemError {}
...
fn test_serialize_deserialize() {
    let with_code = DiemError::new_with_vm_status(
        "Invalid transaction",
        DiemErrorCode::VmError,
        diem_types::vm_status::StatusCode::UNKNOWN_MODULE,
    );
    let _: DiemError = bcs::from_bytes(&bcs::to_bytes(&with_code).unwrap()).unwrap();
    let _: DiemError = serde_json::from_str(&serde_json::to_string(&with_code).unwrap()).unwrap();

    let without_code =
        DiemError::new_with_error_code("some message", DiemErrorCode::MempoolIsFull);
    let _: DiemError = bcs::from_bytes(&bcs::to_bytes(&without_code).unwrap()).unwrap();
    let _: DiemError =
