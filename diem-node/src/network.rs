...
use diem_consensus::network_interface::{ConsensusMsg, DIRECT_SEND, RPC};
use diem_event_notifications::EventSubscriptionService;
use diem_logger::debug;
use diem_mempool::network::MempoolSyncMsg;
use diem_network::{
    application::{
...
use diem_network_builder::builder::NetworkBuilder;
...
pub fn consensus_network_configuration(node_config: &NodeConfig) -> NetworkApplicationConfig {
    ...
    let network_service_config = NetworkServiceConfig::new(
        ...
        rpc_protocols,
        diem_channel::Config::new(node_config.consensus.max_network_channel_size)
            .queue_style(QueueStyle::FIFO)
            .counters(&diem_consensus::counters::PENDING_CONSENSUS_NETWORK_EVENTS),
    );
...
pub fn mempool_network_configuration(node_config: &NodeConfig) -> NetworkApplicationConfig {
    ...
    let network_service_config = NetworkServiceConfig::new(
        direct_send_protocols,
        rpc_protocols,
        diem_channel::Config::new(node_config.mempool.max_network_channel_size)
            .queue_style(QueueStyle::KLAST) // TODO: why is this not FIFO?
            .counters(&diem_mempool::counters::PENDING_MEMPOOL_NETWORK_EVENTS),
    );
    NetworkApplicationConfig::new(network_client_config, network_service_config)
}
...
pub fn peer_monitoring_network_configuration(node_config: &NodeConfig) -> NetworkApplicationConfig {
    ...
    let network_service_config = NetworkServiceConfig::new(
        direct_send_protocols,
        rpc_protocols,
        diem_channel::Config::new(max_network_channel_size)
            .queue_style(QueueStyle::FIFO)
            .counters(
                &diem_peer_monitoring_service_server::metrics::PENDING_PEER_MONITORING_SERVER_NETWORK_EVENTS,
            ),
    );
    NetworkApplicationConfig::new(network_client_config, network_service_config)
}
...
pub fn storage_service_network_configuration(node_config: &NodeConfig) -> NetworkApplicationConfig {
    ...
    let network_service_config = NetworkServiceConfig::new(
        ...
        diem_channel::Config::new(max_network_channel_size)
            .queue_style(QueueStyle::FIFO)
            .counters(
                &diem_storage_service_server::metrics::PENDING_STORAGE_SERVER_NETWORK_EVENTS,
            ),
    );
    NetworkApplicationConfig::new(network_client_config, network_service_config)
}
...
fn create_network_runtime(network_config: &NetworkConfig) -> Runtime {
    ...
    diem_runtimes::spawn_named_runtime(thread_name, network_config.runtime_threads)
}
