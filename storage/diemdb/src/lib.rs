...
use diem_db_indexer::Indexer;
use diem_infallible::Mutex;
use diem_logger::prelude::*;
use diem_schemadb::{SchemaBatch, DB};
...
static ROCKSDB_PROPERTY_MAP: Lazy<HashMap<&str, String>> = Lazy::new(|| {
    ...
    .iter()
    .map(|x| (*x, format!("diem_{}", x.replace('.', "_"))))
    .collect()
});
...
fn update_rocksdb_properties(ledger_rocksdb: &DB, state_merkle_db: &StateMerkleDb) -> Result<()> {
    ...
    for cf_name in ledger_db_column_families() {
        for (rockdb_property_name, diem_rocksdb_property_name) in &*ROCKSDB_PROPERTY_MAP {
            ROCKSDB_PROPERTIES
                .with_label_values(&[cf_name, diem_rocksdb_property_name])
...
impl DiemDB {
    ...
    fn open_indexer(
        ...
        if indexer.next_version() < ledger_next_version {
            ...
            while next_version < ledger_next_version {
                info!(next_version = next_version, "DiemDB Indexer catching up. ",);
    ...
    pub fn create_checkpoint(
        ...
        info!(
            ...
            "Creating checkpoint for DiemDB."
        );
...
impl DbReader for DiemDB {
    ...
    /// Returns the transaction by version, delegates to `DiemDB::get_transaction_with_proof`.
    ...
    fn get_table_info(&self, handle: TableHandle) -> Result<TableInfo> {
        gauged_api("get_table_info", || {
            self.get_table_info_option(handle)?
                .ok_or_else(|| DiemDbError::NotFound(format!("TableInfo for {:?}", handle)).into())
        })
    }
