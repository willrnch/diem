...
use diem_executor_types::ProofReader;
...
pub(crate) fn update_store(
    ...
    use diem_storage_interface::{jmt_update_refs, jmt_updates};
    let mut root_hash = *diem_crypto::hash::SPARSE_MERKLE_PLACEHOLDER_HASH;
...
pub fn test_save_blocks_impl(
    ...
    let db =
        DiemDB::new_for_test_with_buffered_state_target_items(&tmp_dir, snapshot_size_threshold);
...
pub fn put_transaction_info(db: &DiemDB, version: Version, txn_info: &TransactionInfo) {
...
pub fn put_as_state_root(db: &DiemDB, version: Version, key: StateKey, value: StateValue) {
