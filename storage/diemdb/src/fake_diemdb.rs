...
use diem_accumulator::{HashReader, MerkleAccumulator};
use diem_crypto::{
...
/// Alternate implementation of [DiemDB] for use with consensus-only-perf-test feature.
/// It stores and serves data from in-memory data structures as opposed to [DiemDB],
/// which uses RocksDB. Note that FakeDiemDB re-implements only a subset of the
/// features of [DiemDB] while passing through remaining features to the wrapped inner
/// [DiemDB].
...
impl FakeDiemDB {
    ...
    fn save_and_compute_root_hash(
        ...
        let (root_hash, writes) =
            MerkleAccumulator::<FakeDiemDB, TransactionAccumulatorHasher>::append(
                self,
    ...
    fn get_frozen_subtree_hashes(&self, num_transactions: u64) -> Result<Vec<HashValue>> {
        MerkleAccumulator::<FakeDiemDB, TransactionAccumulatorHasher>::get_frozen_subtree_hashes(
            self,
            num_transactions,
        )
    }
...
impl DbWriter for FakeDiemDB {
    fn get_state_snapshot_receiver(
        &self,
        version: Version,
        expected_root_hash: HashValue,
    ) -> Result<Box<dyn diem_storage_interface::StateSnapshotReceiver<StateKey, StateValue>>> {
        self.inner
...
impl DbReader for FakeDiemDB {
    fn get_epoch_ending_ledger_infos(
        ...
        start_epoch: u64,
        end_epoch: u64,
    ) -> Result<diem_types::epoch_change::EpochChangeProof> {
    ...
    fn get_events(
        &self,
        event_key: &diem_types::event::EventKey,
        start: u64,
        order: diem_storage_interface::Order,
        limit: u64,
        ledger_version: Version,
    ...
    fn get_account_transactions(
        ...
        include_events: bool,
        ledger_version: Version,
    ) -> Result<diem_types::transaction::AccountTransactionsWithProof> {
        self.inner
    ...
    fn get_state_value_by_version(
        ...
        // Since the genesis write set is persisted with DiemDB, we call
        // it to serve state values targetting the framework account
        // (to access DiemCoin, for example).
    ...
    fn get_latest_executed_trees(&self) -> Result<ExecutedTrees> {
        // If the genesis is not executed yet, we need to get the executed trees from the inner DiemDB
        // This is because when we call save_transactions for the genesis block, we call [DiemDB::save_transactions]
        // where there is an expectation that the root of the SMTs are the same pointers. Here,
        // we get from the inner DiemDB which ensures that the pointers match when save_transactions is called.
...
/// requests.
impl HashReader for FakeDiemDB {
...
mod tests {
    ...
    proptest! {
        ...
        fn test_save_transactions(input in arb_blocks_to_commit()) {
            ...
            let db = FakeDiemDB::new(DiemDB::new_for_test(&tmp_dir));
