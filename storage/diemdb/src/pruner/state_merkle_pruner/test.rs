...
use crate::{
    ...
    DiemDB, PrunerManager, StateKvPrunerManager, StateMerklePrunerManager,
};
use diem_config::config::{LedgerPrunerConfig, StateMerklePrunerConfig};
use diem_crypto::HashValue;
use diem_schemadb::{ReadOptions, SchemaBatch};
use diem_storage_interface::{jmt_update_refs, jmt_updates, DbReader};
...
fn test_state_store_pruner() {
    ...
    let diem_db = DiemDB::new_for_test_no_cache(&tmp_dir);
    let state_store = &diem_db.state_store;
    ...
    {
        let pruner =
            create_state_merkle_pruner_manager(&diem_db.state_merkle_db, prune_batch_size);
