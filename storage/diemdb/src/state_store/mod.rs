...
use crate::{
    ...
    DiemDbError, LedgerStore, ShardedStateKvSchemaBatch, StaleNodeIndexCrossEpochSchema,
...
use diem_executor_types::in_memory_state_calculator::InMemoryStateCalculator;
use diem_infallible::Mutex;
use diem_jellyfish_merkle::iterator::JellyfishMerkleIterator;
use diem_logger::info;
use diem_schemadb::{ReadOptions, SchemaBatch};
    ...
    fn get_state_storage_usage(&self, version: Option<Version>) -> Result<StateStorageUsage> {
        ...
        version.map_or(Ok(StateStorageUsage::zero()), |version| {
            Ok(self
                ...
                .ok_or_else(|| DiemDbError::NotFound(format!("VersionData at {}", version)))?
...
impl StateStore {
    ...
    pub fn merklize_value_set(
        ...
        node_hashes: Option<&HashMap<diem_types::nibble::nibble_path::NibblePath, HashValue>>,
        version: Version,
    ...
    // TODO: change to async comment once it does like https://github.com/aptos-labs/diem-core/blob/159b00f3d53e4327523052c1b99dd9889bf13b03/storage/backup/backup-cli/src/backup_types/state_snapshot/restore.rs#L147 or overlap at least two chunks.
    ...
    pub fn get_all_jmt_nodes_referenced(
        &self,
        version: Version,
    ) -> Result<Vec<diem_jellyfish_merkle::node_type::NodeKey>> {
        diem_jellyfish_merkle::JellyfishMerkleTree::new(self.state_merkle_db.as_ref())
            .get_all_nodes_referenced(version)
    }

    #[cfg(test)]
    pub fn get_all_jmt_nodes(&self) -> Result<Vec<diem_jellyfish_merkle::node_type::NodeKey>> {
