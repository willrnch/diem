...
use diem_schemadb::{iterator::SchemaIterator, ReadOptions};
use diem_types::{
...
impl<'a> PrefixedStateValueIterator<'a> {
    pub fn new(
        ...
        // diem/abc|0
        // diem/abd|1
        // if we seek('diem/'), and call next, we may not reach `diem/abd/1` because the prefix extractor we adopted
        // here will stick with prefix `diem/abc` and return `None` or any arbitrary result after visited all the
        // keys starting with `diem/abc`.
