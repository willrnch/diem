...
fn test_various_cmd_parsing() {
    ...
    run_cmd(&["diem-db-tool", "backup", "verify", "--local-fs-dir", "."]);
    run_cmd(&[
        "diem-db-tool",
        "replay-verify",
        "--target-db-dir",
...
mod compaction_tests {
    ...
    fn db_restore_test_setup(
        ...
        use diem_db::utils::iterators::PrefixedStateValueIterator;
        ...
        let (_ledger_db, tree_db, state_kv_db) =
            DiemDB::open_dbs(new_db_dir, RocksdbConfigs::default(), true, 0).unwrap();
