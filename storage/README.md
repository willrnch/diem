...
id: storage
title: Storage
custom_edit_url: https://github.com/aptos-labs/diem-core/edit/main/storage/README.md
---

## Overview

The storage modules implement:
* the DiemDB which holds the authenticated blockchain data structure within a
Diem Node. It serves the current "state" readable by Move contracts being
executed, as well as a configurable length of the blockchain history to fellow
Diem Nodes and the Rest API. It takes in new data from either the consensus or
...
can be critical when emergency happens were an DiemDB needs to be recreated
a. without the help of widely available healthy running Diem Nodes b. to
...
https://github.com/aptos-labs/diem-core/tree/main/documentation/specifications/common
* To know more about the state authentication data structure, see
https://github.com/aptos-labs/diem-core/developer-docs-site/static/papers/jellyfish-merkle-tree/2021-01-14.pdf
* To learn more about the backup system including the backup data format, see
https://github.com/aptos-labs/diem-core/blob/main/documentation/specifications/db_backup/spec.md
...
As part of the Diem Node config, these are specific for the storage components.
...
to be changed. Another reason to not overriding them is the developers of Diem
...
https://github.com/aptos-labs/diem-core/blob/main/config/src/config/storage_config.rs
...
storage: 
  ...
  # `/opt/diem/data/db/ledger_db` and  `/opt/diem/data/db/state_merkle_db`
  dir: db
  # DiemDB persists the state authentication structure off the critical path
  ...
  # DiemDB keeps recent history of the blockchain ledger and recent versions
...
to boot up a DiemDB instance on an empty disk. Use State Sync (it's Fast Sync
mode). Read more about state sync here:
https://github.com/aptos-labs/diem-core/blob/main/state-sync/README.md


### Continuously backing up to a cloud storage

The backup coordinator runs continously, talks to the backup service embedded
inside a Diem Node and writes backup data automatically to a configured cloud
storage.
...
```bash
$ cargo run -p diem-db-tool backup continuously --help
    Finished dev [unoptimized + debuginfo] target(s) in 1.06s
     Running `target/debug/diem-db-tool backup continuously --help`
diem-db-tool-backup-continuously 0.1.0
Run the backup coordinator which backs up blockchain data continuously off a Diem Node.

USAGE:
    diem-db-tool backup continuously [OPTIONS] <--local-fs-dir <LOCAL_FS_DIR>|--command-adapter-config <COMMAND_ADAPTER_CONFIG>>

OPTIONS:
        --backup-service-address <ADDRESS>
            Backup service address. By default a Diem Node runs the backup service serving on tcp
...
Example command:
```
$ cargo run -p diem-db-tool backup continuously \
...
There are other subcommands of the diem-db-tool, all of which are experimental
and can mess up with the backup storage, use only at your own risk.

### Creating an DiemDB with minimal data at the latest epoch ending in a backup

It's part of the Diem API functionality to bootstrap a DiemDB with a backup.
When emergency happens and the need to do the somewhat manual bootstrapping is
high, Diem will provide a backup source in the form of a yaml config file. Otherwise
...
```bash
diem-node-bootstrap-db-from-backup 0.3.5
Tool to bootstrap DB from backup

USAGE:
    diem node bootstrap-db-from-backup [OPTIONS] --config-path <CONFIG_PATH> --target-db-dir <DB_DIR>
...
OPTIONS:
    ...
    -h, --help
        ...
        --target-db-dir <DB_DIR>
            Target dir where the tool recreates a DiemDB with snapshots and transactions provided
            in the backup. The data folder can later be used to start an Diem node. e.g. /opt/
            diem/data/db
...
the "auto" mode of `cargo run -p diem-db-tool restore`, but with more
