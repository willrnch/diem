...
use crate::metrics::{DIEM_JELLYFISH_LEAF_COUNT, DIEM_JELLYFISH_LEAF_DELETION_COUNT};
    ...
    K: Key,
{
    ...
    pub fn batch_put_value_set(
        ...
        let root_hash = if let Some(root_node) = root_node_opt {
            DIEM_JELLYFISH_LEAF_COUNT.set(root_node.leaf_count() as i64);
            let hash = root_node.hash();
            batch.put_node(node_key, root_node);
            hash
        } else {
            DIEM_JELLYFISH_LEAF_COUNT.set(0);
    ...
    K: Key,
{
    ...
    if kvs.len() == 1 && kvs[0].0 == existing_leaf_key {
        if let (key, Some((value_hash, state_key))) = kvs[0] {
            ...
            DIEM_JELLYFISH_LEAF_DELETION_COUNT.inc();
            Ok(None)
        }
    } else {
