...
variable "ecr_base" {
  default = "${AWS_ECR_ACCOUNT_NUM}.dkr.ecr.us-west-2.amazonaws.com/diem"
}
...
target "debian-base" {
  ...
  contexts = {
    debian = "docker-image://debian:bullseye@sha256:1e8185ca8683c04e6c23cc7bb21b564d54faeb7e20852a6068c85cbdd7bac531"
  }
}
...
target "_common" {
  contexts = {
    ...
    node-builder  = "target:diem-node-builder"
...
function "generate_tags" {
  ...
  result = TARGET_REGISTRY == "remote-all" ? [
    ...
    TARGET_REGISTRY == "gcp" || TARGET_REGISTRY == "remote" ? [
      ...
      "diem-core/${target}:${IMAGE_TAG_PREFIX}${GIT_SHA}-from-local",
      "diem-core/${target}:${IMAGE_TAG_PREFIX}from-local",
    ]
  )
}
