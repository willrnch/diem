...
FROM builder-base as diem-node-builder
...
RUN --mount=type=secret,id=GIT_CREDENTIALS,target=/root/.git-credentials \
    ...
    --mount=type=cache,target=/diem/target,id=node-builder-target-cache \
...
RUN --mount=type=secret,id=GIT_CREDENTIALS,target=/root/.git-credentials \
    ...
    --mount=type=cache,target=/diem/target,id=tools-builder-target-cache \
