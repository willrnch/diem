...
use crate::{
    ...
    liveness::{
        ...
        leader_reputation::{
            extract_epoch_to_proposers, DiemDBBackend, LeaderReputation,
...
use diem_config::config::{ConsensusConfig, NodeConfig};
...
use diem_network::{application::interface::NetworkClient, protocols::network::Event};
use diem_safety_rules::SafetyRulesManager;
use diem_types::{
...
pub struct EpochManager {
    ...
    self_sender: diem_channels::Sender<Event<ConsensusMsg>>,
    network_sender: ConsensusNetworkClient<NetworkClient<ConsensusMsg>>,
    timeout_sender: diem_channels::Sender<Round>,
    ...
    block_retrieval_tx:
        Option<diem_channel::Sender<AccountAddress, IncomingBlockRetrievalRequest>>,
    quorum_store_msg_tx: Option<diem_channel::Sender<AccountAddress, VerifiedEvent>>,
    quorum_store_coordinator_tx: Option<Sender<CoordinatorCommand>>,
    quorum_store_storage: Arc<dyn QuorumStoreStorage>,
    batch_retrieval_tx:
        Option<diem_channel::Sender<AccountAddress, IncomingBatchRetrievalRequest>>,
...
impl EpochManager {
    ...
    fn spawn_block_retrieval_task(&mut self, epoch: u64, block_store: Arc<BlockStore>) {
        let (request_tx, mut request_rx) = diem_channel::new(
            QueueStyle::LIFO,
            1,
    ...
    async fn start_recovery_manager(
        ...
        let (recovery_manager_tx, recovery_manager_rx) = diem_channel::new(
            QueueStyle::LIFO,
            1,
    ...
    fn forward_event_to<K: Eq + Hash + Clone, V>(
        mut maybe_tx: Option<diem_channel::Sender<K, V>>,
        key: K,
        value: V,
    ) -> anyhow::Result<()> {
    ...
    pub async fn start(
        mut self,
        mut round_timeout_sender_rx: diem_channels::Receiver<Round>,
