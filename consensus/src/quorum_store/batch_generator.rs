...
use diem_config::config::QuorumStoreConfig;
...
impl BatchGenerator {
    pub(crate) fn new(
            ...
            .expect("Could not read from db")
        {
            ...
            BatchId::new(diem_infallible::duration_since_epoch().as_micros() as u64)
        };
    ...
    pub(crate) async fn handle_scheduled_pull(&mut self, max_count: u64) -> Vec<Batch> {
        ...
        let expiry_time = diem_infallible::duration_since_epoch().as_micros() as u64
