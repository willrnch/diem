...
use diem_config::config::{QuorumStoreConfig, SecureBackend};
use diem_consensus_types::{common::Author, request_response::GetPayloadCommand};
...
impl QuorumStoreBuilder {
    pub fn init_payload_manager(
        ...
        Arc<PayloadManager>,
        Option<diem_channel::Sender<AccountAddress, VerifiedEvent>>,
    ) {
        match self {
    ...
    pub fn start(
        ...
        diem_channel::Sender<AccountAddress, IncomingBatchRetrievalRequest>,
    )> {
        match self {
...
pub struct InnerBuilder {
    ...
    quorum_store_msg_tx: diem_channel::Sender<AccountAddress, VerifiedEvent>,
    quorum_store_msg_rx: Option<diem_channel::Receiver<AccountAddress, VerifiedEvent>>,
...
impl InnerBuilder {
    pub(crate) fn new(
        ...
        let (quorum_store_msg_tx, quorum_store_msg_rx) =
            diem_channel::new::<AccountAddress, VerifiedEvent>(
                QueueStyle::FIFO,
                config.channel_size,
                None,
            );
    ...
    fn spawn_quorum_store(
        ...
        let (batch_retrieval_tx, mut batch_retrieval_rx) =
            diem_channel::new::<AccountAddress, IncomingBatchRetrievalRequest>(
                QueueStyle::LIFO,
                10,
