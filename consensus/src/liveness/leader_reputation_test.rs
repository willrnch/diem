...
use super::leader_reputation::{
    extract_epoch_to_proposers_impl, DiemDBBackend, ProposerAndVoterHeuristic,
};
use crate::liveness::{
    leader_reputation::{
...
use diem_consensus_types::common::{Author, Round};
use diem_crypto::{bls12381, HashValue};
use diem_infallible::Mutex;
use diem_keygen::KeyGen;
use diem_storage_interface::{DbReader, Order};
use diem_types::{
...
impl Example1 {
    fn new(window_size: usize) -> Self {
        ...
        let diem_db = Arc::new(MockDbReader::new());
        let backend = DiemDBBackend::new(window_size, 0, diem_db.clone());
...
fn test_api(use_root_hash: bool) {
    ...
    for epoch in 1..1000 {
        ...
        assert_eq!(
            (epoch, 1),
            diem_db.add_event_with_data(proposers[0], vec![1, 2], vec![])
        );
        assert_eq!(
            (epoch, 2),
            diem_db.add_event_with_data(proposers[0], vec![3], vec![])
        );
        let backend = Box::new(DiemDBBackend::new(1, 4, diem_db.clone()));
        ...
        let state = if use_root_hash {
            [
                diem_db.get_accumulator_root_hash(0).unwrap().to_vec(),
...
fn backend_wrapper_test() {
    let diem_db = Arc::new(MockDbReader::new());
    let backend = DiemDBBackend::new(3, 3, diem_db.clone());

    diem_db.add_event(0, 1);
    diem_db.new_epoch();
    diem_db.skip_rounds(1);
    for i in 2..6 {
        diem_db.add_event(1, i);
    }
    ...
    let mut assert_history = |round, expected_history: Vec<Round>, to_fetch| {
        ...
        assert_eq!(fetch_count, diem_db.fetched(), "At round {}", round);
    };
    ...
    diem_db.add_another_transaction();
    assert_history(12, vec![12, 11, 10], false);
    ...
    diem_db.add_another_transaction();
    diem_db.add_event_after_call(1, 13);
...
fn backend_test_cross_epoch() {
    ...
    diem_db.add_event(0, 1);
    diem_db.new_epoch();
    diem_db.add_event(1, 1);
    diem_db.add_event(1, 2);
    diem_db.add_event(1, 3);
    diem_db.new_epoch();
    diem_db.add_event(2, 1);
    diem_db.add_event(2, 2);
