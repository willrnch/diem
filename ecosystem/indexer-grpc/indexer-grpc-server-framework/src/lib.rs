...
where
    C: RunnableConfig,
{
    let runtime = diem_runtimes::spawn_named_runtime(config.get_server_name(), None);
...
fn handle_panic(panic_info: &PanicInfo<'_>) {
    ...
    // This is a workaround until https://github.com/aptos-labs/diem-core/issues/2038 is resolved.
    eprintln!("{}", crash_info);
    // Kill the process
    process::exit(12);
}
...
async fn register_probes_and_metrics_handler(port: u16) {
    ...
    let metrics_endpoint = warp::path("metrics").map(|| {
        // Metrics encoding.
        let metrics = diem_metrics_core::gather();
