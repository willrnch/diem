...
use anyhow::Result;
use diem_indexer_grpc_data_service::service::RawDataServerWrapper;
use diem_indexer_grpc_server_framework::{RunnableConfig, ServerArgs};
use diem_indexer_grpc_utils::config::IndexerGrpcFileStoreConfig;
use diem_protos::{
    ...
    async fn run(&self) -> Result<()> {
        ...
        let authentication_inceptor =
            move |req: Request<()>| -> std::result::Result<Request<()>, Status> {
                ...
                if let Some(token) =
                    metadata.get(diem_indexer_grpc_utils::constants::GRPC_AUTH_TOKEN_HEADER)
                {
        ...
        let svc = diem_protos::indexer::v1::raw_data_server::RawDataServer::new(server)
