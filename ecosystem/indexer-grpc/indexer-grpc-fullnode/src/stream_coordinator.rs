...
use diem_api_types::{AsConverter, Transaction as APITransaction, TransactionOnChainData};
use diem_logger::{error, info, sample, sample::SampleRate};
use diem_protos::{
...
impl IndexerStreamCoordinator {
    ...
    async fn convert_to_api_txns(
        ...
        let mut epoch_bcs = diem_api_types::U64::from(epoch);
        let mut block_height = block_event.height();
        let mut block_height_bcs = diem_api_types::U64::from(block_height);
        ...
        for (ind, raw_txn) in raw_txns.into_iter().enumerate() {
            ...
            if ind > 0 {
                ...
                if let Some(txn) = raw_txn.transaction.try_as_block_metadata() {
                    ...
                    epoch = txn.epoch();
                    epoch_bcs = diem_api_types::U64::from(epoch);
                    block_height += 1;
                    block_height_bcs = diem_api_types::U64::from(block_height);
                }
            }
            match converter
...
fn record_fetched_transaction_latency(txn: &diem_api_types::Transaction) {
