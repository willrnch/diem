...
use diem_moving_average::MovingAverage;
use diem_protos::internal::fullnode::v1::{
...
pub fn bootstrap(
    ...
    let runtime = diem_runtimes::spawn_named_runtime("indexer-grpc".to_string(), None);
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                match tx.send(Result::<_, Status>::Ok(batch_end_status)).await {
                    ...
                    Err(_) => {
                        diem_logger::warn!("[indexer-grpc] Unable to send end batch status");
                        break;
                    },
                }
