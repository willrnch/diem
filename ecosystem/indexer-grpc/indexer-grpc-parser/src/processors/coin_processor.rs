...
use anyhow::bail;
use diem_protos::transaction::v1::Transaction;
...
const DIEM_COIN_TYPE_STR: &str = "0x1::diem_coin::DiemCoin";
...
impl ProcessorTrait for CoinTransactionProcessor {
    ...
    async fn process_transactions(
        ...
        // get diem_coin info for supply tracking
        // TODO: This only needs to be fetched once. Need to persist somehow
        let maybe_diem_coin_info =
            &CoinInfoQuery::get_by_coin_type(DIEM_COIN_TYPE_STR.to_string(), &mut conn).unwrap();
        ...
        for txn in &transactions {
            let (
                ...
                current_coin_balances,
                mut coin_supply,
            ) = CoinActivity::from_transaction(txn, maybe_diem_coin_info);
