# Generated TS client for Diem Node Health Checker
...
## Quickstart
```bash
pnpm add diem-node-checker-client
```
...
```
cargo run -p diem-node-checker -- server run --baseline-node-config-paths ecosystem/node-checker/configuration_examples/local_testnet.yaml --listen-address 0.0.0.0
```
...
[npm-url]: https://npmjs.org/package/diem-node-checker-client
