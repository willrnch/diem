...
from diem_sdk.account import Account
from diem_sdk.async_client import FaucetClient, RestClient
...
async def main():
    ...
    # :!:>section_4
    txn_hash = await rest_client.create_collection(
        alice, collection_name, "Alice's simple collection", "https://diem.dev"
    )  # <:!:section_4
    await rest_client.wait_for_transaction(txn_hash)

    # :!:>section_5
    txn_hash = await rest_client.create_token(
        alice,
        collection_name,
        token_name,
    ...
    # :!:>section_9
    txn_hash = await rest_client.offer_token(
        alice,
        bob.address(),
        alice.address(),
        collection_name,
    ...
    # :!:>section_10
    txn_hash = await rest_client.claim_token(
        bob,
        alice.address(),
        alice.address(),
        collection_name,
    ...
    alice_balance = rest_client.get_token_balance(
        alice.address(), alice.address(), collection_name, token_name, property_version
    )
    bob_balance = rest_client.get_token_balance(
    ...
    # :!:>section_11
    txn_hash = await rest_client.direct_transfer_token(
