...
import { DiemApiError, DiemRequest, DiemResponse, ClientConfig } from "./types";
import "./cookieJar";
...
async function axiosRequest<Request, Response>(
  ...
  const headers: Record<string, string | number | boolean> = {
    ...overrides?.HEADERS,
    "x-diem-client": `diem-ts-sdk/${VERSION}`,
 ...
 * Wraps axios error response with DiemApiError
 *
 * @param options DiemRequest
 * @returns the response or DiemApiError
 */
export async function diemRequest<Req, Res>(options: DiemRequest): Promise<DiemResponse<Req, Res>> {
  ...
  const result: DiemResponse<Req, Res> = {
  ...
  throw new DiemApiError(options, result, errorMessage ?? "Generic Error");
}
