import { DiemClient, Provider, OptionalTransactionArgs, ApiError } from "../providers";
...
export class AnsClient {
  ...
  /**
   * Mint a new Diem name
   *
   * @param account DiemAccount where collection will be created
   * @param domainName Diem domain name to mint
  ...
  async mintDiemName(
    ...
    const builder = new TransactionBuilderRemoteABI(this.provider.diemClient, {
  ...
  /**
   * Mint a new Diem Subdomain
   *
   * @param account DiemAccount the owner of the domain name
   * @param subdomainName subdomain name to mint
   * @param domainName Diem domain name to mint under
   * @param expirationTimestampSeconds must be set between the domains expiration and the current time
   * @returns The hash of the pending transaction submitted to the API
   */
  async mintDiemSubdomain(
    account: DiemAccount,
   ...
   *
   * @param owner the `diem_names` DiemAccount
   * @returns The hash of the pending transaction submitted to the API
   */
  async initReverseLookupRegistry(owner: DiemAccount, extraArgs?: OptionalTransactionArgs): Promise<Gen.HashValue> {
    const builder = new TransactionBuilderRemoteABI(this.provider.diemClient, {
