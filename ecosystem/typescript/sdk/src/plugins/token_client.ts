...
import * as TokenTypes from "../diem_types/token_types";
...
import { Token, TokenData } from "../diem_types/token_types";
...
export class TokenClient {
   ...
   *
   * @param account DiemAccount where collection will be created
  ...
  async createCollection(
    ...
    const builder = new TransactionBuilderRemoteABI(this.diemClient, { sender: account.address(), ...extraArgs });
    ...
    const bcsTxn = DiemClient.generateBCSTransaction(account, rawTxn);
   ...
   *
   * @param account DiemAccount where token will be created
   ...
   *
   * @param account DiemAccount where token from which tokens will be transfered
   * @param receiver  Hex-encoded 32 byte Diem account address to which tokens will be transfered
   * @param creator Hex-encoded 32 byte Diem account address to which created tokens
   ...
   *
   * @param account DiemAccount which will claim token
   * @param sender Hex-encoded 32 byte Diem account address which holds a token
   * @param creator Hex-encoded 32 byte Diem account address which created a token
   * @param collectionName Name of collection where token is stored
   * @param name Token name
   * @param property_version the version of token PropertyMap with a default value 0.
   * @returns The hash of the transaction submitted to the API
   ...
   *
   * @param account DiemAccount which will remove token from pending list
   * @param receiver Hex-encoded 32 byte Diem account address which had to claim token
   * @param creator Hex-encoded 32 byte Diem account address which created a token
   ...
   *
   * @param sender DiemAccount where token from which tokens will be transfered
   * @param receiver Hex-encoded 32 byte Diem account address to which tokens will be transfered
   * @param creator Hex-encoded 32 byte Diem account address to which created tokens
   * @param collectionName Name of collection where token is stored
   * @param name Token name
  ...
  async directTransferToken(
    ...
    const builder = new TransactionBuilderRemoteABI(this.diemClient, { sender: sender.address(), ...extraArgs });
    const rawTxn = await builder.build(
    ...
    const transactionRes = await this.diemClient.submitSignedBCSTransaction(bcsTxn);
   ...
   *
   * @param sender DiemAccount where the token will be transferred
   * @param optIn boolean value indicates user want to opt-in or out of direct transfer
   * @returns The hash of the transaction submitted to the API
   */
  async optInTokenTransfer(sender: DiemAccount, optIn: boolean, extraArgs?: OptionalTransactionArgs): Promise<string> {
    const builder = new TransactionBuilderRemoteABI(this.diemClient, { sender: sender.address(), ...extraArgs });
    const rawTxn = await builder.build("0x3::token::opt_in_direct_transfer", [], [optIn]);
    const bcsTxn = DiemClient.generateBCSTransaction(sender, rawTxn);
    const pendingTransaction = await this.diemClient.submitSignedBCSTransaction(bcsTxn);
    return pendingTransaction.hash;
  }
  ...
  async burnByCreator(
    ...
    const builder = new TransactionBuilderRemoteABI(this.diemClient, { sender: creator.address(), ...extraArgs });
    const rawTxn = await builder.build(
    ...
    const bcsTxn = DiemClient.generateBCSTransaction(creator, rawTxn);
    const pendingTransaction = await this.diemClient.submitSignedBCSTransaction(bcsTxn);
    return pendingTransaction.hash;
  }
  ...
  async burnByOwner(
    ...
    const builder = new TransactionBuilderRemoteABI(this.diemClient, { sender: owner.address(), ...extraArgs });
    const rawTxn = await builder.build(
      "0x3::token::burn",
      [],
    ...
    const bcsTxn = DiemClient.generateBCSTransaction(owner, rawTxn);
    const pendingTransaction = await this.diemClient.submitSignedBCSTransaction(bcsTxn);
    return pendingTransaction.hash;
  }
   ...
   *
   * @param account DiemAccount who modifies the token properties
   ...
   * @param creator Hex-encoded 32 byte Diem account address which created a collection
  ...
  async getCollectionData(creator: MaybeHexString, collectionName: string): Promise<any> {
    const resources = await this.diemClient.getAccountResources(creator);
    ...
    const collectionTable = await this.diemClient.getTableItem(handle, getCollectionTableItemRequest);
  ...
  async getTokenData(
    ...
    const collection: { type: Gen.MoveStructTag; data: any } = await this.diemClient.getAccountResource(
      creatorHex,
    ...
    // implicit cast.
    const rawTokenData = await this.diemClient.getTableItem(handle, getTokenTableItemRequest);
   ...
   * @param account Hex-encoded 32 byte Diem account address which created a token
  ...
  async getTokenForAccount(account: MaybeHexString, tokenId: TokenTypes.TokenId): Promise<TokenTypes.Token> {
    const tokenStore: { type: Gen.MoveStructTag; data: any } = await this.diemClient.getAccountResource(
    ...
    try {
      const rawToken = await this.diemClient.getTableItem(handle, getTokenTableItemRequest);
