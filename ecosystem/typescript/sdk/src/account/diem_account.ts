...
import { AccountAddress, AuthenticationKey, Ed25519PublicKey } from "../diem_types";
import { bcsToBytes } from "../bcs";

export interface DiemAccountObject {
...
/**
 * Class for creating and managing Diem account
 */
export class DiemAccount {
  /**
  ...
  static fromDiemAccountObject(obj: DiemAccountObject): DiemAccount {
    return new DiemAccount(HexString.ensure(obj.privateKeyHex).toUint8Array(), obj.address);
  }
  ...
  /**
   * This is the key by which Diem account is referenced.
   ...
   * See here for more info: {@link https://diem.dev/concepts/accounts#single-signer-authentication}
   ...
   * @returns DiemAccountObject instance.
   * @example An example of the returned DiemAccountObject object
   * ```
   * {
   ...
   * ```
   */
  toPrivateKeyObject(): DiemAccountObject {
    return {
...
// Returns an account address as a HexString given either an DiemAccount or a MaybeHexString.
export function getAddressFromAccountOrAddress(accountOrAddress: DiemAccount | MaybeHexString): HexString {
  return accountOrAddress instanceof DiemAccount ? accountOrAddress.address() : HexString.ensure(accountOrAddress);
}
