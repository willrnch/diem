...
describe("ANS", () => {
  ...
  test(
    ...
    async () => {
      const owner = new DiemAccount(new HexString(ANS_OWNER_PK).toUint8Array());
  ...
  test(
    ...
    async () => {
      ...
      const txnHash = await ans.mintDiemName(alice, DOMAIN_NAME);
      await provider.waitForTransactionWithResult(txnHash, { checkSuccess: true });
    },
    longTestTimeout,
  );
  ...
  test(
    ...
    async () => {
      ...
      const txnHash = await ans.mintDiemSubdomain(alice, SUBDOMAIN_NAME, DOMAIN_NAME);
      await provider.waitForTransactionWithResult(txnHash, { checkSuccess: true });
