...
describe("Provider", () => {
  it("uses provided network as API", async () => {
    ...
    expect(provider.diemClient.nodeUrl).toBe(NetworkToNodeAPI[Network.DEVNET]);
  ...
  it("uses custom endpoints as API", async () => {
    ...
    expect(provider.diemClient.nodeUrl).toBe("full-node-url/v1");
  ...
  it("has DiemClient method defined", () => {
    const provider = new Provider(Network.DEVNET);
