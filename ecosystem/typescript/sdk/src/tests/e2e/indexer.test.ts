...
const faucetClient = new FaucetClient("https://fullnode.devnet.diemlabs.com", "https://faucet.devnet.diemlabs.com", {
  TOKEN: FAUCET_AUTH_TOKEN,
});
const tokenClient = new TokenClient(provider.diemClient);
...
describe("Indexer", () => {
  ...
  beforeAll(async () => {
    ...
    if (!skipTest) {
      ...
      await provider.waitForTransaction(
        await diemToken.createCollection(
          alice,
  ...
  runTests("get data", () => {
    ...
    it(
      ...
      async () => {
        ...
        expect(accountCoinData.current_coin_balances[0].coin_type).toEqual("0x1::diem_coin::DiemCoin");
      },
      longTestTimeout,
    );
