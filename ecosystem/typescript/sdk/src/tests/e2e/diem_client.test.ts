...
import { DiemToken, TokenClient } from "../../plugins";
...
import { AccountAddress, Ed25519PublicKey, stringStructTag, TypeTagStruct } from "../../diem_types";
...
test("gets account modules", async () => {
  ...
  const module = modules.find((r) => r.abi!.name === "diem_coin");
  expect(module!.abi!.address).toBe("0x1");
});

test("gets the DiemCoin module", async () => {
  const client = new DiemClient(NODE_URL);
  const module = await client.getAccountModule("0x1", "diem_coin");
  expect(module!.abi!.address).toBe("0x1");
});
...
test(
  ...
  async () => {
    ...
    await provider.waitForTransaction(
      await diemToken.createCollection(account1, "Collection description", "Collection Name", "https://diem.dev", 5, {
        royaltyNumerator: 10,
...
test(
  ...
  async () => {
    ...
    const account1 = new DiemAccount();
    const account2 = new DiemAccount();
    const account3 = new DiemAccount();
    ...
    const account4 = new DiemAccount();
...
test(
  ...
  async () => {
    ...
    const checkDiemCoin = async () => {
      const resources1 = await client.getAccountResources(account1.address());
      const account1Resource = resources1.find((r) => r.type === diemCoin);
    ...
    const payload: Gen.TransactionPayload = {
      ...
      function: "0x1::diem_account::transfer",
      type_arguments: [],
    ...
    [account1, new Ed25519PublicKey(account1.pubKey().toUint8Array())].forEach(async (accountOrAddress) => {
      ...
      const account2DiemCoin = transactionRes.changes.filter((change) => {
        ...
        return (
          ...
          write.data.type === diemCoin &&
          (write.data.data as { coin: { value: string } }).coin.value === "100000"
        );
      });
      expect(account2DiemCoin).toHaveLength(1);
    });
    await checkDiemCoin();
  },
  longTestTimeout,
...
test(
  ...
  async () => {
    ...
    const checkDiemCoin = async () => {
      ...
      const senderResource = resources1.find((r) => r.type === diemCoin);
    ...
    const bcsTxn = DiemClient.generateBCSSimulation(sender, rawTxn);
    const transactionRes = (await client.submitBCSSimulation(bcsTxn))[0];
    expect(parseInt(transactionRes.gas_used, 10) > 0);
    expect(transactionRes.success);
    const receiverDiemCoin = transactionRes.changes.filter((change) => {
      if (change.type !== "write_resource") {
        return false;
      }
      const write = change as Gen.WriteResource;
    ...
    expect(receiverDiemCoin).toHaveLength(1);
    await checkDiemCoin();
  },
  longTestTimeout,
);
...
test(
  ...
  async () => {
    ...
    await ensureTxnSuccess(
      tokenClient.createCollection(alice, collectionName, "Alice's simple collection", "https://diem.dev"),
    );
...
test(
  ...
  async () => {
    ...
    const getBalance = async (account: DiemAccount) => {
...
test(
  ...
  async () => {
    ...
    const helperAccount = new DiemAccount();
...
test(
  ...
  async () => {
    ...
    await client.waitForTransaction(
      await tokenClient.createCollection(alice, collectionName, "Alice's simple collection", "https://diem.dev"),
      { checkSuccess: true },
    );
