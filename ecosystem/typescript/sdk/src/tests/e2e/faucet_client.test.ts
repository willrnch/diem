...
import { DiemClient } from "../../providers";
...
test(
  ...
  async () => {
    ...
    const events = await client.getEventsByEventHandle(tx.sender, diemCoin, "withdraw_events");
    expect(events[0].type).toBe("0x1::coin::WithdrawEvent");

    const eventSubset = await client.getEventsByEventHandle(tx.sender, diemCoin, "withdraw_events", {
      start: BigInt(0),
      limit: 1,
    });
