...
describe("BuilderUtils", () => {
  ...
  it("parses a sturct TypeTag", async () => {
    ...
    const diemCoin = new TypeTagParser(
      "0x1::coin::CoinStore < 0x1::test_coin::DiemCoin1 ,  0x1::test_coin::DiemCoin2 > ",
    ).parseTypeTag();
    expect(diemCoin instanceof TypeTagStruct).toBeTruthy();
    assertStruct(diemCoin as TypeTagStruct, "0x1", "coin", "CoinStore");

    const diemCoinTrailingComma = new TypeTagParser(
      "0x1::coin::CoinStore < 0x1::test_coin::DiemCoin1 ,  0x1::test_coin::DiemCoin2, > ",
    ).parseTypeTag();
    expect(diemCoinTrailingComma instanceof TypeTagStruct).toBeTruthy();
    assertStruct(diemCoinTrailingComma as TypeTagStruct, "0x1", "coin", "CoinStore");

    const structTypeTags = (diemCoin as TypeTagStruct).value.type_args;
    expect(structTypeTags.length).toBe(2);

    const structTypeTag1 = structTypeTags[0];
    assertStruct(structTypeTag1 as TypeTagStruct, "0x1", "test_coin", "DiemCoin1");

    const structTypeTag2 = structTypeTags[1];
    assertStruct(structTypeTag2 as TypeTagStruct, "0x1", "test_coin", "DiemCoin2");

    const coinComplex = new TypeTagParser(
      // eslint-disable-next-line max-len
      "0x1::coin::CoinStore < 0x2::coin::LPCoin < 0x1::test_coin::DiemCoin1 <u8>, vector<0x1::test_coin::DiemCoin2 > > >",
    ).parseTypeTag();
    ...
    expect(() => {
      new TypeTagParser("0x1::test_coin::CoinStore<0x1::test_coin::DiemCoin").parseTypeTag();
    }).toThrow("Invalid type tag.");
    ...
    expect(() => {
      new TypeTagParser("0x1:test_coin::DiemCoin").parseTypeTag();
    }).toThrow("Unrecognized token.");

    expect(() => {
      new TypeTagParser("0x!::test_coin::DiemCoin").parseTypeTag();
    }).toThrow("Unrecognized token.");
    ...
    expect(() => {
      new TypeTagParser("0x1::<::CoinStore<0x1::test_coin::DiemCoin,").parseTypeTag();
    }).toThrow("Invalid type tag.");

    expect(() => {
      new TypeTagParser("0x1::test_coin::><0x1::test_coin::DiemCoin,").parseTypeTag();
    }).toThrow("Invalid type tag.");
