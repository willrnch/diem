...
import { DiemAccount, DiemAccountObject, getAddressFromAccountOrAddress } from "../../account";
import { HexString } from "../../utils";
import nacl from "tweetnacl";

const diemAccountObject: DiemAccountObject = {
...
test("generates random accounts", () => {
  const a1 = new DiemAccount();
  const a2 = new DiemAccount();
...
test("generates derive path accounts", () => {
  ...
  const a1 = DiemAccount.fromDerivePath("m/44'/637'/0'/0'/0'", mnemonic);
  expect(a1.address().hex()).toBe(address);
});

test("generates derive path accounts", () => {
  expect(() => {
    DiemAccount.fromDerivePath("", mnemonic);
  }).toThrow(new Error("Invalid derivation path"));
});

test("accepts custom address", () => {
  const address = "0x777";
  const a1 = new DiemAccount(undefined, address);
  expect(a1.address().hex()).toBe(address);
});

test("Deserializes from DiemAccountObject", () => {
  const a1 = DiemAccount.fromDiemAccountObject(diemAccountObject);
  expect(a1.address().hex()).toBe(diemAccountObject.address);
  expect(a1.pubKey().hex()).toBe(diemAccountObject.publicKeyHex);
});

test("Deserializes from DiemAccountObject without address", () => {
  const privateKeyObject = { privateKeyHex: diemAccountObject.privateKeyHex };
  const a1 = DiemAccount.fromDiemAccountObject(privateKeyObject);
  expect(a1.address().hex()).toBe(diemAccountObject.address);
  expect(a1.pubKey().hex()).toBe(diemAccountObject.publicKeyHex);
});
...
test("Serializes/Deserializes", () => {
  const a1 = new DiemAccount();
  const a2 = DiemAccount.fromDiemAccountObject(a1.toPrivateKeyObject());
...
test("Gets the resource account address", () => {
  ...
  expect(DiemAccount.getResourceAccountAddress(sourceAddress, seed).hex()).toBe(
    "0xcbed05b37b6981a57f535c1f5d136734df822abaf4cd30c51c9b4d60eae79d5d",
  );
});

test("Test getAddressFromAccountOrAddress", () => {
  const account = DiemAccount.fromDiemAccountObject(diemAccountObject);
  expect(getAddressFromAccountOrAddress(diemAccountObject.address!).toString()).toBe(diemAccountObject.address);
  expect(getAddressFromAccountOrAddress(HexString.ensure(diemAccountObject.address!)).toString()).toBe(
    diemAccountObject.address,
  );
  expect(getAddressFromAccountOrAddress(account).toString()).toBe(diemAccountObject.address);
});
...
test("Gets the collection id", async () => {
  ...
  expect(DiemAccount.getCollectionID(creatorAddress, collectionName).hex()).toBe(
