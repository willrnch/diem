...
import { ArgumentABI, EntryFunctionABI, ScriptABI, TransactionScriptABI, TypeArgumentABI } from "../diem_types/abi";
...
export { TypeTagParser } from "../diem_types";
...
export class TransactionBuilderABI {
   ...
   * @param func Fully qualified func names, e.g. 0x1::diem_account::transfer
...
export class TransactionBuilderRemoteABI {
  // We don't want the builder to depend on the actual DiemClient. There might be circular dependencies.
  constructor(
    private readonly diemClient: DiemClientInterface,
  ...
  async fetchABI(addr: string) {
    const modules = await this.diemClient.getAccountModules(addr);
    const abis = modules
  ...
  async build(func: Gen.EntryFunctionId, ty_tags: Gen.MoveType[], args: any[]): Promise<RawTransaction> {
    ...
    const [{ sequence_number: sequenceNumber }, chainId, { gas_estimate: gasUnitPrice }] = await Promise.all([
      rest?.sequenceNumber
        ...
        : this.diemClient.getAccount(senderAddress),
      rest?.chainId ? Promise.resolve(rest?.chainId) : this.diemClient.getChainId(),
      rest?.gasUnitPrice ? Promise.resolve({ gas_estimate: rest?.gasUnitPrice }) : this.diemClient.estimateGasPrice(),
    ]);
