...
import { Ed25519PublicKey, MultiEd25519PublicKey, RawTransaction } from "../diem_types";
import { get, post, ClientConfig, DiemApiError } from "../client";
...
/**
 * Provides methods for retrieving data from Diem node.
 * For more detailed API specification see {@link https://fullnode.devnet.diemlabs.com/v1/spec}
 */
export class DiemClient {
  readonly nodeUrl: string;

  readonly config: ClientConfig | undefined;

  /**
   * Build a client configured to connect to an Diem node at the given URL.
   *
   ...
   *
   * @param nodeUrl URL of the Diem Node API endpoint.
  ...
  /**
   * Queries an Diem account by address
   * @param accountAddress Hex-encoded 32 byte Diem account address
  ...
  static generateBCSTransaction(accountFrom: DiemAccount, rawTxn: TxnBuilderTypes.RawTransaction): Uint8Array {
   ...
   */
  static generateBCSSimulation(accountFrom: DiemAccount, rawTxn: TxnBuilderTypes.RawTransaction): Uint8Array {
   ...
   * @param sender Hex-encoded 32 byte Diem account address of transaction sender
   ...
   * @param accountFrom DiemAccount of transaction sender
  ...
  async signTransaction(
    accountFrom: DiemAccount,
    rawTransaction: TxnBuilderTypes.RawTransaction,
  ): Promise<Uint8Array> {
    return Promise.resolve(DiemClient.generateBCSTransaction(accountFrom, rawTransaction));
  }
   ...
   * that event type.
   * @param address Hex-encoded 32 byte Diem account, with or without a `0x` prefix,
   ...
   * (e.g. `0x1::coin::CoinStore<0x1::diem_coin::DiemCoin>`)
   ...
   *
   * @param accountOrPubkey The sender or sender's public key. When private key is available, `DiemAccount` instance
  ...
  async simulateTransaction(
    accountOrPubkey: DiemAccount | Ed25519PublicKey | MultiEd25519PublicKey,
    rawTransaction: TxnBuilderTypes.RawTransaction,
    query?: {
    ...
    if (accountOrPubkey instanceof DiemAccount) {
      signedTxn = DiemClient.generateBCSSimulation(accountOrPubkey, rawTransaction);
   ...
   *
   * 1. Create hash message bytes: "Diem::Transaction" bytes + BCS bytes of Transaction.
   ...
   * const bcsTxn = DiemClient.generateBCSTransaction(sender, rawTransaction);
   * const pendingTransaction = await this.submitSignedBCSTransaction(bcsTxn);
   * const transasction = await this.diemClient.waitForTransactionWithResult(pendingTransaction.hash);
   * ```
   */
   ...
   *
   * @param sender DiemAccount of transaction sender.
   ...
   * @returns The transaction response from the API.
   */
  async signAndSubmitTransaction(sender: DiemAccount, transaction: RawTransaction): Promise<string> {
    const bcsTxn = DiemClient.generateBCSTransaction(sender, transaction);
    const pendingTransaction = await this.submitSignedBCSTransaction(bcsTxn);
    return pendingTransaction.hash;
  }
   ...
   * `diem move compile --save-metadata [ --included-artifacts=<...> ]`.
   * @param sender
  ...
  async estimateMaxGasAmount(forAccount: MaybeHexString): Promise<Uint64> {
    // Only Diem utility coin is accepted as gas
    const typeTag = `0x1::coin::CoinStore<${DIEM_COIN}>`;
   ...
   * the account.
   * WARNING: You must create a new instance of DiemAccount after using this function.
  ...
  async rotateAuthKeyEd25519(
    ...
    const helperAccount = new DiemAccount(toPrivateKeyBytes);
    ...
    const bcsTxn = DiemClient.generateBCSTransaction(forAccount, rawTransaction);
