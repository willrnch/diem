...
/**
 * TS SDK supports ANS. Since ANS contract is not part of diem-framework
 * we need to get the ANS contract, publish it to local testnet and test against it.
 * This script clones the diem-names-contracts repo {@link https://github.com/aptos-labs/diem-names-contracts},
...
const DIEM_NODE_URL = process.env.DIEM_NODE_URL;
const DIEM_FAUCET_URL = process.env.DIEM_FAUCET_URL;
...
try {
  ...
  execSync(`git clone https://github.com/aptos-labs/diem-names-contracts.git ${tempDir}`, {
    stdout: "inherit",
  });
  ...
  if (DOCKER_IMAGE) {
    ...
    cliInvocation = `docker run --network host --mount=type=bind,source=${tempDir},target=/code ${DOCKER_IMAGE} diem`;
    repoDir = "/code";
  } else {
  ...
  execSync(
    `${cliInvocation} account fund-with-faucet --account ${ANS_TEST_ACCOUNT_ADDRESS} --faucet-url ${DIEM_FAUCET_URL} --url ${DIEM_NODE_URL}`,
    { stdio: "inherit" },
  );
  ...
  execSync(
    `${cliInvocation} move publish --package-dir ${repoDir}/core --assume-yes --private-key=${ANS_TEST_ACCOUNT_PRIVATE_KEY} --named-addresses diem_names=${ANS_TEST_ACCOUNT_ADDRESS},diem_names_admin=${ANS_TEST_ACCOUNT_ADDRESS},diem_names_funds=${ANS_TEST_ACCOUNT_ADDRESS} --url=${DIEM_NODE_URL}`,
    { stdio: "inherit" },
  );
