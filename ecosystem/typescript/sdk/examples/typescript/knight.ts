...
import { DiemAccount, HexString, Provider, Network, Types, FaucetClient, BCS } from "diem";
...
class KnightClient {
  async mintCorn(creator: DiemAccount, receiver: HexString, amount: BCS.AnyNumber): Promise<string> {
  ...
  async mintMeat(creator: DiemAccount, receiver: HexString, amount: BCS.AnyNumber): Promise<string> {
    const rawTxn = await provider.generateTransaction(creator.address(), {
  ...
  async feedCorn(moduleAddr: HexString, creator: DiemAccount, to: HexString, amount: BCS.AnyNumber): Promise<string> {
    const rawTxn = await provider.generateTransaction(creator.address(), {
  ...
  async feedMeat(moduleAddr: HexString, creator: DiemAccount, to: HexString, amount: BCS.AnyNumber): Promise<string> {
    const rawTxn = await provider.generateTransaction(creator.address(), {
...
async function main(): Promise<void> {
  ...
  const admin = new DiemAccount();
  const user = new DiemAccount();
  await faucetClient.fundAccount(admin.address(), 100_000_000);
  await faucetClient.fundAccount(user.address(), 100_000_000);
  console.log(
    "\nCompile and publish the knight module (`diem-core/diem-move/move-examples/token_objects/knight`) using the following profile, and press enter:",
  );
  ...
  let txnHash = await client.mintKnightToken(
    ...
    tokenName,
    "https://raw.githubusercontent.com/aptos-labs/diem-core/main/ecosystem/typescript/sdk/examples/typescript/metadata/knight/",
    userAddr,
  );
  await provider.waitForTransaction(txnHash, { checkSuccess: true });
  console.log("\n=== Knight Token Minted ===");
  console.log(`Txn: https://explorer.diemlabs.com/txn/${txnHash}?network=devnet`);
