...
import { diemCoinStore, FAUCET_URL, NODE_URL } from "./common";
...
(async () => {
  ...
  const owner1 = new DiemAccount();
  const owner2 = new DiemAccount();
  const owner3 = new DiemAccount();
  ...
  // ===========================================================================================
  const recipient = new DiemAccount();
  ...
  let accountResource = await client.getAccountResource(recipient.address(), diemCoinStore);
  ...
  accountResource = await client.getAccountResource(recipient.address(), diemCoinStore);
...
const getNumberOfOwners = async (client: DiemClient, multisigAddress: string): Promise<number> => {
...
const getSignatureThreshold = async (client: DiemClient, multisigAddress: string): Promise<number> => {
  const multisigAccountResource = await client.getAccountResource(
    multisigAddress,
    "0x1::multisig_account::MultisigAccount",
  );
