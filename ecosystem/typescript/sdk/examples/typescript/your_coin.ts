...
import { DiemAccount, DiemClient, TxnBuilderTypes, MaybeHexString, HexString, FaucetClient } from "diem";
/**
  ...
  One method to do so is to use the CLI:
      * Acquire the Diem CLI, see https://diem.dev/cli-tools/diem-cli-tool/install-diem-cli
      * `pnpm your_coin ~/diem-core/diem-move/move-examples/moon_coin`.
      * Open another terminal and `diem move compile --package-dir ~/diem-core/diem-move/move-examples/moon_coin --save-metadata --named-addresses MoonCoin=<Alice address from above step>`.
...
class CoinClient extends DiemClient {
  constructor() {
    super(NODE_URL);
  }

  /** Register the receiver account to receive transfers for the new coin. */
  async registerCoin(coinTypeAddress: HexString, coinReceiver: DiemAccount): Promise<string> {
  ...
  async transferCoin(sender: DiemAccount, receiverAddress: HexString, amount: number | bigint): Promise<string> {
    const rawTxn = await this.generateTransaction(sender.address(), {
      function: "0x1::diem_account::transfer_coins",
  ...
  /** Mints the newly created coin to a specified receiver address */
  async mintCoin(minter: DiemAccount, receiverAddress: HexString, amount: number | bigint): Promise<string> {
