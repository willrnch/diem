...
class AmbassadorClient {
  ...
  async burn(creator: DiemAccount, token: HexString): Promise<string> {
    const rawTxn = await provider.generateTransaction(creator.address(), {
...
async function main(): Promise<void> {
  ...
  console.log(
    "\nCompile and publish the Ambassador module (`diem-core/diem-move/move-examples/token_objects/ambassador`) using the following profile, and press enter:",
  );
  ...
  console.log(`    rest_url: "https://fullnode.devnet.diemlabs.com"`);
  console.log(`    faucet_url: "https://faucet.devnet.diemlabs.com"`);

  await waitForEnter();

  const adminAddr = admin.address();
  const userAddr = user.address();
  const tokenName = "Diem Ambassador #1";
  ...
  let txnHash = await client.mintAmbassadorToken(
    admin,
    "Diem Ambassador Token",
    tokenName,
    "https://raw.githubusercontent.com/aptos-labs/diem-core/main/ecosystem/typescript/sdk/examples/typescript/metadata/ambassador/",
    userAddr,
  );
