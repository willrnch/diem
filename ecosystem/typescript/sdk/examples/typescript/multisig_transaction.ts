...
import { DiemClient, DiemAccount, FaucetClient, BCS, TransactionBuilderMultiEd25519, TxnBuilderTypes } from "diem";
...
(async () => {
  ...
  // Each Diem account stores an auth key. Initial account address can be derived from auth key.
  // See https://diem.dev/concepts/accounts for more details.
  const authKey = TxnBuilderTypes.AuthenticationKey.fromMultiEd25519PublicKey(multiSigPublicKey);

  // Derive the multisig account address and fund the address with 5000 DiemCoin.
