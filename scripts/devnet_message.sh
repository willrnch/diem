...
curl https://fullnode.devnet.diemlabs.com/v1 > "$TEMP"

COMMIT="$(jq -r .git_hash "$TEMP")"
CHAIN_ID="$(jq -r .chain_id "$TEMP")"

DIGEST="$(crane digest diemlabs/validator:devnet_"$COMMIT")"
GENESIS_SHA="$(curl https://devnet.diemlabs.com/genesis.blob | shasum -a 256 | awk '{print $1}')"
WAYPOINT="$(curl https://devnet.diemlabs.com/waypoint.txt)"
...
- waypoint: $WAYPOINT
- Chain ID: $CHAIN_ID
You can follow the instructions here for upgrade: https://diem.dev/nodes/full-node/update-fullnode-with-new-devnet-releases
