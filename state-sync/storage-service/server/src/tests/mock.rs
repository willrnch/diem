...
use diem_config::{config::StorageServiceConfig, network_id::NetworkId};
...
impl MockClient {
    pub fn new(
        ...
        for network_id in network_ids.clone() {
            let queue_cfg =
                diem_channel::Config::new(storage_config.max_network_channel_size as usize)
    ...
    ) -> Receiver<Result<bytes::Bytes, diem_network::protocols::network::RpcError>> {
    ...
    pub async fn wait_for_response(
        &mut self,
        receiver: Receiver<Result<bytes::Bytes, diem_network::protocols::network::RpcError>>,
    ) -> Result<StorageServiceResponse, StorageServiceError> {
        if let Ok(response) =
