...
use diem_logger::{debug, info, sample, sample::SampleRate, trace, warn};
use diem_network::{application::interface::NetworkClient, protocols::network::RpcError};
...
/// An [`DiemDataClientInterface`] that fulfills requests from remote peers' Storage Service
/// over DiemNet.
///
/// The `DiemDataClient`:
///
/// 1. Sends requests to connected Diem peers.
        ...
        match result {
            ...
            Err(error) => {
                ...
                let client_error = match error {
                    diem_storage_service_client::Error::RpcError(rpc_error) => match rpc_error {
                    ...
                    diem_storage_service_client::Error::StorageServiceError(err) => {
