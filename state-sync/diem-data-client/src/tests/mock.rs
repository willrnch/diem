...
use diem_storage_service_server::network::{NetworkRequest, ResponseSender};
...
impl MockNetwork {
    pub fn new(
        ...
        data_client_config: Option<DiemDataClientConfig>,
        networks: Option<Vec<NetworkId>>,
    ) -> (Self, MockTimeService, DiemDataClient, DataSummaryPoller) {
        ...
        let (client, poller) = DiemDataClient::new(
            data_client_config,
            base_config,
            mock_time.clone(),
...
pub fn create_mock_data_client() -> Arc<dyn DiemDataClientInterface + Send + Sync> {
    Arc::new(MockDiemDataClient::new())
}

// This automatically creates a MockDiemDataClient
mock! {
    pub DiemDataClient {}
