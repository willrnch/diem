...
use crate::{
    interface::DiemDataClientInterface,
    logging::{LogEntry, LogEvent, LogSchema},
    metrics,
};
use diem_config::config::DiemDataClientConfig;
use diem_logger::{info, sample, sample::SampleRate, warn};
...
pub struct LatencyMonitor {
    ...
    data_client: Arc<dyn DiemDataClientInterface + Send + Sync>, // The data client through which to see advertised data
...
impl LatencyMonitor {
    pub fn new(
        ...
        data_client: Arc<dyn DiemDataClientInterface + Send + Sync>,
    ...
    pub async fn start_latency_monitor(mut self) {
        info!(
            (LogSchema::new(LogEntry::LatencyMonitor)
                .message("Starting the Diem data client latency monitor!"))
        );
...
mod tests {
    ...
    fn create_latency_monitor() -> (TimeService, LatencyMonitor) {
        let data_client_config = DiemDataClientConfig::default();
