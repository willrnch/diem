...
use crate::{
    ...
    tests::utils::{
        ...
        initialize_logger, MockDiemDataClient, NoopResponseCallback, MAX_ADVERTISED_EPOCH_END,
...
async fn test_stream_data_error() {
    ...
    let pending_response = PendingClientResponse {
        client_request: client_request.clone(),
        client_response: Some(Err(diem_data_client::error::Error::DataIsUnavailable(
            "Missing data!".into(),
        ))),
    };
    insert_response_into_pending_queue(&mut data_stream, pending_response);
...
async fn test_continuous_stream_subscription_timeout() {
    ...
    let data_client_config = DiemDataClientConfig {
...
fn create_data_stream(
    ...
    // Create an diem data client mock and notification generator
    let diem_data_client = MockDiemDataClient::new(data_client_config, true, false, true, false);
...
/// Sets the client response at the index in the pending queue to contain a
/// timeout response.
fn set_timeout_response_in_queue(data_stream: &mut DataStream<MockDiemDataClient>, index: usize) {
    let (sent_requests, _) = data_stream.get_sent_requests_and_notifications();
    let pending_response = sent_requests.as_mut().unwrap().get_mut(index).unwrap();
    let client_response = Some(Err(
        diem_data_client::error::Error::TimeoutWaitingForResponse("Timed out!".into()),
    ));
    pending_response.lock().client_response = client_response;
}
...
async fn wait_for_data_client_to_respond(
    ...
    loop {
        if let Some(client_response) = &pending_response.lock().client_response {
            if !matches!(
                client_response,
                Err(diem_data_client::error::Error::TimeoutWaitingForResponse(
                    _
                ))
            ) {
                return;
