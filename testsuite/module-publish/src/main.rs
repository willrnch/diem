...
#[clap(
    ...
    about = "Write Move packages binaries in a Rust file (raw_module_data.rs). Defaults to \n\
         diem-core/crates/transaction-emitter-lib/src/transaction_generator/publishing/raw_module_data.rs"
)]
struct Args {
...
fn additional_packages() -> Vec<(&'static str, &'static str)> {
    ...
    vec![(
        "ambassador_token",
        "../../diem-move/move-examples/token_objects/ambassador",
    )]
}
...
//
// To update this code, run `cargo run -p module-publish` in diem core.
