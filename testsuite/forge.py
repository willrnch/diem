...
GAR_REPO_NAME = "us-west1-docker.pkg.dev/diem-global/diem-internal"
...
# o11y resources
GRAFANA_BASE_URL = "https://diemlabs.grafana.net/d/overview/overview?orgId=1&refresh=10s&var-Datasource=VictoriaMetrics%20Global%20%28Non-mainnet%29&var-BigQuery=Google%20BigQuery"

# helm chart default override values
HELM_CHARTS = ["diem-node", "diem-genesis"]
...
class ForgeContext:
    ...
    def forge_chain_name(self) -> str:
        forge_chain_name = self.forge_cluster.name.lstrip("diem-")
...
def get_humio_forge_link(
    ...
    columns = [
        ...
        },
        {
            ...
            "openInNewBrowserTab": "***",
            "style": "button",
            "hrefTemplate": 'https://github.com/aptos-labs/diem-core/pull/{{fields["github_pr"]}}',
...
def get_current_cluster_name(shell: Shell) -> str:
    ...
    matches = re.findall(r"diem.*", current_context)
    if len(matches) != 1:
...
def test(
    ...
    # forge_cluster_name = "diem-forge-0"
