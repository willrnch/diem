...
use crate::{
    interface::system_metrics::SystemMetricsThreshold, DiemPublicInfo, ChainInfo, FullNode,
...
pub trait Swarm: Sync {
    ...
    fn diem_public_info(&mut self) -> DiemPublicInfo<'_> {
        self.chain_info().into_diem_public_info()
    }

    fn chain_info_for_node(&mut self, idx: usize) -> ChainInfo<'_>;

    fn diem_public_info_for_node(&mut self, idx: usize) -> DiemPublicInfo<'_> {
        self.chain_info_for_node(idx).into_diem_public_info()
    }
