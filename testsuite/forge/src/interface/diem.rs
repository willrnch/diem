...
use diem_rest_client::{Client as RestClient, PendingTransaction, State, Transaction};
use diem_sdk::{
...
pub trait DiemTest: Test {
    /// Executes the test against the given context.
    async fn run<'t>(&self, ctx: &mut DiemContext<'t>) -> Result<()>;
}

pub struct DiemContext<'t> {
    core: CoreContext,
    public_info: DiemPublicInfo<'t>,
    pub report: &'t mut TestReport,
}

impl<'t> DiemContext<'t> {
    pub fn new(
        core: CoreContext,
        public_info: DiemPublicInfo<'t>,
    ...
    pub fn transaction_factory(&self) -> TransactionFactory {
        let unit_price = std::cmp::max(diem_global_constants::GAS_UNIT_PRICE, 1);
        TransactionFactory::new(self.chain_id()).with_gas_unit_price(unit_price)
    }

    pub fn diem_transaction_factory(&self) -> TransactionFactory {
...
impl<'t> DiemPublicInfo<'t> {
        ...
        let create_account_txn =
            self.root_account
                .sign_with_transaction_builder(self.transaction_factory().payload(
                    diem_stdlib::diem_account_create_account(auth_key.derived_address()),
                ));
        self.rest_client
    ...
    pub async fn mint(&mut self, addr: AccountAddress, amount: u64) -> Result<()> {
        let mint_txn = self.root_account.sign_with_transaction_builder(
            self.transaction_factory()
                .payload(diem_stdlib::diem_coin_mint(addr, amount)),
        );
    ...
    pub async fn transfer_non_blocking(
        ...
        let tx = from_account.sign_with_transaction_builder(self.transaction_factory().payload(
            diem_stdlib::diem_coin_transfer(to_account.address(), amount),
        ));
    ...
    pub async fn get_approved_execution_hash_at_diem_governance(
        &self,
        proposal_id: u64,
    ) -> Vec<u8> {
        let approved_execution_hashes = self
            ...
            .get_account_resource_bcs::<SimpleMap<u64, Vec<u8>>>(
                CORE_CODE_ADDRESS,
                "0x1::diem_governance::ApprovedExecutionHashes",
            )
            .await;
...
pub async fn reconfig(
    ...
    let current = diem_version.into_inner();
    let current_version = *current.major.inner();
    let txn = root_account.sign_with_transaction_builder(
        transaction_factory
            .clone()
            .payload(diem_stdlib::version_set_version(current_version + 1)),
    );
