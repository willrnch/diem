...
use diem_db::{LEDGER_DB_NAME, STATE_MERKLE_DB_NAME};
use diem_logger::info;
use diem_rest_client::Client as RestClient;
use diem_sdk::types::PeerId;
use diem_state_sync_driver::metadata_storage::STATE_SYNC_DB_NAME;
use reqwest::Url;
...
const DIEM_DATA_DIR: &str = "/opt/diem/data";
...
impl Node for K8sNode {
    ...
    async fn clear_storage(&mut self) -> Result<()> {
        ...
        let ledger_db_path = format!("{}/db/{}", DIEM_DATA_DIR, LEDGER_DB_NAME);
        let state_db_path = format!("{}/db/{}", DIEM_DATA_DIR, STATE_MERKLE_DB_NAME);
        let state_sync_db_path = format!("{}/db/{}", DIEM_DATA_DIR, STATE_SYNC_DB_NAME);
