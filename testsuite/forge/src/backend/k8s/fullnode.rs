...
// these are constants given by the diem-node helm chart
// see terraform/helm/diem-node/templates/validator.yaml
...
const GENESIS_CONFIG_VOLUME_PATH: &str = "/opt/diem/genesis";

// the path where the config file is mounted in the fullnode
const DIEM_CONFIG_VOLUME_NAME: &str = "diem-config";
const DIEM_CONFIG_VOLUME_PATH: &str = "/opt/diem/etc";

// the path where the data volume is mounted in the fullnode
const DIEM_DATA_VOLUME_NAME: &str = "diem-data";
const DIEM_DATA_VOLUME_PATH: &str = "/opt/diem/data";
...
fn create_fullnode_persistent_volume_claim(
    ...
    Ok(PersistentVolumeClaim {
        metadata: ObjectMeta {
            name: Some(DIEM_DATA_VOLUME_NAME.to_string()),
            ..ObjectMeta::default()
        },
...
fn create_fullnode_container(
    ...
    Ok(Container {
        ...
        command: Some(vec![
            "/usr/local/bin/diem-node".to_string(),
            "-f".to_string(),
            format!("/opt/diem/etc/{}", FULLNODE_CONFIG_MAP_KEY),
        ]),
        volume_mounts: Some(vec![
            VolumeMount {
                mount_path: DIEM_CONFIG_VOLUME_PATH.to_string(),
                name: DIEM_CONFIG_VOLUME_NAME.to_string(),
                ..VolumeMount::default()
            },
            VolumeMount {
                mount_path: DIEM_DATA_VOLUME_PATH.to_string(),
                name: DIEM_DATA_VOLUME_NAME.to_string(),
                ..VolumeMount::default()
            },
            VolumeMount {
...
pub fn get_default_pfn_node_config() -> NodeConfig {
    ...
    NodeConfig {
        base: BaseConfig {
            ...
            data_dir: PathBuf::from(DIEM_DATA_VOLUME_PATH),
...
mod tests {
    ...
    use diem_config::config::Identity;
    use diem_sdk::crypto::{x25519::PrivateKey, Uniform};
    use k8s_openapi::apimachinery::pkg::api::resource::Quantity;

    /// Get a dummy validator persistent volume claim that looks like one created by terraform/helm/diem-node/templates/validator.yaml
    fn get_dummy_validator_persistent_volume_claim() -> PersistentVolumeClaim {
        PersistentVolumeClaim {
            metadata: ObjectMeta {
                name: Some("diem-node-0-validator-e42069".to_string()),
                ..ObjectMeta::default()
            },
            spec: Some(PersistentVolumeClaimSpec {
                access_modes: Some(vec!["ReadWriteOnce".to_string()]),
    ...
    /// Get a dummy validator stateful set that looks like one created by terraform/helm/diem-node/templates/validator.yaml
    fn get_dummy_validator_stateful_set() -> StatefulSet {
        let labels: BTreeMap<String, String> = [
            ...
            (
                ...
                "diem-node-0-validator-0".to_string(),
            ),
            (
                "app.kubernetes.io/part-of".to_string(),
                "forge-pfn".to_string(),
        ...
        StatefulSet {
            metadata: ObjectMeta {
                name: Some("diem-node-0-validator".to_string()),
                ...
                template: PodTemplateSpec {
                    ...
                    spec: Some(PodSpec {
                        containers: vec![Container {
                            ...
                            image: Some(
                                "banana.fruit.diem/potato/validator:banana_image_tag".to_string(),
                            ),
                            command: Some(vec![
                                "/usr/local/bin/diem-node".to_string(),
                                "-f".to_string(),
                                "/opt/diem/etc/validator.yaml".to_string(),
                            ]),
                            volume_mounts: Some(vec![
                                VolumeMount {
                                    mount_path: DIEM_CONFIG_VOLUME_PATH.to_string(),
    ...
    async fn test_create_node_config_map() {
        let config_map_name = "diem-node-0-validator-0-config".to_string();
    ...
    fn test_create_fullnode_stateful_set_and_service_connected() {
        ...
        let fullnode_genesis_secret_name = format!("diem-node-0-genesis-e{}", era);
