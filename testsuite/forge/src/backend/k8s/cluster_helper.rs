...
use crate::{
    ...
    NodeConfigFn, ReadWrite, Result, DIEM_NODE_HELM_CHART_PATH, DIEM_NODE_HELM_RELEASE_NAME,
...
async fn wait_genesis_job(kube_client: &K8sClient, era: &str, kube_namespace: &str) -> Result<()> {
    diem_retrier::retry_async(k8s_wait_genesis_strategy(), || {
        let jobs: Api<Job> = Api::namespaced(kube_client.clone(), kube_namespace);
        Box::pin(async move {
            let job_name = format!("{}-diem-genesis-e{}", GENESIS_HELM_RELEASE_NAME, era);
...
async fn wait_node_haproxy(
    ...
    diem_retrier::retry_async(k8s_wait_nodes_strategy(), || {
...
/// Deletes a collection of resources in k8s as part of diem-node
...
pub(crate) async fn delete_k8s_resources(client: K8sClient, kube_namespace: &str) -> Result<()> {
    ...
    let diem_node_helm_selector = "app.kubernetes.io/part-of=diem-node";
    let testnet_addons_helm_selector = "app.kubernetes.io/part-of=testnet-addons";
    let genesis_helm_selector = "app.kubernetes.io/part-of=diem-genesis";
    ...
    // cross this with all the compute resources created by diem-node helm chart
...
fn upgrade_diem_node_helm(options: &[String], kube_namespace: String) -> Result<()> {
    upgrade_helm_release(
        DIEM_NODE_HELM_RELEASE_NAME.to_string(),
        DIEM_NODE_HELM_CHART_PATH.to_string(),
        options,
        kube_namespace,
    )
}

// runs helm upgrade on the installed diem-genesis release named "genesis"
...
pub async fn uninstall_testnet_resources(kube_namespace: String) -> Result<()> {
    ...
    info!(
        "diem-node resources for Forge removed in namespace: {}",
        kube_namespace
    );
...
fn get_node_default_helm_path() -> String {
    match ForgeRunnerMode::try_from_env().unwrap_or(ForgeRunnerMode::K8s) {
        ForgeRunnerMode::Local => {
            "testsuite/forge/src/backend/k8s/helm-values/diem-node-default-values.yaml"
        },
        ForgeRunnerMode::K8s => "/diem/terraform/diem-node-default-values.yaml",
    }
    .to_string()
}
...
/// Installs a testnet in a k8s namespace by first running genesis, and the installing the diem-nodes via helm
...
pub async fn install_testnet_resources(
    ...
    let diem_node_values_file = dump_helm_values_to_file(DIEM_NODE_HELM_RELEASE_NAME, &tmp_dir)?;
    ...
    let diem_node_forge_helm_values_yaml = construct_node_helm_values(
        node_helm_config_fn,
    ...
    let diem_node_forge_values_file = dump_string_to_file(
        "diem-node-values.yaml".to_string(),
        diem_node_forge_helm_values_yaml,
        &tmp_dir,
    )?;
    ...
    let diem_node_upgrade_options = vec![
        // use the old values
        "-f".to_string(),
        diem_node_values_file,
        "-f".to_string(),
        diem_node_forge_values_file,
    ];
    ...
    upgrade_diem_node_helm(
        diem_node_upgrade_options.as_slice(),
        kube_namespace.clone(),
    )?;
