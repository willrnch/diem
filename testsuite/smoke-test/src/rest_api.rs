...
use crate::{
    smoke_test_environment::{new_local_swarm_with_diem, SwarmBuilder},
    txn_emitter::generate_traffic,
};
use diem_cached_packages::diem_stdlib;
use diem_crypto::ed25519::Ed25519Signature;
use diem_forge::{LocalSwarm, NodeExt, Swarm, TransactionType};
use diem_global_constants::{DEFAULT_BUCKETS, GAS_UNIT_PRICE};
use diem_rest_client::diem_api_types::{MoveModuleId, TransactionData};
...
async fn test_basic_client() {
    ...
    let tx = account1.sign_with_transaction_builder(
        info.transaction_factory()
            .payload(diem_stdlib::diem_coin_transfer(account2.address(), 1)),
    );
...
async fn test_bcs() {
    ...
    for (i, expected_transaction) in transactions.iter().enumerate() {
        ...
        let expected_hash =
            diem_crypto::HashValue::from(expected_transaction.transaction_info().unwrap().hash);
    ...
    for (i, json_txn) in json_txns.iter().enumerate() {
        ...
        assert_eq!(
            diem_crypto::HashValue::from(json_txn.transaction_info().unwrap().hash),
    ...
    assert_eq!(
        diem_crypto::HashValue::from(json_txn.info.hash),
        bcs_txn.info.transaction_hash()
    );
    ...
    assert_eq!(
        diem_crypto::HashValue::from(json_block.block_hash),
    ...
    assert_eq!(
        diem_crypto::HashValue::from(first_json_txn.transaction_info().unwrap().hash),
    ...
    assert_eq!(
        diem_crypto::HashValue::from(json_block_by_height.block_hash),
