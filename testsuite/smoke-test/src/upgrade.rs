...
use crate::{
    diem::move_test_helpers, smoke_test_environment::SwarmBuilder,
...
use diem_gas::{DiemGasParameters, GasQuantity, InitialGasSchedule, ToOnChainGasSchedule};
use diem_release_builder::{
    components::{
...
#[tokio::test]
/// This test verifies the flow of diem framework upgrade process.
/// i.e: The network will be alive after applying the new diem framework release.
async fn test_upgrade_flow() {
    ...
    let url = env.diem_public_info().url().to_string();
    let private_key = env
        .diem_public_info()
        .root_account()
        .private_key()
    ...
    // TODO: Replace this logic with diem-gas
    let mut gas_parameters = DiemGasParameters::initial();
    gas_parameters.txn.max_transaction_size_in_bytes = GasQuantity::new(100_000_000);

    let gas_schedule = diem_types::on_chain_config::GasScheduleV2 {
        feature_version: diem_gas::LATEST_GAS_FEATURE_VERSION,
        entries: gas_parameters.to_on_chain_gas_schedule(diem_gas::LATEST_GAS_FEATURE_VERSION),
    };
    ...
    assert!(Command::new(diem_cli.as_path())
        ...
        .status
        .success());
    *env.diem_public_info().root_account().sequence_number_mut() += 1;

    let upgrade_scripts_folder = TempPath::new();
    upgrade_scripts_folder.create_as_dir().unwrap();

    let config = diem_release_builder::ReleaseConfig {
            ...
            Proposal {
                ...
                update_sequence: vec![
                    ReleaseEntry::FeatureFlag(Features {
                        enabled: diem_vm_genesis::default_features()
                            .into_iter()
    ...
    move_test_helpers::publish_package(&mut env.diem_public_info(), base_path_v1)
        .await
        .unwrap();
...
// to repeatedly compile all the diem framework pacakges as dependency
//
#[ignore]
...
async fn test_release_validate_tool_multi_step() {
    let (mut env, _, _) = SwarmBuilder::new_local(1)
        ...
        .build_with_cli(2)
        .await;
    let config = diem_release_builder::ReleaseConfig::default();
    ...
    let network_config = diem_release_builder::validate::NetworkConfig {
    ...
    diem_release_builder::validate::validate_config(config, network_config)
        .await
        .unwrap();

    let root_account = env.diem_public_info().root_account().address();
    // Test the module publishing workflow
    *env.diem_public_info().root_account().sequence_number_mut() = env
        .diem_public_info()
        .client()
