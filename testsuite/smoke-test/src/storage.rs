...
async fn test_db_restore() {
    ...
    let mut swarm = SwarmBuilder::new_local(4).with_diem().build().await;
...
fn db_backup_verify(backup_path: &Path, trusted_waypoints: &[Waypoint]) {
    info!("---------- running diem-db-tool backup-verify");
    let now = Instant::now();
    let bin_path = workspace_builder::get_bin("diem-db-tool");
...
pub fn db_backup(
    ...
    info!("---------- running diem db tool backup");
    let now = Instant::now();
    let bin_path = workspace_builder::get_bin("diem-db-tool");
