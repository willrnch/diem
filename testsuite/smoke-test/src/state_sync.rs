...
use diem_forge::{LocalSwarm, Node, NodeExt, Swarm, SwarmExt};
...
async fn test_full_node_bootstrap_state_snapshot() {
    ...
    let state_merkle_pruner_version = inspection_client
        .get_node_metric_i64(
            "diem_pruner_versions{pruner_name=state_merkle_pruner,tag=min_readable}",
        )
        .await
        .unwrap()
        .unwrap();
    let epoch_snapshot_pruner_version = inspection_client
        .get_node_metric_i64(
            "diem_pruner_versions{pruner_name=epoch_snapshot_pruner,tag=min_readable}",
        )
        .await
        .unwrap()
        .unwrap();
    let ledger_pruner_version = inspection_client
        .get_node_metric_i64("diem_pruner_versions{pruner_name=ledger_pruner,tag=min_readable}")
        .await
        .unwrap()
        .unwrap();
...
async fn test_full_node_bootstrap_outputs() {
    ...
    vfn_config.state_sync.diem_data_client.use_compression = true;
...
async fn test_full_node_bootstrap_outputs_no_compression() {
    ...
    vfn_config
        ...
        .state_sync_driver
        .continuous_syncing_mode = ContinuousSyncingMode::ApplyTransactionOutputs;
    vfn_config.state_sync.diem_data_client.use_compression = false;
...
async fn test_full_node_bootstrap_outputs_exponential_backoff() {
    ...
    vfn_config
        ...
        .state_sync_driver
        .continuous_syncing_mode = ContinuousSyncingMode::ApplyTransactionOutputs;
    vfn_config.state_sync.diem_data_client.response_timeout_ms = 1;
...
async fn test_validator_bootstrap_state_snapshot_no_compression() {
    ...
    let mut swarm = SwarmBuilder::new_local(4)
        ...
        .with_init_config(Arc::new(|_, config, _| {
            ...
            config.state_sync.state_sync_driver.continuous_syncing_mode =
                ContinuousSyncingMode::ApplyTransactionOutputs;
            config.state_sync.diem_data_client.use_compression = false;
        }))
        .build()
        .await;
    ...
    let mut swarm = SwarmBuilder::new_local(4)
        ...
        .with_init_config(Arc::new(|_, config, _| {
            ...
            config.state_sync.state_sync_driver.continuous_syncing_mode =
                ContinuousSyncingMode::ApplyTransactionOutputs;
            config.state_sync.diem_data_client.use_compression = false;
            config.state_sync.diem_data_client.response_timeout_ms = 1;
        }))
        .build()
        .await;
