...
use anyhow::anyhow;
use diem_forge::{NetworkContext, NetworkTest, Result, Test};
...
impl NetworkTest for ReconfigurationTest {
    fn run(&self, _ctx: &mut NetworkContext<'_>) -> Result<()> {
        Err(anyhow!("Not supported in diem-framework yet"))
    }
    // TODO(https://github.com/aptos-labs/diem-core/issues/317): add back after support those transactions in diem-framework
    //     let rt = Runtime::new()?;
    //
