...
SHARED_DOCKER_VOLUME_NAMES = ["diem-shared", "indexer-grpc-file-store"]
    ...
    for _ in range(GRPC_PROGRESS_THRESHOLD_SECS // retry_secs):
        res = shell.run(
            [
                ...
                '{ "starting_version": 0 }',
                "-H",
                "x-diem-data-authorization:dummy_token",
                "-import-path",
                "crates/diem-protos/proto",
                "-proto",
                "diem/indexer/v1/raw_data.proto",
                "-plaintext",
                GRPC_DATA_SERVICE_URL,
                "diem.indexer.v1.RawData/GetTransactions",
            ],
        )
        if (
            res.exit_code == 124
