...
pub fn get_registry() -> Result<Registry> {
    ...
    tracer.trace_value(
        &mut samples,
        &diem_consensus_types::block::Block::make_genesis_block(),
    )?;
    ...
    tracer.trace_type::<diem_consensus::network_interface::ConsensusMsg>(&samples)?;
    tracer.trace_type::<diem_consensus_types::block_data::BlockType>(&samples)?;
    tracer.trace_type::<diem_consensus_types::block_retrieval::BlockRetrievalStatus>(&samples)?;
    tracer.trace_type::<diem_consensus_types::common::Payload>(&samples)?;
