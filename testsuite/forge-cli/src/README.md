...
id: forge cli
title: Forge CLI
custom_edit_url: https://github.com/aptos-labs/diem-core/edit/main/testsuite/forge-cli/README.md
---

# Forge CLI

This crate contains the Forge command line interface (CLI) tool. This enables users to
run local and remote Diem swarms (i.e., networks of validators and validator fullnodes). For
example, to deploy a local validator swarm, run:

```
cargo run -p diem-forge-cli -- --suite "run_forever" --num-validators 4 test local-swarm
```
...
```
...
2022-09-01T15:41:27.228289Z [main] INFO crates/diem-genesis/src/builder.rs:462 Building genesis with 4 validators. Directory of output: "/private/var/folders/dx/c0l2rrkn0656gfx6v5_dy_p80000gn/T/.tmpq9uPMJ"
...
...
...
2022-09-01T15:41:28.094800Z [main] INFO testsuite/forge/src/backend/local/node.rs:129 Started node 0 (PID: 78939) with command: ".../diem-core/target/debug/diem-node" "-f" "/private/var/folders/dx/c0l2rrkn0656gfx6v5_dy_p80000gn/T/.tmpq9uPMJ/0/node.yaml"
...
```
kill -9 <Node 0 PID>
cargo run -p diem-node -- -f <Location to the node 0 configuration file displayed above>
```
...
```bash
cargo run -p diem-faucet-service -- run-simple --key <key> --node-url <node_url>
```
...
```
cargo run -p diem-faucet-cli -- --amount 10 --accounts <account_address> --key <private_key>
```

:::tip Faucet and Diem CLI
See more on how the faucet works in the [README](https://github.com/aptos-labs/diem-core/tree/main/crates/diem-faucet).

Also see how to use the [Diem CLI](../cli-tools/diem-cli-tool/use-diem-cli.md#account-examples) with an existing faucet.
:::
...
```
cargo run -p diem-forge-cli -- --suite "run_forever" --num-validators 3 --num-validator-fullnodes 1 test local-swarm
```
...
```
cargo run -p diem-forge-cli --help
```
