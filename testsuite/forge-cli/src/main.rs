...
use diem_config::config::{ChainHealthBackoffValues, ConsensusConfig, PipelineBackpressureValues};
use diem_forge::{
...
use diem_sdk::{move_types::account_address::AccountAddress, transaction_builder::diem_stdlib};
use diem_testcases::{
...
fn get_changelog(prev_commit: Option<&String>, upstream_commit: &str) -> String {
    ...
    let commits = github_client.get_commits("aptos-labs/diem-core", upstream_commit);
    match commits {
        Err(e) => {
...
fn run_forever() -> ForgeConfig {
    ForgeConfig::default()
        ...
        .with_genesis_module_bundle(diem_cached_packages::head_release_bundle().clone())
        .add_diem_test(RunForever)
}

fn local_test_suite() -> ForgeConfig {
    ForgeConfig::default()
        .add_diem_test(FundAccount)
        .add_diem_test(TransferCoins)
        .add_admin_test(GetMetadata)
    ...
    ForgeConfig::default()
        ...
        .with_emit_job(
            EmitJobRequest::default()
                ...
                .gas_price(5 * diem_global_constants::GAS_UNIT_PRICE),
        )
        .with_genesis_helm_config_fn(Arc::new(|helm_values| {
            helm_values["chain"]["epoch_duration_secs"] = 300.into();
        }))
...
impl DiemTest for RunForever {
    async fn run<'t>(&self, _ctx: &mut DiemContext<'t>) -> Result<()> {
...
#[async_trait::async_trait]
impl DiemTest for TransferCoins {
    async fn run<'t>(&self, ctx: &mut DiemContext<'t>) -> Result<()> {
        let client = ctx.client();
        ...
        let transfer_txn = payer.sign_with_transaction_builder(
            ctx.diem_transaction_factory()
                .payload(diem_stdlib::diem_coin_transfer(payee.address(), 10)),
        );
