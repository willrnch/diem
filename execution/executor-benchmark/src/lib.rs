...
use diem_block_executor::counters as block_executor_counters;
use diem_config::config::{NodeConfig, PrunerConfig};
use diem_db::DiemDB;
use diem_executor::{
    ...
    metrics::{
        ...
        DIEM_EXECUTOR_OTHER_TIMERS_SECONDS, DIEM_EXECUTOR_VM_EXECUTE_BLOCK_SECONDS,
    },
};
...
use diem_metrics_core::Histogram;
use diem_sdk::types::LocalAccount;
use diem_storage_interface::DbReaderWriter;
use diem_transaction_generator_lib::{
    ...
    V: TransactionBlockExecutor + 'static,
{
    ...
    let (mut config, genesis_key) = diem_genesis::test_utils::test_config();
    ...
    if let Some(NonConflictingCoinTransfer { .. }) = transaction_type {
        ...
        // `diem_executor_benchmark::transaction_generator::TransactionGenerator` needs to hold
    ...
    let start_execution_total = DIEM_EXECUTOR_EXECUTE_BLOCK_SECONDS.get_sample_sum();
    let start_vm_only = DIEM_EXECUTOR_VM_EXECUTE_BLOCK_SECONDS.get_sample_sum();
    ...
    let start_by_other = other_labels
        ...
        .map(|(_prefix, _top_level, other_label)| {
            (
                ...
                DIEM_EXECUTOR_OTHER_TIMERS_SECONDS
                    .with_label_values(&[other_label])
                    .get_sample_sum(),
            )
        })
        .collect::<HashMap<_, _>>();
    let start_commit_total = DIEM_EXECUTOR_COMMIT_BLOCKS_SECONDS.get_sample_sum();

    let start_vm_time = DIEM_EXECUTOR_VM_EXECUTE_BLOCK_SECONDS.get_sample_sum();
    ...
    let delta_vm_time = DIEM_EXECUTOR_VM_EXECUTE_BLOCK_SECONDS.get_sample_sum() - start_vm_time;
    info!(
    ...
    let time_in_execution =
        DIEM_EXECUTOR_EXECUTE_BLOCK_SECONDS.get_sample_sum() - start_execution_total;
    info!(
        "Overall fraction of total: {:.3} in execution (component TPS: {})",
        time_in_execution / elapsed,
        delta_v / time_in_execution
    );
    let time_in_vm = DIEM_EXECUTOR_VM_EXECUTE_BLOCK_SECONDS.get_sample_sum() - start_vm_only;
    info!(
    ...
    for (prefix, top_level, other_label) in other_labels {
        let time_in_label = DIEM_EXECUTOR_OTHER_TIMERS_SECONDS
            .with_label_values(&[other_label])
            .get_sample_sum()
    ...
    let time_in_commit = DIEM_EXECUTOR_COMMIT_BLOCKS_SECONDS.get_sample_sum() - start_commit_total;
    info!(
    ...
    V: TransactionBlockExecutor + 'static,
{
    ...
    println!(
        ...
        DIEM_JELLYFISH_INTERNAL_ENCODED_BYTES.get()
    );
    println!(
        "Total written leaf nodes value size: {} bytes",
        DIEM_JELLYFISH_LEAF_ENCODED_BYTES.get()
    );
}
...
mod tests {
    ...
    use diem_config::config::NO_OP_STORAGE_PRUNER_CONFIG;
    use diem_executor::block_executor::TransactionBlockExecutor;
    use diem_temppath::TempPath;
    use diem_transaction_generator_lib::args::TransactionTypeArg;
    use diem_vm::DiemVM;
    ...
    #[test]
    fn test_benchmark() {
        test_generic_benchmark::<DiemVM>(None, true);
    }

    #[test]
    fn test_benchmark_transaction() {
        test_generic_benchmark::<DiemVM>(Some(TransactionTypeArg::TokenV2AmbassadorMint), true);
    }
