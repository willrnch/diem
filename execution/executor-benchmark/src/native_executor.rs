...
impl NativeExecutor {
    ...
    fn withdraw_from_signer(
        ...
        let sender_coin_store_key = DbAccessUtil::new_state_key_diem_coin(sender_address);
    ...
    fn deposit(
        ...
        let recipient_coin_store_key = DbAccessUtil::new_state_key_diem_coin(recipient_address);
            ...
            transactions
                ...
                .map(|txn| match &txn {
                    ...
                    Transaction::UserTransaction(user_txn) => match user_txn.payload() {
                        diem_types::transaction::TransactionPayload::EntryFunction(f) => {
                            match (
                                *f.module().address(),
                                ...
                                (AccountAddress::ONE, "diem_account", "transfer") => {
                                    Self::handle_account_creation_and_transfer(
                                        user_txn.sender(),
                                        bcs::from_bytes(&f.args()[0]).unwrap(),
                                        bcs::from_bytes(&f.args()[1]).unwrap(),
                                ...
                                (AccountAddress::ONE, "diem_account", "create_account") => {
                                    Self::handle_account_creation_and_transfer(
                                        user_txn.sender(),
                                        bcs::from_bytes(&f.args()[0]).unwrap(),
                                        0,
                                ...
                                (AccountAddress::ONE, "diem_account", "batch_transfer") => {
