...
use diem_types::{
    ...
    account_config::{diem_test_root_address, AccountResource, CORE_CODE_ADDRESS},
...
fn test_genesis() {
    ...
    let genesis = diem_vm_genesis::test_genesis_transaction();
    ...
    let (diem_framework_account_resource, state_proof) = db
        .reader
    ...
    state_proof
        .verify(
            ...
            diem_framework_account_resource.as_ref(),
        )
        .unwrap();
}
...
fn test_reconfiguration() {
    ...
    let txn1 = get_test_signed_transaction(
        ...
        Some(diem_stdlib::diem_coin_mint(validator_account, 1_000_000)),
    );
    ...
    // txn3 = set the diem version
    let txn3 = get_test_signed_transaction(
        diem_test_root_address(),
        /* sequence_number = */ 1,
        genesis_key.clone(),
        genesis_key.public_key(),
        Some(diem_stdlib::version_set_version(42)),
    );
    ...
    let t3 = db
        .reader
        .get_account_transaction(diem_test_root_address(), 1, true, current_version)
        .unwrap();
    ...
    let diem_framework_account_state_view2 =
        db_state_view.as_account_with_state_view(&CORE_CODE_ADDRESS);

    assert_eq!(
        diem_framework_account_state_view2
            .get_version()
            .unwrap()
            .unwrap()
            .major,
