...
use diem_executor_types::{ExecutedBlock, ExecutedChunk};
use diem_infallible::Mutex;
use diem_logger::{sample, sample::SampleRate, trace, warn};
...
pub static SHARDED_BLOCK_EXECUTOR: Lazy<Arc<Mutex<ShardedBlockExecutor<CachedStateView>>>> =
    Lazy::new(|| {
        let executor_clients =
            ShardedExecutorClient::create_sharded_executor_clients(DiemVM::get_num_shards(), None);
...
impl ChunkOutput {
    ...
    fn execute_block<V: VMExecutor>(
        ...
        use diem_state_view::{StateViewId, TStateView};
...
pub fn update_counters_for_processed_chunk(
    ...
    let detailed_counters = DiemVM::get_processed_transactions_detailed_counters();
    ...
    for (txn, output) in transactions.iter().zip(transaction_outputs.iter()) {
        ...
        metrics::DIEM_PROCESSED_TXNS_COUNT
            .with_label_values(&[process_type, kind, state])
            .inc();

        if !error_code.is_empty() {
            metrics::DIEM_PROCESSED_FAILED_TXNS_REASON_COUNT
                .with_label_values(&[
        ...
        if let Transaction::UserTransaction(user_txn) = txn {
            match user_txn.payload() {
                diem_types::transaction::TransactionPayload::Script(_script) => {
                    metrics::DIEM_PROCESSED_USER_TRANSACTIONS_PAYLOAD_TYPE
                        .with_label_values(&[process_type, "script", state])
                        .inc();
                },
                diem_types::transaction::TransactionPayload::EntryFunction(function) => {
                    metrics::DIEM_PROCESSED_USER_TRANSACTIONS_PAYLOAD_TYPE
                        .with_label_values(&[process_type, "function", state])
                        .inc();

                    let is_core = function.module().address() == &CORE_CODE_ADDRESS;
                    metrics::DIEM_PROCESSED_USER_TRANSACTIONS_ENTRY_FUNCTION_MODULE
                        .with_label_values(&[
                            detailed_counters_label,
                            process_type,
                    ...
                    if is_core && detailed_counters {
                        metrics::DIEM_PROCESSED_USER_TRANSACTIONS_ENTRY_FUNCTION_CORE_METHOD
                            .with_label_values(&[
                                process_type,
                                function.module().name().as_str(),
                ...
                diem_types::transaction::TransactionPayload::Multisig(_) => {
                    metrics::DIEM_PROCESSED_USER_TRANSACTIONS_PAYLOAD_TYPE
                ...
                diem_types::transaction::TransactionPayload::ModuleBundle(_module) => {
                    metrics::DIEM_PROCESSED_USER_TRANSACTIONS_PAYLOAD_TYPE
        ...
        for event in output.events() {
            ...
            metrics::DIEM_PROCESSED_USER_TRANSACTIONS_CORE_EVENTS
                .with_label_values(&[
                    detailed_counters_label,
                    process_type,
                    if is_core { "core" } else { "user" },
