...
If you're not familar with GCP (Google Cloud Platform), checkout this [tutorial](https://diem.dev/nodes/full-node/run-a-fullnode-on-gcp#prerequisites) for GCP account setup.

## Run on GCP
This guide assumes you already have GCP account setup, and have created a new project for deploying Diem node.
...
4. Modify `main.tf` file to configure Terraform, and create fullnode from Terraform module. Example content for `main.tf`:
  ...
  module "diem-node" {
    ...
    source        = "github.com/aptos-labs/diem-core.git//terraform/diem-node/gcp?ref=testnet"
  ...
  For the full customization options, see the variables file [here](https://github.com/aptos-labs/diem-core/blob/main/terraform/diem-node/gcp/variables.tf), and the [helm values](https://github.com/aptos-labs/diem-core/blob/main/terraform/helm/diem-node/values.yaml).
...
8. Once Terraform apply finishes, you can check if those resources are created:
  ...
  If you have just created your GCP project, you'll need to authorize the service account of your cluster to fetch images from the `diem-global` Docker registry.
  
    - open [the registry](https://console.cloud.google.com/artifacts?referrer=search&project=diem-global)
    - select `diem-internal` then click on `ADD PRINCIPAL` on the right in the `Permissions` tab and assign the `Artifact Registry Reader` role to the service account

9. Get your node IP info:

    ```
    $ export VALIDATOR_ADDRESS="$(kubectl get svc \
        ${WORKSPACE}-diem-node-0-validator-lb \
        --output jsonpath='{.status.loadBalancer.ingress[0].ip}')"

    $ export FULLNODE_ADDRESS="$(kubectl get svc \
        ${WORKSPACE}-diem-node-0-fullnode-lb \
        --output jsonpath='{.status.loadBalancer.ingress[0].ip}')"
    ```
...
10. Generate key pairs (node owner key, consensus key and networking key) in your working directory.
    ...
    ```
    $ diem genesis generate-keys --output-dir ~/$WORKSPACE
    ```
...
11. Configure validator information.
    ...
    This will create a directory in your working directory with your username, e.g. `diembot`, and two files inside it: `operator.yaml` and `owner.yaml`. `operator.yaml` should looks like:
...
12. Create layout YAML file, which defines the node in the validatorSet. For test mode, we can create a genesis blob containing only one node. **Note: this step is only needed for starting the node in test mode, for production, it will be generated by Diem Labs**
...
13. Download DiemFramework Move bytecode into a folder named `framework`. **Note: this step is only needed for starting the node in test mode, for production, it will be generated by Diem Labs**

    Download the Diem Framework from the release page: https://github.com/aptos-labs/diem-core/releases/tag/diem-framework-v0.1.0
...
14. Compile genesis blob and waypoint. **Note: this step is only needed for starting the node in test mode, for production, it will be generated by Diem Labs**

    ```
    $ diem genesis generate-genesis --local-repository-dir ~/$WORKSPACE --output-dir ~/$WORKSPACE
    ``` 
...
15. To recap, in your working directory, you should have a list of files:
    ...
    - `framework` folder which contains all the move bytecode for DiemFramework.
...
16. Insert `genesis.blob`, `waypoint.txt` and identity files as secret into k8s cluster.
    ...
    ```
    $ kubectl create secret generic ${WORKSPACE}-diem-node-genesis-e1 \
...
17. Check all pods running.
    ...
    node1-diem-node-fullnode-e9-0              1/1     Running   0          4h31m
    node1-diem-node-haproxy-7cc4c5f74c-l4l6n   1/1     Running   0          4h40m
    node1-diem-node-validator-0                1/1     Running   0          4h30m
    ```
