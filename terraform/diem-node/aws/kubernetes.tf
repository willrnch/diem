...
locals {
  ...
  diem_node_helm_chart_path = var.helm_chart != "" ? var.helm_chart : "${path.module}/../../helm/diem-node"
}
...
locals {
  helm_values = jsonencode({
    ...
    validator = {
      ...
      remoteLogAddress = var.enable_logger ? "${helm_release.logger[0].name}-diem-logger.${helm_release.logger[0].namespace}.svc:5044" : null
    }
    fullnode = {
      storage = {
...
resource "helm_release" "validator" {
  ...
  chart       = local.diem_node_helm_chart_path
  max_history = 5
  wait        = false
  ...
  dynamic "set" {
    ...
    content {
      ...
      name  = "chart_sha1"
      value = sha1(join("", [for f in fileset(local.diem_node_helm_chart_path, "**") : filesha1("${local.diem_node_helm_chart_path}/${f}")]))
    }
  }
}
...
resource "helm_release" "logger" {
  ...
  values = [
    jsonencode({
      ...
      serviceAccount = {
        create = false
        # this name must match the serviceaccount created by the diem-node helm chart
        name = local.helm_release_name == "diem-node" ? "diem-node-validator" : "${local.helm_release_name}-diem-node-validator"
      }
    }),
