...
data "kubernetes_service" "validator-lb" {
  ...
  metadata {
    # This is the main validator LB service that is created by the diem-node helm chart
    name = "${local.workspace_name}-diem-node-0-validator-lb"
  }
...
data "kubernetes_service" "fullnode-lb" {
  count = var.zone_id == "" || !var.create_records ? 0 : 1
  metadata {
    # This is the main fullnode LB service that is created by the diem-node helm chart
    name = "${local.workspace_name}-diem-node-0-fullnode-lb"
  }
  depends_on = [time_sleep.lb_creation]
}
