...
module "validator" {
  ...
  helm_release_name_override = var.enable_forge ? "diem-node" : ""
  helm_values                = var.diem_node_helm_values
...
locals {
  ...
  diem_node_helm_prefix = var.enable_forge ? "diem-node" : "${module.validator.helm_release_name}-diem-node"
}
