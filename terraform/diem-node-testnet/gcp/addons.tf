    ...
    jsonencode({
      chaos-mesh = {
        chaosDaemon = {
          ...
          image = {
            repository = "diem-internal/chaos-daemon"
            tag        = "latest"
          }
        },
        controllerManager = {
          image = {
            repository = "diem-internal/chaos-mesh"
            tag        = "latest"
          }
        },
        dashboard = {
          image = {
            repository = "diem-internal/chaos-dashboard"
            tag        = "latest"
          }
        }
        images = {
          registry = "us-west1-docker.pkg.dev/diem-global"
        }
      }
    })
  ]
  ...
  values = [
    jsonencode({
      ...
      extraArgs = [
        ...
        "--txt-owner-id=diem-${local.workspace_name}",
        # "--txt-prefix=diem-",
      ]
    })
  ]
...
resource "google_compute_global_address" "testnet-addons-ingress" {
  count   = var.zone_name != "" ? 1 : 0
  project = var.project
  name    = "diem-${local.workspace_name}-testnet-addons-ingress"
}
...
resource "helm_release" "testnet-addons" {
  ...
  values = [
    jsonencode({
      ...
      genesis = {
        era             = var.era
        username_prefix = local.diem_node_helm_prefix
      ...
      ingress = {
        gce_static_ip           = "diem-${local.workspace_name}-testnet-addons-ingress"
        gce_managed_certificate = "diem-${local.workspace_name}-${var.zone_name}-testnet-addons"
      }
      load_test = {
        fullnodeGroups = try(var.diem_node_helm_values.fullnode.groups, [])
        config = {
