...
variable "chain_id" {
  description = "Diem chain ID. If var.enable_forge set, defaults to 4"
  default     = 4
}
...
variable "chain_name" {
  description = "Diem chain name. If unset, defaults to using the workspace name"
  default     = ""
}

variable "image_tag" {
  description = "Docker image tag for all Diem workloads, including validators, fullnodes, backup, restore, genesis, and other tooling"
  default     = "devnet"
}
...
variable "diem_node_helm_values" {
  description = "Map of values to pass to diem-node helm chart"
  type        = any
  default     = {}
}
...
variable "manage_via_tf" {
  description = "Whether to manage the diem-node k8s workload via Terraform. If set to false, the helm_release resource will still be created and updated when values change, but it may not be updated on every apply"
  default     = true
}
