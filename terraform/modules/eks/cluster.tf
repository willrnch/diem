...
resource "aws_iam_role" "aws-ebs-csi-driver" {
  name                 = "diem-${local.workspace_name}-ebs-csi-controller"
  path                 = var.iam_path
...
data "aws_iam_policy_document" "cluster-autoscaler" {
  statement {
    ...
    condition {
      test     = "StringEquals"
      variable = "aws:ResourceTag/k8s.io/cluster-autoscaler/${aws_eks_cluster.diem.name}"
      values   = ["owned"]
    }
  }
...
resource "aws_iam_role" "cluster-autoscaler" {
  name                 = "diem-fullnode-${local.workspace_name}-cluster-autoscaler"
  path                 = var.iam_path
  permissions_boundary = var.permissions_boundary_policy
...
resource "helm_release" "autoscaling" {
  ...
  values = [
    jsonencode({
      autoscaler = {
        enabled     = true
        clusterName = aws_eks_cluster.diem.name
        image = {
          # EKS does not report patch version
          tag = "v${aws_eks_cluster.diem.version}.0"
        }
        serviceAccount = {
          annotations = {
