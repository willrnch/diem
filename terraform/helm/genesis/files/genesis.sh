...
MOVE_FRAMEWORK_DIR=${MOVE_FRAMEWORK_DIR:-"/diem-framework/move"}
...
for i in $(seq 0 $(($NUM_VALIDATORS-1))); do
    ...
    if [[ -z "${RANDOM_SEED}" ]]; then
      diem genesis generate-keys --output-dir $user_dir
    else
      seed=$(printf "%064x" "$((${RANDOM_SEED_IN_DECIMAL}+i))")
      echo "seed=$seed for ${i}th validator"
      diem genesis generate-keys --random-seed $seed --output-dir $user_dir
    fi

    diem genesis set-validator-configuration --owner-public-identity-file $user_dir/public-keys.yaml --local-repository-dir $WORKSPACE \
        --username $username \
...
# get the framework
# this is the directory the diem-framework is located in the diemlabs/tools docker image
