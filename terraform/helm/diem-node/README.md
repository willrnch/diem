...
Diem blockchain node deployment
...
| fullnode.config | object | `{"full_node_networks":[{"network_id":"public","seeds":{}}]}` | Fullnode configuration. See NodeConfig https://github.com/aptos-labs/diem-core/blob/main/config/src/config/mod.rs |
...
| validator.config | object | `{}` | Validator configuration. See NodeConfig https://github.com/aptos-labs/diem-core/blob/main/config/src/config/mod.rs |
...
| validator.image.repo | string | `"diemlabs/validator"` | Image repo to use for validator images |
...
StatefulSets:
* `<RELEASE_NAME>-diem-node-0-validator` - The validator StatefulSet
* `<RELEASE_NAME>-diem-node-0-fullnode-e<ERA>` - The fullnode StatefulSet

Deployments:
* `<RELEASE_NAME>-diem-node-0-validator` - The HAProxy deployment
...
Services:
* `<RELEASE_NAME>-diem-node-0-validator-lb` - Inbound load balancer service that routes to the validator
* `<RELEASE_NAME>-diem-node-0-fullnode-lb` - Inbound load balancer service that routes to the fullnode
