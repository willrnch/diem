provider "kubernetes" {
  host                   = digitalocean_kubernetes_cluster.diem.endpoint
  cluster_ca_certificate = base64decode(digitalocean_kubernetes_cluster.diem.kube_config[0].cluster_ca_certificate)
  token                  = digitalocean_kubernetes_cluster.diem.kube_config[0].token
}

resource "kubernetes_namespace" "diem" {
  metadata {
...
resource "helm_release" "fullnode" {
  ...
  values = [
    jsonencode({
      ...
      nodeSelector = {
        "doks.digitalocean.com/node-pool" = digitalocean_kubernetes_cluster.diem.node_pool[0].name
      }
      storageClass = {
