...
resource "google_service_account_iam_binding" "backup" {
  ...
  members            = [for i in range(var.num_fullnodes) : "serviceAccount:${google_container_cluster.diem.workload_identity_config[0].workload_pool}[${var.k8s_namespace}/pfn${i}-diem-fullnode]"]
}
