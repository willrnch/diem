...
def get_release_number_from_release_tag(release_tag: str) -> str:
    """Get the release number from the release tag. The release tag looks like diem-node-vX.Y.Z"""
...
def main():
    required_envs = ["RELEASE_TAG", "DIEM_NODE_CARGO_TOML"]
    ...
    DIEM_NODE_TOML = os.environ.get("DIEM_NODE_CARGO_TOML")

    new_lines = []  # construct the file again
    with open(DIEM_NODE_TOML, "r") as f:
        lines = f.readlines()
        for line in lines:
            new_line = line
    ...
    with open(DIEM_NODE_TOML, "w") as f:
...
class CheckDiemNodeVersionTests(unittest.TestCase):
    def test_envs_not_specified(self):
        with self.assertRaises(SystemExit):
            os.environ["RELEASE_TAG"] = ""
            os.environ["DIEM_NODE_CARGO_TOML"] = ""
            main()
    ...
    def test_version_match(self):
        ...
        os.environ["DIEM_NODE_CARGO_TOML"] = tmp.name
