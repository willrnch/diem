#!/usr/bin/env python3
r"""
This script installs the Diem CLI.
...
SCRIPT = "diem.exe" if WINDOWS else "diem"
...
def bin_dir() -> Path:
    if WINDOWS and not MINGW:
        ...
        return Path("~/.diemcli/bin").expanduser()
    else:
        return Path("~/.local/bin").expanduser()


PRE_MESSAGE = """Welcome to the {diem} CLI installer!

This will download and install the latest version of the {diem} CLI at this location:

{diem_home_bin}
"""

POST_MESSAGE = """The {diem} CLI ({version}) is installed now. Great!
...
POST_MESSAGE_NOT_IN_PATH = """The {diem} CLI ({version}) is installed now. Great!

To get started you need the {diem} CLI's bin directory ({diem_home_bin}) in your `PATH`
environment variable.
{configure_message}
Alternatively, you can call the {diem} CLI explicitly with `{diem_executable}`.
...
class Installer:
    ...
    METADATA_URL = (
        "https://api.github.com/repos/aptos-labs/diem-core/releases?per_page=100"
    )
    ...
    def latest_release_info(self):
        ...
        for release in self.release_info:
            if release["tag_name"].startswith("diem-cli-"):
                return release
    ...
    def install(self, version, target):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            zip_file = os.path.join(tmpdirname, "diem-cli.zip")
            urlretrieve(url, zip_file)
            # This assumes that the binary within the zip file is always
            # called `diem` / `diem.exe`.
    ...
    def build_binary_url(self, version: str, target: str) -> str:
        return f"https://github.com/aptos-labs/diem-core/releases/download/diem-cli-v{version}/diem-cli-{version}-{target}.zip"

    def display_pre_message(self) -> None:
        kwargs = {
            "diem": colorize("info", "Diem"),
            "diem_home_bin": colorize("comment", self.bin_dir),
        }
    ...
    def display_post_message_windows(self, version: str) -> None:
        ...
        self._write(
            message.format(
                ...
                diem_home_bin=colorize("comment", self.bin_dir),
                diem_executable=colorize("b", self.bin_path),
    ...
    def _get(self, url):
        request = Request(url, headers={"User-Agent": "Diem CLI Installer"})
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Installs the latest version of the Diem CLI"
    )
    parser.add_argument(
        "-f",
        "--force",
    ...
    try:
        ...
        installer._write(colorize("error", "Diem CLI installation failed."))
        ...
        if e.log is not None:
            ...
            _, path = tempfile.mkstemp(
                suffix=".log",
                prefix="diem-cli-installer-error-",
                dir=str(Path.cwd()),
                text=True,
            )
