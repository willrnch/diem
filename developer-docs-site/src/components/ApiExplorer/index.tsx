...
const ApiExplorer = ({ network, layout }: ApiExplorerProps) => {
  ...
  useEffect(() => {
    ...
    const fetchAndFixSpec = async () => {
      // Get the spec file.
      const data = await fetch(`https://raw.githubusercontent.com/aptos-labs/diem-core/${network}/api/doc/spec.json`);
      // Convert the data to json.
      const json = await data.json();
      // Change the `servers` field in the spec file to point to the public
      // API fullnode for that network.
      json.servers = [{ url: `https://fullnode.${network}.diemlabs.com/v1` }];
