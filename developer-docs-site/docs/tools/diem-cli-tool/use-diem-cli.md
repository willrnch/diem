---
title: "Use the Diem CLI"
id: "use-diem-cli"
---

# Use the Diem CLI

The `diem` tool is a command line interface (CLI) for developing on the Diem blockchain, debugging, and for node operations. This document describes how to use the `diem` CLI tool. To download or build the CLI, follow [Install Diem CLI](../install-cli/index.md).

## Command line help

Command line help is available.  Type `diem help` or `diem --help` to see the available command options. See below the usage output from `diem --help`:
...
SUBCOMMANDS:
    ...
    config        Tool for interacting with configuration of the Diem CLI tool
    genesis       Tool for setting up an Diem chain Genesis transaction
    governance    Tool for on-chain governance
    help          Print this message or the help of the given subcommand(s)
    info          Show build information about the CLI
    init          Tool to initialize current directory for the diem tool
...
Command-specific help is also available.  For example, see below the usage output from `diem move --help`:

```bash

USAGE:
    diem move <SUBCOMMAND>
...
SUBCOMMANDS:
    ...
    create-resource-account-and-publish-package
            Publishes the modules in a Move package to the Diem blockchain under a resource account
    disassemble
    ...
    publish
            Publishes the modules in a Move package to the Diem blockchain
    run
            Run a Move function
    run-script
            Run a Move script
...
Help for sub-commands is also available.  For example, see below the usage output from `diem move compile --help`:

```bash

USAGE:
    diem move compile [OPTIONS]
...
Run the `diem info` command to get the CLI information for debugging purposes. See an example output of the `diem info` command:
...
1. Your configurations are in a **local** YAML configuration file `.diem/config.yaml`, i.e., located in the current working directory where you run the CLI. In this case you must run your CLI commands from this current working directory for this configuration to be used.
2. You can verify that the CLI is set to use this local configuration YAML file by running the command:

  ```bash
  diem config show-global-config
  ```
  ...
  ```
  The `Workspace` value for the `config_type` indicates that the `.diem/config.yaml` file is used for the CLI configuration.

### In the home directory for the global runs

1. Your configurations are in a **global** YAML configuration file `~/.diem/global_config.yaml`, i.e., located in your home directory.
...
If you did not set any global configuration, then the `./.diem/config.yaml` in the current working directory is used for configuration.
:::
...
```bash
diem config generate-shell-completions --shell zsh --output-file ~/.oh-my-zsh/completions/_diem
```

## Initialize local configuration and create an account

A local folder named `.diem/` will be created with a configuration `config.yaml` which can be used to store configuration between CLI runs.  This is local to your run, so you will need to continue running CLI from this folder, or reinitialize in another folder.

### Step 1: Run Diem init

The `diem init` command will initialize the configuration with the private key you provided.

```bash
$ diem init
Configuring for profile default
Enter your rest endpoint [Current: None | No input: https://fullnode.devnet.diemlabs.com]

No rest url given, using https://fullnode.devnet.diemlabs.com...
Enter your faucet endpoint [Current: None | No input: https://faucet.devnet.diemlabs.com]

No faucet url given, using https://faucet.devnet.diemlabs.com...
...
Diem is now set up for account 00f1f20ddd0b0dd2291b6e42c97274668c479bca70f07c6b6a80b99720779696!  Run `diem help` for more information about commands
...
To change the configuration, you can either run the command `diem init` or you can manually edit the `.diem/config.yaml` that is in your current working directory.
...
Diem is now set up for account 18B61497FD290B02BB0751F44381CADA1657C2B3AA6194A00D9BC9A85FAD3B04!  Run `diem help` for more information about commands
{
  "Result": "Success"
}
```
...
You can fund an account with the faucet via the CLI by using either an account address or with `default` (which defaults to the account address created with `diem init`).

For example, to fund the account `00f1f20ddd0b0dd2291b6e42c97274668c479bca70f07c6b6a80b99720779696` that was created above with the `diem init` command:

```bash
$ diem account fund-with-faucet --account 00f1f20ddd0b0dd2291b6e42c97274668c479bca70f07c6b6a80b99720779696
{
...
```bash
$ diem account list --query balance --account 00f1f20ddd0b0dd2291b6e42c97274668c479bca70f07c6b6a80b99720779696
```
or by specifying the `default` as below:
```bash
$ diem account list --query balance --account default
```
...
```bash
$ diem account list --query resources --account default
```
or
```bash
$ diem account list --query resources --account 0xf1f20ddd0b0dd2291b6e42c97274668c479bca70f07c6b6a80b99720779696
```
...
```bash
$ diem account list --query resources --account superuser
{
  "Result": [
    {
      "coin": {
...
```bash
$ diem account list --query modules
{
  "Result": [
    {
...
The Diem CLI is a simple wallet as well, and can transfer coins between accounts.
```bash
$ diem account transfer --account superuser --amount 100
{
  "Result": {
    "gas_used": 73,
    "balance_changes": {
...
To allow generating private keys, you can use the `diem key generate` command.  You can generate
...
```bash
$ diem key generate --output-file starts_with_ace.key --vanity-prefix 0xace
{
  "Result": {
...
```bash
% diem key generate --output-file starts_with_bee.key --vanity-prefix 0xbee --vanity-multisig
{
  "Result": {
...
Note the vanity flag documentation from the `diem key generate` help:
...
the `diem` CLI to generate a peer configuration and write it into a file named `peer_config.yaml`.
```bash
$ diem key extract-peer --output-file peer_config.yaml
```
...
Move examples can be found in the [Move section](../../move/move-on-diem/cli).

## Node command examples

This section summarizes how to run a local testnet with Diem CLI. See [Run a Local Testnet with Diem CLI](../../nodes/local-testnet/using-cli-to-run-a-local-testnet.md) for more details.

For Diem CLI commands applicable to validator nodes, see the [Owner](../../nodes/validator-node/operator/staking-pool-operations.md#owner-operations-with-cli) and [Voter](../../nodes/validator-node/voter/index.md#steps-using-diem-cli) instructions.

### Running a local testnet

You can run a local testnet from the diem CLI, that will match the version it was built with.  Additionally, it can
...
The `diem` tool supports bootstrapping new blockchains through what is known as a genesis ceremony. The output of the genesis ceremony is the output of move instructions that prepares a blockchain for online operation. The input consists of:
...
* For test chains, there also exists an account that manages the minting of DiemCoin

## Generating genesis

* The genesis organizer constructs a `Layout` and distributes it.
* The genesis organizer prepares the Diem framework's bytecode and distributes it.
...
* Each participant begins their `diem-node`. The `diem-node` verifies upon startup that the `genesis.blob` with the waypoint provided by the genesis organizer .
* The blockchain will begin consensus after a quorum of stake is available.

### Prepare diem-core

The following sections rely on tools from the Diem source. See [Building Diem From Source](../../guides/building-from-source.md) for setup.

### The `layout` file

The layout file contains:
* `root_key`: an Ed25519 public key for DiemCoin management.
* `users`: the set of participants
* `chain_id`: the `ChainId` or a unique integer that distinguishes this deployment from other Diem networks
...
### Building the Diem Framework

From your Diem-core repository, build the framework and package it:
```
cargo run --package framework
mkdir diem-framework-release
cp diem-framework/releases/artifacts/current/build/**/bytecode_modules/* diem-framework-release
```

The framework will be stored within the `diem-framework-release` directory.
...
To generate this using the `diem` CLI:

1. Generate your validator's keys:

```
cargo run --package diem -- genesis generate-keys --output-dir bobs
```

2. Generate your `ValidatorConfiguration`:

```
cargo run --package diem -- \\
...
* Use the `diem` CLI to generate genesis and waypoint:

```
cargo run --package diem -- genesis generate-genesis --local-repository-dir genesis
```

### Starting an `diem-node`
