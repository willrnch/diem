---
title: "Integrate with Diem Wallets"
...
# Integrate with Diem wallets

Decentralized applications often run through a browser extension or mobile application to read onchain data and submit
transactions.  The Diem Wallet Adapter allows for a single interface for apps and wallets to integrate together.

## Implementing the Diem Wallet Adapter
...
For that, we provide an [Diem Wallet Adapter](https://github.com/aptos-labs/diem-wallet-adapter) monorepo for wallet and dapps creators to ease development and ensure a smooth process in building projects on the Diem network.
The Diem Wallet Adapter acts as a service between dapps and wallets and exposes APIs for dapps to interact with the wallets by following our [Wallet Standard](../standards/wallets.md). This in turns allows dapps to support many wallets with minimal integration efforts, and for wallets to follow a known standard and gain visibility.
...
The [Adapter Core package](https://github.com/aptos-labs/diem-wallet-adapter/tree/main/packages/wallet-adapter-core) handles the interaction between the dapp and the wallet. It:
...
The light [Adapter React package](https://github.com/aptos-labs/diem-wallet-adapter/tree/main/packages/wallet-adapter-react) is for dapps to import and use. The package contains a `Provider` and a `Context` to implement and use within your app.
