...
All transactions executed on the Diem blockchain must be signed. This requirement is enforced by the chain for security reasons.
...
- The Diem node's [REST API](https://fullnode.devnet.diemlabs.com/v1/spec#/). The Diem node will generate the signing message, the transaction signature and will submit the signed transaction to the Diem blockchain. However, this approach is not secure.
...
This guide will introduce the concepts behind constructing a transaction, generating the appropriate message to sign using the BCS-serialization, and various methods for signing within Diem.

:::tip

We strongly recommend that you use the BCS format for submitting transactions to the Diem blockchain.

:::

## Overview

Creating a transaction that is ready to be executed on the Diem blockchain requires the following four steps:
...
Unsigned transactions are known as `RawTransaction`s. They contain all the information about how to execute an operation on an account within Diem. But they lack the appropriate authorization with a signature or `Authenticator`.

In Diem blockchain, all the data is encoded as [BCS (Binary Canonical Serialization)](#bcs).

Diem supports many different approaches to signing a transaction but defaults to a single signer using the [Ed25519](https://en.wikipedia.org/wiki/EdDSA#Ed25519) signature scheme.

The `Authenticator` produced during the signing of the transaction gives authorization to the Diem blockchain to execute the transaction on behalf of the account owner.
...
- **payload**: Instructions for the Diem blockchain, including publishing a module, execute a script function or execute a script payload.
...
const bytes: Uint8Array = bcs_serialize_string("diem");
...
In addition, in Diem, any content that is signed or hashed is salted with a unique prefix to distinguish it from other types of messages. This is done to ensure that the content can only be used in the intended scenarios. The signing message of a RawTransaction is prefixed with `prefix_bytes`, which is `sha3_256("DIEM::RawTransaction")`. Therefore:
...
For [batch transactions](https://fullnode.mainnet.diemlabs.com/v1/spec#/operations/submit_batch_transactions), create vector of `SignedTransactions`, and then BCS encode the vector as a whole, like so:

```shell
let txn_payload = bcs::to_bytes(&txns.to_vec())?;
```

For example, see the [`pub async fn submit_batch_bcs(`](https://github.com/aptos-labs/diem-core/blob/main/crates/diem-rest-client/src/lib.rs#L448) reference in `lib.rs`. BCS encode only the array.
...
A signature is the result of hashing the signing message with the client's private key. By default Diem uses the [Ed25519](https://en.wikipedia.org/wiki/EdDSA#Ed25519) scheme to generate the signature of the raw transaction.

- By signing a signing message with the private key, clients prove to the Diem blockchain that they have authorized the transaction be executed.
- Diem blockchain will validate the signature with client account's public key to ensure that the transaction submitted is indeed signed by the client.
...
This signed transaction is further BCS-serialized (not shown in the diagram in [Overview](#overview) section), after which the signed transaction is ready for submission to the [Diem REST interface](https://fullnode.devnet.diemlabs.com/v1/spec#/).

### Multisignature transactions

The Diem blockchain supports several signing methods for transactions, including the single signature, the K-of-N multisig, and more.
...
Transaction signatures are wrapped in `Authenticator`. The Diem blockchain validates the transactions submitted by clients by using the Authenticator data. See a few examples below:
...
1. Generate prefix (`prefix_bytes`) with [SHA3-256](https://en.wikipedia.org/wiki/SHA-3) hash bytes of string `DIEM::RawTransaction`.
...
```
curl -X POST -H "Content-Type: application/x.diem.signed_transaction+bcs" --data-binary "@path/to/file_contains_bcs_bytes_of_signed_txn" https://some_domain/transactions
```

[first_transaction]: /tutorials/first-transaction
[account]: /concepts/accounts
[rest_spec]: https://fullnode.devnet.diemlabs.com/v1/spec#/
