...
This is the first chapter of the tutorial on [building an end-to-end dapp on Diem](./index.md). If you haven’t done it, review that introduction, and ensure your environment meets the [prerequisites](../../tools/install-cli/index.md) listed there.
...
2.  `cd` into the new `move` directory and run: `diem move init --name my_todo_list`
...
Take a look at the new `Move.toml` file. You should see your package information and an `DiemFramework` dependency. Note that the `name` property is the same `--name` attribute we passed to the `diem move init` command before. The `DiemFramework` dependency points to the `diem-core/diem-move/framework/diem-framework` GitHub repo main branch.
...
1. In our `move` directory, run `diem init --network devnet`. Press enter when prompted.

   This creates for us a `.diem` directory with a `config.yaml` file that holds our profile information. In the `config.yaml` file, we now have our profiles list that holds a `default` profile. If you open that file, you will see content resembling:
   ...
   :::tip
   You just created a new account on the Diem (dev) network! Yay! You can see it by going to the [Diem Explorer](https://explorer.diemlabs.com/?network=devnet) Devnet network view, pasting the `account` address value from your configuration file into the search field, and clicking on the dropdown option!
   :::

As mentioned, our `sources` directory holds our `.move` module files; so let’s add our first Move file.

2. Open the `Move.toml` file.
3. Add the following code to that Move file, substituting your actual default profile account address from `.diem/config.yaml`:
...
:::tip
Creating an event is not mandatory yet useful if dapps/users want to monitor data, such as how many people create a new task, using the [Diem Indexer](../../integration/indexing.md).
:::
...
4. Run the `diem move compile` command again; If all goes well, we should see a response resembling (where the resulting account address is your default profile account address):
...
```rust
use std::signer;
use diem_std::table::{Self, Table}; // This one we already have, need to modify it
```
...
- Finally, we emit the `task_created` event that holds the new Task data. `emit_event` is an `diem-framework` function that accepts a reference to the event handle and a message. In our case, we are passing the function a reference (using the sign &) to the account’s `TodoListresource` `set_task_event` property as the first argument and a second message argument which is the new Task we just created. Remember, we have a `set_task_event` property in our `TodoList` struct.
...
Let’s stop for one moment and make sure our code compiles by running the `diem move compile` command. If all goes well, we should output resembling:
...
4. Run the `diem move test` command. If all goes right, we should see a success message like:
...
6. Run the `diem move test` command. If all goes right, we should see a success message like:
...
1. `cd` into our `move` directory, and run: `diem move compile`
...
2. Run: `diem move test` and `diem move compile` - all should work without errors.
3. Run: `diem move publish`
...
6. You can now head to the [Diem Explorer](https://explorer.diemlabs.com/), change the dropdown on the top right to the _Devnet_ network and look for that `transaction_hash` value - this will show you the transaction details.
