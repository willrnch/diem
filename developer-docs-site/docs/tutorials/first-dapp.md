...
This lets us use the `window.diem` API without having to do `(window as any).diem`.
...
<details>
<summary>Publish the `hello_blockchain` module with the Diem CLI</summary>
We will use the Diem CLI to compile and publish the `hello_blockchain` module.

1. Download [the `hello_blockchain` package](https://github.com/aptos-labs/diem-core/tree/main/diem-move/move-examples/hello_blockchain).

2. Use the `diem move publish` command (replacing `/path/to/hello_blockchain/` and `<address>`):

```bash
diem move publish --profile my-first-nft --package-dir /path/to/hello_blockchain/ --named-addresses hello_blockchain=<address>
```

For example:

```bash
diem move publish --profile my-first-nft --package-dir ~/code/diem-core/diem-move/move-examples/hello_blockchain/ --named-addresses hello_blockchain=0x5af503b5c379bd69f46184304975e1ef1fa57f422dd193cdad67dc139d532481
```
...
You can also verify the module was published by going to the [Diem Explorer](https://explorer.diemlabs.com/) and looking up your account. If you scroll down to the *Account Modules* section, you should see something resembling:
...
We will use the Diem CLI to compile the `hello_blockchain` module and use the [TypeScript SDK](../sdks/ts-sdk/index.md) to publish the module.

1. Download the [`hello_blockchain`](https://github.com/aptos-labs/diem-core/tree/main/diem-move/move-examples/hello_blockchain) package.

2. Next, use the `diem move compile --save-metadata` command (replacing `/path/to/hello_blockchain/` and `<address>`):

```bash
diem move compile --save-metadata --package-dir /path/to/hello_blockchain/ --named-addresses hello_blockchain=<address>
```

For example:

```bash
diem move compile --save-metadata --package-dir ~/code/diem-core/diem-move/move-examples/hello_blockchain/ --named-addresses hello_blockchain=0x5af503b5c379bd69f46184304975e1ef1fa57f422dd193cdad67dc139d532481
```
...
We would need our account's private key to initialize an `DiemAccount` to publish the module with. You can get the private key from the Petra Wallet by going to: **Settings** > **Manage account**, show the private key, and copy that field. Since a private key is *very* sensitive data, we dont want to expose it in the code but rather hold it in an `.env` file and use it from there.
...
```typescript
import { Types, DiemClient, DiemAccount, HexString, TxnBuilderTypes} from "diem";
  // ...
...
function App() {
  ...
  const onPublishModule = async () => {
    ...
    const diemAccount = new DiemAccount(
      ...
      <div>
        ...
        {publishPackageTxnHash && (
          <div>
            <p>
              <a
                href={`https://fullnode.devnet.diemlabs.com/v1/accounts/${address}/modules`}
                target="_blank"
              >
                Account modules
              </a>
...
This confirms that you are reading and writing messages on the Diem blockchain.
