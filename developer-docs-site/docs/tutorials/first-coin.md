...
This tutorial introduces how you can compile, deploy, and mint your own coin, named [MoonCoin](https://github.com/aptos-labs/diem-core/tree/main/diem-move/move-examples/moon_coin).
...
Run the TypeScript [`your_coin`](https://github.com/aptos-labs/diem-core/blob/main/ecosystem/typescript/sdk/examples/typescript/your_coin.ts) example:

```bash
cd examples/typescript
pnpm your_coin ~/diem-core/diem-move/move-examples/moon_coin
```
...
Run the Python [`your-coin`](https://github.com/aptos-labs/diem-core/blob/main/ecosystem/python/sdk/examples/your-coin.py) example:

```bash
poetry run python -m examples.your-coin ~/diem-core/diem-move/move-examples/moon_coin
```
...
```bash
cd ~/diem-core/diem-move/move-examples/moon_coin
```

Next, build the package using the CLI:

```bash
diem move compile --named-addresses MoonCoin=0x5e603a89cf690d7134cf2f24fdb16ba90c4f5686333721c12e835fb6c76bc7ba --save-metadata
```
...
These are read by the example and published to the Diem blockchain:
...
[`ManagedCoin`](https://github.com/aptos-labs/diem-core/blob/f81ccb01f00227f9c0f36856fead4879f185a9f6/diem-move/framework/diem-framework/sources/managed_coin.move#L1) is a simple coin management framework for coins directly managed by users. It provides convenience wrappers around `mint` and `burn`.
:::
...
Once a coin type has been published to the Diem blockchain, the entity that published that coin type can initialize it:
...
Diem provides several building blocks to support coin transfers:
...
- `diem_account::transfer_coins<CoinType>`: Transfer coins of specific CoinType to a receiver.
