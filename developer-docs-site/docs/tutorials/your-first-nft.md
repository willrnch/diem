...
This tutorial describes how to create and transfer NFTs on the Diem blockchain. The Diem implementation for core NFTs can be found in the [token.move](https://github.com/aptos-labs/diem-core/blob/main/diem-move/framework/diem-token/sources/token.move) Move module.

For reference, also see:
* [mint_nft](https://github.com/aptos-labs/diem-core/tree/main/diem-move/move-examples/mint_nft) Move example on how to airdrop an NFT 
* [mint_nft.rs](https://github.com/aptos-labs/diem-core/blob/main/diem-move/e2e-move-tests/src/tests/mint_nft.rs) Rust end-to-end test
...
<Tabs groupId="sdk-examples">
  ...
  Run the Typescript [`simple_nft`](https://github.com/aptos-labs/diem-core/blob/main/ecosystem/typescript/sdk/examples/typescript/simple_nft.ts) example:
  ```bash
  pnpm run simple_nft
  ```
  </TabItem>
  ...
  Run the Python [`simple-nft`](https://github.com/aptos-labs/diem-core/blob/main/ecosystem/python/sdk/examples/simple-nft.py) example:
  ```bash
...
Alice's token data: {
    ...
    "supply": "1",
    "uri": "https://diem.dev/img/nyan.jpeg"
}
...
See [`simple_nft`](https://github.com/aptos-labs/diem-core/blob/main/ecosystem/typescript/sdk/examples/typescript/simple_nft.ts) for the complete code as you follow the below steps.
:::
  </TabItem>
  <TabItem value="python" label="Python">

:::tip See the full code
See [`simple-nft`](https://github.com/aptos-labs/diem-core/blob/main/ecosystem/python/sdk/examples/simple-nft.py) for the complete code as you follow the below steps.
:::
  </TabItem>
  <TabItem value="rust" label="Rust">
...
In Diem, each account must have an on-chain representation in order to receive tokens and coins and interact with other dApps. An account represents a medium for storing assets; hence, it must be explicitly created. This example leverages the Faucet to create Alice and Bob's accounts:
...
```python
:!: static/sdks/python/diem_sdk/client.py create_collection
```
  </TabItem>
  <TabItem value="rust" label="Rust">
...
```python
:!: static/sdks/python/diem_sdk/client.py create_token
```
  </TabItem>
  <TabItem value="rust" label="Rust">
...
```python
:!: static/sdks/python/diem_sdk/client.py read_token_data_table
```
...
Each token within Diem is a distinct asset. The assets owned by the user are stored within their `TokenStore`. To get the balance:
...
Many users of other blockchains have received unwanted tokens that may cause anything from minimal embarrassment to serious ramifications. Diem gives the rights to each account owner to dictate whether or not to receive unilateral transfers. By default, unilateral transfers are unsupported. So Diem provides a framework for *offering* and *claiming* tokens.
