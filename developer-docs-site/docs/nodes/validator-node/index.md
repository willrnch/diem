...
To participate in the consensus process in the Diem mainnet, you must deploy and run a validator node and a validator fullnode. Optionally you can also run a public fullnode. This document presents a high-level conceptual overview of the important steps involved in deploying the nodes for validation.
...
<div class="col">
  <div class="card h-100" >
    ...
    <p class="card-text">Bootstrapped the nodes. Diem network becomes aware of the nodes.</p>
</div>
</div>
</div>
<div class="col">
...
Follow the documents within this section to run a validator node and a validator fullnode in Diem. Here is a summary of the process:
...
1. Generate identity for the nodes. This is the first step in progressively making your nodes secure and ready to be integrated into the Diem network. 
1. Using YAML files, configure your nodes with user and network identity. This step enables the nodes to be recognized by other nodes in the Diem network. Handshaking is possible after this step.  
1. With the node identity established for the Diem network, next you install the necessary binaries and locally generate the genesis blob and waypoint files. These will allow the node to be connected to the Diem network. 
1. Bootstrap the nodes. The nodes now have the Diem node binary running on them with the identity set. This fulfills the requirement for the Diem network to become aware of your nodes. However, your nodes cannot connect to the Diem network yet because these nodes are not yet in the validator set. On the Diem network a validator can only accept another validator for connections. Until your nodes are in the validator set, they will be rejected by other validator nodes on the network. 
