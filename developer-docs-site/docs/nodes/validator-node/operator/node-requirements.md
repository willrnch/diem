...
For running an Diem **validator node and validator fullnode** we recommend the following hardware resources:
...
There are three types of Diem networks:
...
You can configure the port settings on your node using the configuration YAML file. See the [example configuration YAML here](https://github.com/aptos-labs/diem-core/blob/4ce85456853c7b19b0a751fb645abd2971cc4c0c/docker/compose/diem-node/fullnode.yaml#L10-L9). With this configuration YAML on your node, the public network connects to your node on port 6182 and the VFN network on 6181. Because these port settings are configurable, we don't explicitly say port X is for network Y.
