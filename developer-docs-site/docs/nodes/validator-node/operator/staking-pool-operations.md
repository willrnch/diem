...
[Connect to the Diem network](./connect-to-diem-network.md) and start your node with `validator-identity` and `validator-fullnode-identity` addresses using your staking pool address.
...
The below CLI command examples use mainnet. Change the `--network` value for testnet and devnet. View the values in [Diem Blockchain Deployments](../../deployments.md) to see how profiles can be configured based on the network.
:::

## Perform pool owner operations

This document describes how to [use the Diem CLI](../../../tools/diem-cli-tool/use-diem-cli.md) to perform owner operations during validation.
...
```bash
diem init --profile mainnet-owner \
  --network mainnet
```
...
```bash
diem stake initialize-stake-owner \
...
```bash
diem stake set-delegated-voter \
...
```bash
diem stake increase-lockup --profile mainnet-owner
```

### Unlock stake

```bash
diem stake unlock-stake \
  --amount <amount> \
  --profile mainnet-owner
```
...
### 1. Initialize Diem CLI

  ```bash
  diem init --profile mainnet-operator \
  --network mainnet \
...
Make sure you have enough APT to pay for gas. You can check for this either on the Diem Explorer or using the CLI:

- On the Diem Explorer `https://explorer.diemlabs.com/account/<account-address>?network=Mainnet`, or 
- Use the CLI:

  ```bash
  diem account list --profile mainnet-operator
  ```
...
```bash
diem node update-validator-network-addresses  \
...
```bash
diem node update-consensus-key  \
  --pool-address <pool-address> \
  --operator-config-file ~/$WORKSPACE/$USERNAME/operator.yaml \
  --profile mainnet-operator
```
...
You can see it on the [Diem Explorer](https://explorer.diemlabs.com/validators/all?network=mainnet) or by running the command `diem node get-stake-pool` as shown in [Checking your stake pool information](#checking-your-stake-pool-information).
:::

### 6. Check the validator set
   
When you join the validator set, your validator node will be in "Pending Active" state until the next epoch occurs. **During this time you might see errors like "No connected DiemNet peers". This is normal.** Run the below command to look for your validator in the "pending_active" list.

```bash
diem node show-validator-set --profile mainnet-operator | jq -r '.Result.pending_active' | grep <pool_address>
```
...
```bash
diem node show-validator-set --profile mainnet-operator | jq -r '.Result.active_validators' | grep <pool_address>
```

## Checking your stake pool information

:::tip How validation works
Before you proceed, see [Validation on the Diem blockchain](../../../concepts/staking.md#validation-on-the-diem-blockchain) for a brief overview.
:::
...
:::tip
For testnet or devnet `--url` field values, see [Diem Blockchain Deployments](../../deployments.md).
:::

```bash
diem node get-stake-pool \
...
- This can be set only by the stake pool owner. Operator receives the "commission_percentage" of the generated staking rewards. If you request the commission (you can do so by running the command `diem stake request-commission`), then at the end of the `lockup_expiration_utc_time` the commission part of the rewards will go to the operator address while the rest will stay in the stake pool and belong to the owner. Here "the commission part of the rewards" means the value of **commission_not_yet_unlocked**. 

  For example, in a scenario with a lock up of one month, you call `diem stake request-commission` every month. This will pay out the commission that was accrued during the previous month but only when unlocked at the end of the previous month. Regardless of how often you run `diem stake request-commission` during the month, the commission is only paid out upon the completion of `lockup_expiration_utc_time`.
...
- The date when the commission will unlock. However, this unlocked commission will not be auto-disbursed. It will only disburse when the command `diem stake request-commission` is called again.
...
Either an owner or an operator can request commission. You can request commission at the end of a lockup period, i.e., at the end of **lockup_expiration_utc_time**, by running the `diem stake request-commission` command. Make sure to provide the operator and the owner addresses. See an example command below:

```bash
diem stake request-commission \
...
If you run the `diem stake request-commission` command before the end of the the lockup expiration, the command will initiate unlock for any locked commission earned up until that moment in time.
...
- Successful leader-validator proposals during the current epoch. Also see [Validation on the Diem blockchain](../../../concepts/staking.md#validation-on-the-diem-blockchain) for the distinction between leader-validator and the voter-validator.
...
```bash
diem node analyze-validator-performance \
...
`DistributeEvent` is emitted when there is a transfer from staking_contract to the operator or staker (owner). Rewards can be tracked either by listening to `DistributeEvent` or by using the [View functon](../../../integration/diem-apis.md#reading-state-with-the-view-function) to call `staking_contract_amounts`. This will return `accumulated_rewards` and `commission_amount`.
