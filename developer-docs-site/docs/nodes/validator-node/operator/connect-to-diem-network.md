---
title: "Connect to Diem Network"
slug: "connect-to-diem-network"
---

# Connect to Diem Network

This document describes how to connect your running validator node and validator fullnode to an Diem network. Follow these instructions only if your validator has met the minimal [staking](../../../concepts/staking.md) requirement. 
...
1. Stop your node and remove the data directory. 
   - **Make sure you remove the `secure-data.json` file also**. View [validator.yaml](https://github.com/aptos-labs/diem-core/blob/e358a61018bb056812b5c3dbd197b0311a071baf/docker/compose/diem-node/validator.yaml#L13) to see the location of the `secure-data.json` file. 
2. Download the `genesis.blob` and `waypoint.txt` files published by Diem. 
...
1. Stop your node and remove the data volumes: `docker compose down --volumes`. 
   - **Make sure you remove the `secure-data.json` file too.** See this [validator.yaml](https://github.com/aptos-labs/diem-core/blob/e358a61018bb056812b5c3dbd197b0311a071baf/docker/compose/diem-node/validator.yaml#L13) line for the location of the `secure-data.json` file. 
2. Download the `genesis.blob` and `waypoint.txt` files published by Diem. 
...
2. Update `chain_id` to 1 (for mainnet). The chain IDs for other Diem networks are in [Diem Blockchain Deployments](../../deployments.md).
...
6. **Add monitoring components**
  ...
  :::
     ...
     4. Access the dashboard.
         ...
         ```bash
         kubectl get svc ${WORKSPACE}-mon-diem-monitoring --output jsonpath='{.status.loadBalancer.ingress[0]}'
         ```
...
8. Download the `genesis.blob` and `waypoint.txt` files published by Diem. 
   - See [Node Files](../../node-files-all-networks/node-files.md) for locations and commands to download these files.
...
10. Recreate the secrets. Make sure the secret name matches your `era` number, e.g. if you have `era = 3`, then you should replace the secret name to be:
  ```bash
  ${WORKSPACE}-diem-node-0-genesis-e3
  ```

  ```bash
  export WORKSPACE=<your workspace name>

  kubectl create secret generic ${WORKSPACE}-diem-node-0-genesis-e2 \
...
Now that you have [connected to the Diem network](./connect-to-diem-network.md), you should verify your node connections.
...
1. Verify that your node is connecting to other peers on the network. **Replace `127.0.0.1` with your validator IP/DNS if deployed on the cloud**.
    ...
    ```bash
    curl 127.0.0.1:9101/metrics 2> /dev/null | grep "diem_connections{.*\"Validator\".*}"
    ```

    The command will output the number of inbound and outbound connections of your validator node. For example:

    ```bash
    diem_connections{direction="inbound",network_id="Validator",peer_id="f326fd30",role_type="validator"} 5
    diem_connections{direction="outbound",network_id="Validator",peer_id="f326fd30",role_type="validator"} 2
    ```

    As long as one of the metrics is greater than zero, your node is connected to at least one of the peers on the testnet.

2. You can also check if your node is connected to an Diem node: replace `<Diem Peer ID>` with the peer ID shared by Diem team.

    ```bash
    curl 127.0.0.1:9101/metrics 2> /dev/null | grep "diem_network_peer_connected{.*remote_peer_id=\"<Diem Peer ID>\".*}"
    ```

3. Check if your node is state syncing.

    ```bash
    curl 127.0.0.1:9101/metrics 2> /dev/null | grep "diem_state_sync_version"
    ```
...
4. After your node state syncs to the latest version, you can also check if consensus is making progress, and your node is proposing.
    ...
    ```bash
    curl 127.0.0.1:9101/metrics 2> /dev/null | grep "diem_consensus_current_round"

    curl 127.0.0.1:9101/metrics 2> /dev/null | grep "diem_consensus_proposals_count"
    ```

    You should expect to see this number keep increasing.
    
5. Finally, the most straight forward way to see if your node is functioning properly is to check if it is making staking reward. You can check it on the Diem Explorer: `https://explorer.diemlabs.com/account/<owner-account-address>?network=Mainnet`:
