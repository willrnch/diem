...
This is a step-by-step guide to install an Diem node using source code. Follow these steps to configure a validator node and a validator fullnode on separate machines. Use the `fullnode.yaml` to run a validator fullnode&mdash;see Step 12.
...
6. Create a directory for your Diem node composition, and pick a username for your node. e.g.
    ```bash
...
12. Start your validator by running the below commands, with the paths assuming you are in the root of the `diem-core` directory:

    ```bash
    cargo clean
    cargo build -p diem-node --release
    sudo mv target/release/diem-node /usr/local/bin
    diem-node -f ~/$WORKSPACE/config/validator.yaml
    ```
    ...
    cargo build -p diem-node --release
    sudo mv target/release/diem-node /usr/local/bin
    diem-node -f ~/$WORKSPACE/config/fullnode.yaml
    ```

Optionally, you may set up `diem-node` to run as a service controlled by `systemctl` in a file resembling:
...
WorkingDirectory=/home/nodeuser/diem-core
ExecStart=/usr/local/bin/diem-node -f /home/nodeuser/diem-mainnet/config/validator.yaml
