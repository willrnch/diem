...
1. [Install](../../../tools/install-cli/index.md) and [configure](../../../tools/diem-cli-tool/use-diem-cli.md#configuration-examples) the Diem CLI. If you are looking to develop on the Diem blockchain, debug apps, or perform node operations, the Diem tool offers a command line interface for these purposes.
2. [Initialize local configuration and create an account](../../../tools/diem-cli-tool/use-diem-cli.md#initialize-local-configuration-and-create-an-account) on the Diem blockchain.

## Connect to Diem network

To create a delegation pool and obtain information about it, [connect to the Diem Network](./connect-to-diem-network.md) and launch your own Diem node.
...
- Before you create the delegation pool:  
    ```bash
    diem account derive-resource-account-address --address <owner_address> --seed "diem_framework::delegation_pool<SEED>" --seed-encoding utf8
    ```  
- After you create the delegation pool: 
    ```bash
    diem account derive-resource-account-address
    ```
...
1. Run the command below, substitute in the profile you previously configured during initialization:
    ```bash
    diem move run --profile <your-profile> \ 
...
This section describes the available operations that can be performed on this recently created pool. Once the delegation pool has been established, use the Diem CLI to operate the pool. The available actions that can be performed on it include:
...
* Add `amount` of coins to the delegation pool `pool_address` using the public entry method `add_stake(delegator: &signer, pool_address: address, amount u64)` and substituting your values into the command below before running it:
  ...
  ```bash
  diem move run --profile delegator \
...
Delegation pool owners have access to specific methods designed for modifying the `operator` and `voter` roles of the delegation pool. Use the following Diem CLI commands and include the relevant addresses:

* Set the operator address for the delegation pool:

  ```bash
  diem move run --profile delegation_pool_owner \
...
Until the delegation pool has received 1 million APT and the validator has been added to the set of active validators, there will be no rewards to track during each cycle. In order to obtain information about a delegation pool, use the Diem [View functon](../../../integration/diem-apis.md#reading-state-with-the-view-function).
...
In the [Diem TypeScript SDK](../../../sdks/ts-sdk/index.md), a View function request would resemble:

```bash
import {DiemClient, ViewRequest} from "diem";

const NODE_URL = "https://diem-testnet.public.blastapi.io";
...
```
Alternatively, you can use Diem CLI to call View functions. 

```bash
 diem move view [OPTIONS] --function-id <FUNCTION_ID>
```

To discover the available options and the process for making an `diem move view` call, access the help information with `diem move view --help`. This will display the required arguments for invoking the view functions.
...
1. Get the amount of `active` and `pending_inactive` staking from the [`get_stake`](https://github.com/aptos-labs/diem-core/blob/ed63ab756cda61439287304ed89bbb156fcbeaed/diem-move/framework/diem-framework/sources/delegation_pool.move#L321) view function.
