...
The Diem Node Health Checker (NHC) service can be used to check the health of the following Diem node types:
...
The Diem Node Health Checker now also supports fullnodes via a central web service with validator support on the way:
https://nodetools.diemfoundation.org/#/node_checker
...
**Important**: If your local node is not a devnet fullnode, you must use a different baseline config. See [the configuration examples in diem-core](https://github.com/aptos-labs/diem-core/tree/main/ecosystem/node-checker/configuration_examples) for other such example configs.
...
mkdir /tmp/nhc
cd /tmp/nhc
wget https://raw.githubusercontent.com/aptos-labs/diem-core/main/ecosystem/node-checker/configuration_examples/devnet_fullnode.yaml
```
...
```
docker run -v /tmp/nhc:/nhc -p 20121:20121 -t diemlabs/node-checker:nightly /usr/local/bin/diem-node-checker server run --baseline-config-paths /nhc/devnet_fullnode.yaml
```
...
- A public fullnode connected to the Diem mainnet.
- A validator node connected to the Diem testnet.
- A node running in a single node testnet.

### Baseline configuration

In all the above cases, a baseline node is used to compare your node's health. For example, for a public fullnode connected to the Diem devnet, the baseline node might be a node run by the Diem team and this node demonstrates optimal performance and participation characteristics.

You will download the baseline configuration YAML before running the NHC service for your node. The baseline node's configuration YAML describes where to find this baseline node (URL + port), what evaluators (e.g. metrics checks, TPS tests, API validations, etc.) the NHC service should run, what parameters the NHC should use for those evaluators, what name the configuration has, and so on. See these [example baseline configuration YAML files](https://github.com/aptos-labs/diem-core/tree/main/ecosystem/node-checker/configuration_examples).
...
```
curl 'http://nhc.diemlabs.com/check?node_url=http://myfullnode.mysite.com&baseline_configuration_id=devnet_fullnode'
```
...
You can find a few [example baseline configuration YAML files](https://github.com/aptos-labs/diem-core/tree/main/ecosystem/node-checker/configuration_examples) that work for each of the above use cases and more.
...
mkdir /tmp/nhc
cd /tmp/nhc
configs=(devnet_fullnode testnet_fullnode mainnet_fullnode); for c in ${configs[@]}; do wget https://raw.githubusercontent.com/aptos-labs/diem-core/main/ecosystem/node-checker/configuration_examples/$c.yaml; done
```
...
:::tip
While the Diem team hosts our own instances of this service, we encourage node operators to run their own instances.
:::
...
```
docker run -v /etc/nhc:/etc/nhc -p 20121:20121 -t diemlabs/node-checker:nightly /usr/local/bin/diem-node-checker server run --baseline-config-paths /tmp/nhc/devnet_fullnode.yaml /tmp/nhc/testnet_fullnode.yaml /tmp/nhc/mainnet/fullnode.yaml
```
...
```
git clone git@github.com:aptos-labs/diem-core.git
cd diem-core
```
...
```
cargo run -p diem-node-checker --release -- server run --baseline-config-paths /tmp/nhc/devnet_fullnode.yaml
```
