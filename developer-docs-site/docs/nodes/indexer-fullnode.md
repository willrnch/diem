...
This document describes how to operate an indexer fullnode on the Diem network. To understand and ingest the indexer data in your app, see [Use the Diem Indexer](../integration/indexing.md).
...
For an Diem indexer fullnode, install these packages:
...
1. Create a PostgreSQL user `postgres` with the `createuser` command (find it with `which`):
   ...
   ```
1. Clone `diem-core` repository if you have not already:
    ```bash
    git clone https://github.com/aptos-labs/diem-core.git
    ```
1. Navigate (or `cd`) into `diem-core/crates/indexer` directory.
1.  Create the database schema:
    ```bash
    diesel migration run --database-url postgresql://localhost/postgres
    ```
    This will create a database schema with the subdirectory `migrations` located in this `diem-core/crates/indexer` directory. If for some reason this database is already in use, try a different database. For example: `DATABASE_URL=postgres://postgres@localhost:5432/indexer_v2 diesel database reset`
...
1. Pull the latest indexer Docker image with:
    ```bash
    docker pull diemlabs/validator:nightly_indexer
    ```
...
1. Run the indexer fullnode with either `cargo run` or `docker run` depending upon your setup. Remember to supply the arguments you need for your specific node:
    ...
    docker run -p 8080:8080 \
      ...
      --name=diem-fullnode diemlabs/validator:nightly_indexer diem-node \
      -f /opt/diem/etc/fullnode.yaml
    ```
    or:
    ```bash
    cargo run -p diem-node --features "indexer" --release -- -f ./fullnode.yaml
    ```
