...
- You may want your fullnode to be added to a specific upstream fullnode's allowlist (i.e., another fullnode participant in the Diem network), because:
  ...
  - You may wish to advertise your fullnode for other Diem fullnodes to connect to (to help support the Diem network).
...
Docker container is currently supported only on Linux x86-64 platform. If you are on macOS or Windows platform, use the Diem-core source approach.
...
1. Preparation
    ...
    **Using Docker**
    ...
    Alternatively, if you are on Linux x86-64 platform, you can use the Diem Docker image.

    `cd` into the directory for your local public fullnode and start a Docker container with the latest tools, for example:

    ```bash
    cd ~/my-full-node
    docker run -it diemlabs/tools:devnet /bin/bash
    ```
...
2. Generate the private key
  ...
  **Using Diem-core source code**
  ...
  Run the [Diem CLI](../../tools/diem-cli-tool/use-diem-cli.md) `diem` to produce a hex encoded static x25519 private key. This will be the private key for your network identity.
  ...
  ```bash
  diem key generate --key-type x25519 --output-file /path/to/private-key.txt
  ...
  **Using Docker**
  ...
  Run this step from inside the `diemlabs/tools` Docker container. Open a new terminal and `cd` into the directory where you started the Docker container for your fullnode. Making sure to provide the full path to where you want the private key TXT file to be stored, run the command as below:
...
3. Retrieve the peer identity
  ...
  **Using Diem-core source code**
  ...
  ```bash
  diem key extract-peer --host example.com:6180 \
  ...
  or
  ```bash
  diem key extract-peer --host 1.1.1.1:6180 \
      --public-network-key-file private-key.txt.pub \
      --output-file peer-info.yaml
  ```
  which will produce the following output:
  ...
  **Using Docker**
  ...
  Run the same above commands to extract the peer from inside the `diemlabs/tools` Docker container. For example:
