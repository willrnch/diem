...
# Use the Diem Move CLI

The `diem` tool is a command line interface (CLI) for developing on the Diem blockchain, debugging, and for node operations. This document describes how to use the `diem` CLI tool. To download or build the CLI, follow [Install Diem CLI](../../tools/install-cli/index.md).

## Compiling Move

The `diem` CLI can be used to compile a Move package locally.
The below example uses the `HelloBlockchain` in [move-examples](https://github.com/aptos-labs/diem-core/tree/main/diem-move/move-examples).

The named addresses can be either an account address, or a profile name.

```bash
$ diem move compile --package-dir diem-move/move-examples/hello_blockchain/ --named-addresses hello_blockchain=superuser
```
...
The `diem` CLI can also be used to compile and run unit tests locally.
In this example, we'll use the `HelloBlockchain` in [move-examples](https://github.com/aptos-labs/diem-core/tree/main/diem-move/move-examples).

```bash
$ diem move test --package-dir diem-move/move-examples/hello_blockchain/ --named-addresses hello_blockchain=superuser
```
...
The `diem` CLI can be used to analyze and improve the testing of your Move modules. To use this feature:
1. In your `diem-core` source checkout, navigate to the `diem-move/framework/move-stdlib` directory.
...
| % Move Coverage: 83.50  |
+-------------------------+
Please use `diem move coverage -h` for more detailed test coverage of this package
{
  "Result": "Success"
}
   ```
...
4. Optionally, narrow down your test runs and results to a specific package name with the `--filter` option, like so:
   ```bash
   $ diem move test --coverage --filter vector
   ```
   ...
   ```
5. Run the `diem move coverage` command to obtain more detailed coverage information.
6. Optionally, isolate the results to a module by passing its name to the `--module` option, for example:
   ```bash
   $ diem move coverage source --module signer
   ```
...
The `diem` CLI can be used to run [Move Prover](../../move/prover/index.md), which is a formal verification tool for the Move language. The below example proves the `hello_prover` package in [move-examples](https://github.com/aptos-labs/diem-core/tree/main/diem-move/move-examples).
```bash
...
This *experimental* feature lets you [profile gas usage](https://github.com/aptos-labs/diem-core/tree/main/diem-move/diem-gas-profiling) in the Diem virtual machine locally rather than [simulating transactions](../../concepts/gas-txn-fee.md#estimating-the-gas-units-via-simulation) at the [fullnode](https://fullnode.devnet.diemlabs.com/v1/spec#/operations/simulate_transaction). You may also use it to visualize gas usage in the form of a flame graph.

Run the gas profiler by appending the `--profile-gas` option to the Diem CLI `move publish`, `move run` or `move run-script` command, for example:
```bash
diem move publish --profile-gas
```
...
In this example, we will use `DebugDemo` in [debug-move-example](https://github.com/aptos-labs/diem-core/tree/main/crates/diem/debug-move-example).

Now, you can use `debug::print` and `debug::print_stack_trace` in your [DebugDemo Move file](https://github.com/aptos-labs/diem-core/tree/main/crates/diem/debug-move-example/sources/DebugDemo.move).

You can run the following command:
```bash
$ diem move test --package-dir crates/diem/debug-move-example
```
...
```bash
$ diem move publish --package-dir diem-move/move-examples/hello_blockchain/ --named-addresses HelloBlockchain=8946741e5c907c43c9e042b3739993f32904723f8e2d1491564d38959b59ac71
```

:::tip
As an open source project, the source code as well as compiled code published to the Diem blockchain is inherently open by default. This means code you upload may be downloaded from on-chain data. Even without source access, it is possible to regenerate Move source from Move bytecode. To disable source access, publish with the `--included-artifacts none` argument, like so:

```
diem move publish --included-artifacts none
```
:::

You can additionally use named profiles for the addresses.  The first placeholder is `default`
```bash
$ diem move publish --package-dir diem-move/move-examples/hello_blockchain/ --named-addresses HelloBlockchain=default
```
...
```bash
$ diem move run --function-id 0xb9bd2cfa58ca29bce1d7add25fce5c62220604cd0236fe3f90d9de91ed9fb8cb::message::set_message --args string:hello!
{
  "Result": {
    "changes": [
      {
      ...
      },
      {
        ...
        "event": "write_resource",
        "resource": "0x1::coin::CoinStore<0x1::diem_coin::DiemCoin>"
      },
      {
        "address": "b9bd2cfa58ca29bce1d7add25fce5c62220604cd0236fe3f90d9de91ed9fb8cb",
        "data": {
...
```bash
$ diem move run --function-id default::message::set_message --args string:hello!
{
  "Result": {
    "changes": [
      {
...
This section references the [`CliArgs` example package](https://github.com/aptos-labs/diem-core/tree/main/diem-move/move-examples/cli_args), which contains the following manifest:

import move_toml from '!!raw-loader!../../../../diem-move/move-examples/cli_args/Move.toml';
...
:::tip
Set your working directory to [`diem-move/move-examples/cli_args`](https://github.com/aptos-labs/diem-core/tree/main/diem-move/move-examples/cli_args) to follow along:

```bash
cd <diem-core-parent-directory>/diem-core/diem-move/move-examples/cli_args
```
:::
...
```bash title=Command
diem account fund-with-faucet --account $ace_addr
```
...
After the package has been published, `diem move run` can be used to call `set_vals()`:
...
import entry_json_file from '!!raw-loader!../../../../diem-move/move-examples/cli_args/entry_function_arguments.json';

<CodeBlock language="json" title="entry_function_arguments.json">{entry_json_file}</CodeBlock>

Here, the call to `diem move run` looks like:
...
diem move view --json-file view_function_arguments.json
```
...
import view_json_file from '!!raw-loader!../../../../diem-move/move-examples/cli_args/view_function_arguments.json';
...
diem move compile --named-addresses test_account=$ace_addr
```
...
Next, run `diem move run-script`:

```bash title="Arguments via CLI"
diem move run-script \
...
import script_json_file from '!!raw-loader!../../../../diem-move/move-examples/cli_args/script_function_arguments.json';
...
:::note
As of the time of this writing, the `diem` CLI only supports script function arguments for vectors of type `u8`, and only up to a vector depth of 1. Hence `vector<address>` and `vector<vector<u8>>` are invalid script function argument types.
:::
...
This section builds upon the [Arguments in JSON](#arguments-in-json) section, and likewise references the [`CliArgs` example package](https://github.com/aptos-labs/diem-core/tree/main/diem-move/move-examples/cli_args).

:::tip
If you would like to follow along, start by completing the [Arguments in JSON](#arguments-in-json) tutorial steps!
:::

For this example, Ace and Bee will conduct governance operations from a 2-of-2 "multisig v2" account (an on-chain multisig account per [`multisig_account.move`](https://github.com/aptos-labs/diem-core/blob/main/diem-move/framework/diem-framework/sources/multisig_account.move))
...
```bash title=Command
diem account fund-with-faucet --account $bee_addr
```
...
Use the assorted [`multisig_account.move` view functions](https://github.com/aptos-labs/diem-core/blob/9fa0102c3e474d99ea35a0a85c6893604be41611/diem-move/framework/diem-framework/sources/multisig_account.move#L237) to inspect the multisig:
...
The first multisig transaction enqueued will be a transaction for publication of the [`CliArgs` example package](https://github.com/aptos-labs/diem-core/tree/main/diem-move/move-examples/cli_args).
First, generate a publication payload entry function JSON file:

```bash title="Command"
diem move build-publish-payload \
...
```bash title="Command"
diem multisig create-transaction \
...
diem multisig execute-with-payload \
    --multisig-address $multisig_addr \
    --json-file publication.json \
    --private-key-file bee.key \
    --max-gas 10000 \
...
:::tip
Pending the resolution of [#8304](https://github.com/aptos-labs/diem-core/issues/8304), the transaction simulator (which is used to estimate gas costs) is broken for multisig transactions, so you will have to manually specify a max gas amount.
:::

<details><summary>Output</summary>

Also pending the resolution of [#8304](https://github.com/aptos-labs/diem-core/issues/8304), the CLI output for a successful multisig publication transaction execution results in an API error if only the payload hash has been stored on-chain, but the transaction can be manually verified using an explorer.
