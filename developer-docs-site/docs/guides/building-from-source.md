---
title: "Building Diem From Source"
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Building Diem From Source

[Binary releases are available](../tools/install-cli/index.md), but if you want to build from source or develop on the Diem tools, this is how.

## Supported operating systems

Diem can be built on various operating systems, including Linux, macOS. and Windows. Diem is tested extensively on Linux and macOS, and less so on Windows. Here are the versions we use:
...
1. Install [Git](https://git-scm.com/book/en/v2/Getting-Started-Installing-Git). Git is required to clone the diem-core repo, and will be need to be installed prior to continuing.  You can install it with the instructions on the official [Git website](https://git-scm.com/book/en/v2/Getting-Started-Installing-Git).

1. Clone the Diem repository. To clone the Diem repository (repo), you first need to open a command line prompt (Terminal on Mac / Linux, Powershell on Windows).  Then run the following command to clone the Git repository from GitHub.

      ```
      git clone https://github.com/aptos-labs/diem-core.git
      ```

1. Now let's go into the newly created directory `diem-core` by *changing directory* or `cd`ing into it:
    ```
    cd diem-core
    ```
...
Optionally, check out a release branch to install an Diem node. We suggest you check out `devnet` for your first development. See [Choose a network](./system-integrators-guide.md#choose-a-network) for an explanation of their differences.
...
Prepare your developer environment by installing the dependencies needed to build, test and inspect Diem Core.
...
Now your basic Diem development environment is ready. Head over to our [Developer Tutorials](../tutorials/index.md) to get started in Diem.
