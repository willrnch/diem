...
id: "diem-token-comparison"
---
import ThemedImage from '@theme/ThemedImage';
import useBaseUrl from '@docusaurus/useBaseUrl';

# Compare Diem Token Standards

:::tip Diem token standard
For details, see the official [Diem Token](../../standards/diem-token.md) standard.
:::

One of the major applications of blockchains is token generation and use; this use requires a standard defining
the pattern a token must comply with. The Diem team and communities have been moving
fast and steadily to support various projects and creators to launch their tokens on the Diem
blockchain. As a new L1 blockchain equipped with the [Move programming language](../../move/move-on-diem.md) - a relatively new language exclusively designed for the safety of smart contracts - our token standard inevitably differs from other
blockchains. In this document, we will briefly compare the token standard of Diem with those of Ethereum and Solana.
...
### Diem
The [accounts](../../concepts/accounts.md) in Diem are homogeneous in that they can store both smart contracts and data. But
...
However, in Diem, NFTs are stored under each owner's account, respectively. Other
...
Another interesting trade-off between the account models of Ethereum and Solana/Diem is data locality.
For Ethereum, it is hard to know all the tokens owned by a specific account but easy to get all the
owners of a token type. For Solana and Diem, the opposite is true if no indexing is built. Ethereum
code can be easily written to do some operations like airdrop to all the holders relying on on-chain
data only while Solana/Diem facilitate listing all the tokens an account owns.
...
The Diem token standard shares some similarities with Solana, especially how it covers FT, NFT and SFT in one standard and
...
Diem identifies a token by its `TokenId` that includes `TokenDataId` and `property_version`. The
...
one token type. In Diem, a creator account can have multiple token types created by giving
different collections and token names. 


### Token categorization
it is critical to understand, in Diem, how to categorize different tokens to expect different sets
of features:
...
example, serial numbers or seats for tickets. They could be implemented in Diem by bumping the token `property_version` and
mutating corresponding fields in `token_properties`.
In a nutshell, `Edition` is to Solana token is what `property_version` is to Diem token; but there is no similar concept
in Ethereum token standard.

### Token metadata
Diem token has metadata defined in `TokenData` with the multiple fields that describe widely
...
with a token as well, including `collection` which is defined in `TokenDataId` in Diem. Unfortunately, it fails
