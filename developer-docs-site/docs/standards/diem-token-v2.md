...
## Overview of NFTs
An [NFT](https://en.wikipedia.org/wiki/Non-fungible_token) is a non-fungible [token](https://github.com/aptos-labs/diem-core/blob/main/diem-move/framework/diem-token-objects/sources/token.move) 
...
## Design principles
The [Diem token v2 standard](https://github.com/aptos-labs/diem-core/blob/main/diem-move/framework/diem-token-objects/sources/token.move) 
was developed with the following as an improvement on the Diem Token standard. It has these ideas in mind:
...
these fuctionalities or use "no code" solutions also provided in the framework. One such solution is [diem_token](#diem-token)
which provides functionalities such as custom metadata (via PropertyMap) and soul bound.

## Comparison to Token V1

Token V2 uses Diem [objects](https://github.com/aptos-labs/diem-core/blob/main/diem-move/framework/diem-framework/sources/object.move)
...
In this Token standard, both collections and tokens will be separate [objects](./diem-object.md). They have their own
...
functionality provided by [the royalty module](https://github.com/aptos-labs/diem-core/blob/main/diem-move/framework/diem-token-objects/sources/royalty.move)
Royalty can be updated as long as a `MutatorRef`, a storable struct that grants  permissions, is generated at creation
time and stored.

See [Diem Token](#diem-token) for examples on how Royalty's `MutatorRef` can be stored and used.
...
## Diem Token
[Diem Token](https://github.com/aptos-labs/diem-core/blob/main/diem-move/framework/diem-token-objects/sources/diem_token.move)
...
### Property Map
Similar to in Token Standard v1, Diem Token provides an extensible [PropertyMap](https://github.com/aptos-labs/diem-core/blob/main/diem-move/framework/diem-token-objects/sources/property_map.move)
...
Tokens can be minted using the provided `diem_token::mint`. This is an entry function and can be called via a transaction
directly.
```rust
...
To mint a soul bound token, the creator can call `diem_token::mint_soul_bound` instead. This will create a token that
...
Diem Token is provided as a "no code" convenient solution, but it's not extensible. This is evident as most functions
are entry functions and do not return any ref (constructor, mutator, etc.). The `diem_token` module stores and manages
...
and code from the Diem Token module.
...
```rust
use diem_framework::primary_fungible_store;
