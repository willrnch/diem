...
Any transaction execution on the Diem blockchain requires a processing fee. As of today, this fee comprises two components:
...
When a transaction is submitted to the Diem blockchain, the transaction must contain the following mandatory gas fields:
...
The following gas parameters are set by Diem governance.

:::tip On-chain gas schedule
These on-chain gas parameters are published on the Diem blockchain at `0x1::gas_schedule::GasScheduleV2`.
:::
...
If, let's say, you transfer all the money out of your account so that you have no remaining balance to pay for the transaction fee. In such case the Diem blockchain would let you know that the transaction will fail, and your transfer wouldn't succeed either.
...
1. `estimate_gas_unit_price`: This flag will estimate the gas unit price in the transaction using the same algorithm as the [`estimate_gas_price`](https://fullnode.devnet.diemlabs.com/v1/spec#/operations/estimate_gas_price) API.
