...
An account on the Diem blockchain contains blockchain assets. These assets, for example, coins and NFTs, are by nature
...
Each account on the Diem blockchain is identified by a 32-byte account address. An account can store data, and the account stores
this data in resources. The initial resource is the account data itself (authentication key and sequence number).
Additional resources like currency or NFTs are added after creating the account. And you can employ the [Diem Name Service](../integration/diem-name-service-connector.md) at [www.diemnames.com](https://www.diemnames.com/) to secure .apt domains for key accounts to make them memorable and unique.

Different from other blockchains where accounts and addresses are implicit, accounts on Diem are explicit and need to be
created before they can hold resources and modules. The account can be created explicitly or implicitly by transferring Diem tokens (APT) there.
...
* Native multisig support. Accounts on Diem support multi-ed25519 which allows for a multisig authentication scheme when
...
integrated with the Diem account.

There are two types of accounts in Diem:
...
## Account identifiers
Currently, Diem supports only a single, unified identifier for an account. Accounts on Diem are universally
...
When a user requests to create an account, for example by using the [Diem SDK](https://aptos-labs.github.io/ts-sdk-doc/classes/DiemAccount.html), the following cryptographic steps are executed:
...
Every transaction submitted must contain the current sequence number for the sender account. When the Diem blockchain
...
An account can send transactions. The Diem blockchain supports the following signature schemes: 
...
:::note
The Diem blockchain defaults to single signature transactions.
:::
...
1. **Generate a key-pair**: Generate a fresh key-pair (`privkey_A`, `pubkey_A`). The Diem blockchain uses the PureEdDSA scheme over the Ed25519 curve, as defined in RFC 8032.
...
:::tip Accounts on Diem Testnet
In order to create accounts, the Diem testnet requires the account's public key and an amount of `Coin<TestCoin>` to
add to that account, resulting in the creation of a new account with those two resources.
:::

## Rotating the keys
An Account on Diem has the ability to rotate keys so that potentially compromised keys cannot be used to access the accounts.
...
To simplify this for the system integrators, Diem provides an on-chain mapping via diem account lookup-address.
The on-chain data maps an effective account address as defined by the current mnemonic to the actual account address.

For more information, see [`account.move`](https://github.com/aptos-labs/diem-core/blob/d4a859bb0987f8e35e7471469c3bcd4ae4b49855/diem-move/framework/diem-framework/sources/account.move#L251).
...
then the Diem Move VM translates the identity of the account that signed the transaction into a signer in a Move module entry point.
...
A Move module encodes the rules for updating the Diem blockchain's global state.
- **Move resources**: Move resources contain data but no code. Every resource value has a type that is declared in a module
published in the Diem blockchain's distributed database.

## Preventing replay attacks
When the Diem blockchain processes the transaction, it looks at the sequence number in the transaction and compares it
...
Find more details on creating and using these at [using resource accounts in your app](../move/move-on-diem/resource-accounts.md).
